...
{
   if (a.size() != 15) {
      cout << "-E- InvSym15: size is not correct" << endl;
      return kFALSE;
   }
   ...
   if (det == 0) {
      cout << "-E- InvSym15: zero determinant" << endl;
      return kFALSE;
   }
...
Bool_t Mult25(
   const vector<Double_t>& a,
   const vector<Double_t>& b,
   vector<Double_t>& c)
{
   if (a.size() != 25 || b.size() != 25 || c.size() != 25) {
      cout << "-E- Mult25: size is not correct" << endl;
      return kFALSE;
   }
...
{
   if (a.size() != 25) {
      cout << "-E- Transpose25: size is not correct" << endl;
      return kTRUE;
   }
   vector<Double_t> b(a);
   a[0] = b[0];
   ...
   vector<Double_t>& c)
{
   if (a.size() != 25 || b.size() != 5 || c.size() != 5) {
      cout << "-E- Mult25On5: size is not correct" << endl;
      return kFALSE;
   }
   ...
   vector<Double_t>& c)
{
   if (a.size() != 15 || b.size() != 5 || c.size() != 5) {
      cout << "-E- Mult15On5: size is not correct" << endl;
      return kFALSE;
   }
   c[0] = a[0] * b[0] + a[1] * b[1] + a[2]  * b[2] + a[3]  * b[3] + a[4]  * b[4];
   ...
   vector<Double_t>& c)
{
   if (a.size() != b.size() || a.size() != c.size()) {
      cout << "-E- Subtract: size is not correct" << endl;
      return kFALSE;
   }
...
{
   if (a.size() != b.size() || a.size() != c.size()) {
      cout << "-E- Add: size is not correct" << endl;
      return kFALSE;
   }
   for (unsigned int i = 0; i < a.size(); ++i) { c[i] = a[i] + b[i]; }
   return kTRUE;
   ...
   vector<Double_t>& c)
{
   if (a.size() != 25 || b.size() != 15 || c.size() != 15) {
      cout << "-E- Similarity: size is not correct" << endl;
      return kFALSE;
   }

   Double_t A = a[0] * b[0] + a[1] * b[1] + a[2] * b[2] + a[3] * b[3] + a[4] * b[4];
   Double_t B = a[0] * b[1] + a[1] * b[5] + a[2] * b[6] + a[3] * b[7] + a[4] * b[8];
   Double_t C = a[0] * b[2] + a[1] * b[6] + a[2] * b[9] + a[3] * b[10] + a[4] * b[11];
   Double_t D = a[0] * b[3] + a[1] * b[7] + a[2] * b[10] + a[3] * b[12] + a[4] * b[13];
   Double_t E = a[0] * b[4] + a[1] * b[8] + a[2] * b[11] + a[3] * b[13] + a[4] * b[14];

   Double_t F = a[5] * b[0] + a[6] * b[1] + a[7] * b[2] + a[8] * b[3] + a[9] * b[4];
   Double_t G = a[5] * b[1] + a[6] * b[5] + a[7] * b[6] + a[8] * b[7] + a[9] * b[8];
   Double_t H = a[5] * b[2] + a[6] * b[6] + a[7] * b[9] + a[8] * b[10] + a[9] * b[11];
   Double_t I = a[5] * b[3] + a[6] * b[7] + a[7] * b[10] + a[8] * b[12] + a[9] * b[13];
   Double_t J = a[5] * b[4] + a[6] * b[8] + a[7] * b[11] + a[8] * b[13] + a[9] * b[14];

   Double_t K = a[10] * b[0] + a[11] * b[1] + a[12] * b[2] + a[13] * b[3] + a[14] * b[4];
   Double_t L = a[10] * b[1] + a[11] * b[5] + a[12] * b[6] + a[13] * b[7] + a[14] * b[8];
   Double_t M = a[10] * b[2] + a[11] * b[6] + a[12] * b[9] + a[13] * b[10] + a[14] * b[11];
   Double_t N = a[10] * b[3] + a[11] * b[7] + a[12] * b[10] + a[13] * b[12] + a[14] * b[13];
   Double_t O = a[10] * b[4] + a[11] * b[8] + a[12] * b[11] + a[13] * b[13] + a[14] * b[14];

   Double_t P = a[15] * b[0] + a[16] * b[1] + a[17] * b[2] + a[18] * b[3] + a[19] * b[4];
   Double_t Q = a[15] * b[1] + a[16] * b[5] + a[17] * b[6] + a[18] * b[7] + a[19] * b[8];
   Double_t R = a[15] * b[2] + a[16] * b[6] + a[17] * b[9] + a[18] * b[10] + a[19] * b[11];
   Double_t S = a[15] * b[3] + a[16] * b[7] + a[17] * b[10] + a[18] * b[12] + a[19] * b[13];
   Double_t T = a[15] * b[4] + a[16] * b[8] + a[17] * b[11] + a[18] * b[13] + a[19] * b[14];
   ...
   vector<Double_t>& c)
{
   if (a.size() != 15 || b.size() != 25 || c.size() != 25) {
      cout << "-E- Mult15On25: size is not correct" << endl;
      return kFALSE;
   }
   c[0] = a[0] * b[0] + a[1] * b[5] + a[2] * b[10] + a[3] * b[15] + a[4] * b[20];
   c[1] = a[0] * b[1] + a[1] * b[6] + a[2] * b[11] + a[3] * b[16] + a[4] * b[21];
