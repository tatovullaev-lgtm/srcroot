...
class BmnFileSource : public FairFileSourceBase
{
  public:
    ...
    // BmnFileSource(const BmnFileSource& file);
    ...
    TObjArray* GetListOfFoldersBmn() { return &fListFolderBmn; }
  ...
  private:
    ...
    /** function to extract period and run numbers in the class constructor if exist */
    int ExtractRunNumber();
    /** period number extracted in the class constructor if exists */
    Int_t fPeriodNumber = -1;   //!
    /** run number extracted in the class constructor if exists */
    Int_t fRunNumber = -1;   //!
    /** input file type (data level): 0 - EXP DIGI, 1 - EXP DST, 10 - SIM DIGI, 11 - SIM DST, <0 - errors */
    Int_t fFileType = -1;

    ClassDefOverride(BmnFileSource, 1);
};
