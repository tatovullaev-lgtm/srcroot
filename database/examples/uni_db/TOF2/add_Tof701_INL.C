...
/* 
 * File:   add_Tof701_INL.C
 * Author: mikhailr
 *
 * Created on Sep 11, 2024, 2:08 PM
 */
...
using namespace std;
//add_Tof701_INL(the .ini filename, start period, start run, end period, end run)

void add_Tof701_INL(TString ListOfINLFiles = "TOF701_ListINLFiles_Run8.txt", int sP = 8, int sR = 6310, int eP = 8, int eR = 8427) {
    cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!" << endl;
    ...
    //            for (Int_t ch = 0; ch < CHANNEL_NUMBER; ch++) {
    //                InlSum = 0;
    ...
    UniParameter* pParameter2 = UniParameter::CreateParameter("TOF2_inl", TdcInlType, 0); //(TString parameter_name, int parameter_type)
    delete pParameter2;
    ...
    if (anwser == 'Y' || anwser == 'y') {
        //Upload it
        UniDetectorParameter* t = UniDetectorParameter::CreateDetectorParameter("TOF2", "TOF2_inl", sP, sR, eP, eR, InlData);
        if (t == nullptr) {
