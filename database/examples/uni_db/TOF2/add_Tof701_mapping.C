...
/* 
 * File:   add_Tof2_mapping.C
 * Author: mikhailr
 *
 * Created on Sep 11, 2024, 3:01 PM
 */
 ...
 * 
 */
void add_Tof701_mapping(TString placement_map_filename = "TOF701_PlaceMap_RUN8.txt", TString main_map_filename = "TOF701_StripMap_RUN8.txt", int sP = 8, int sR = 6310, int eP = 8, int eR = 8427) {
    fstream ff;
    ...
    UniParameter* pParameter = UniParameter::CreateParameter("TOF2_TdcMap", MapIntType, 0); //(TString parameter_name, int parameter_type)
    delete pParameter;
    //write detector parameters in DB
    UniDetectorParameter* pDetectorParameter = UniDetectorParameter::CreateDetectorParameter("TOF2", "TOF2_TdcMap", sP, sR, eP, eR, PlMap);
...
//    }
    ...
    //create parameter in DB
        UniParameter* pParameter2 = UniParameter::CreateParameter("TOF2_StripMap", MapDVectorType, 0); //(TString parameter_name, int parameter_type)
        delete pParameter2;
    //write detector parameters in DB
        cout << "Write to DB " << endl;
        UniDetectorParameter* pDetectorParameter2 = UniDetectorParameter::CreateDetectorParameter("TOF2", "TOF2_StripMap", sP, sR, eP, eR, StrMap);
