...
// -------------------------------------------------------------------------
#include "BmnStsVectorFinderV9.h"
...
// -----   Default constructor   -------------------------------------------
BmnStsVectorFinderV9::BmnStsVectorFinderV9()
    : FairTask("STS Vector Finder V9")
    , fHitArray(NULL)
    , fTrackArray(NULL)
    , fEvent(0)
    , fPass(0)
    ...
    ,   // 0,2,3 //AZ-300922
    fTMVAtracks(1)
    ,   // AZ-040525 - discriminate good from fake tracks
    fNSi(4)
    ,   // AZ-051022
    ...
    , fSilConfigFile("SiliconRun8.xml")
{
    ...
    // fOutf = new ofstream("log.txt"); // AZ-290525 - for auxiliary output
}
// -------------------------------------------------------------------------
...
// -----   Public method Init   --------------------------------------------
InitStatus BmnStsVectorFinderV9::Init()
{

    // Get RootManager
    FairRootManager* ioman = FairRootManager::Instance();
    if (!ioman) {
        cout << "-E- BmnStsVectorFinderV9::Init: " << "RootManager not instantiated!" << endl;
        return kFATAL;
    }
    ...
    if (!fClusArray[0]) {
        // cout << "-W- BmnStsVectorFinderV9::Init: "
        ...
        for (int j = 0; j < 4; ++j) {
            ...
            if (!fClusArray[0] && !fClusArray[2]) {   // AZ-241222
                cout << "-W- BmnStsVectorFinderV9::Init: " << "No BmnCluster array!" << j << endl;
                return kERROR;
            }
        }
    }
    ...
    if (!fSilPoints) {
        // cout << "-E- BmnStsVectorFinderV9::Init: No SiliconPoint array!" << endl;
    }

    fVspPoints = (TClonesArray*)ioman->GetObject("VSPPoint");
    if (!fVspPoints) {
    ...
    if (fOut / 3 > 0) {   // AZ-260922
        // Store triplets
        // AZ-240623 fMap3OutPtr = &fMap3Out;
        // AZ-240623 ioman->RegisterAny("Triplets", fMap3OutPtr, kTRUE);
        fStream3out.open("triplets.txt", std::ofstream::out);
    }
    if (fOut && fOut % 2 == 0) {   // AZ-260922
        // Store doublets
        // AZ-240623 fMap2OutPtr = &fMap2Out;
        // AZ-240623 ioman->RegisterAny("Doublets", fMap2OutPtr, kTRUE);
        fStream2out.open("doublets.txt", std::ofstream::out);
    }

    // AZ-140226 fXyzv[0] = 0.4;
    fXyzv[0] = 0.7;   // AZ-140226
    fXyzv[1] = 0.1;
    fXyzv[2] = 0.0;
    ...
    // AZ-230623 static Double_t dTanY[20] =   {0.01, 0.02, 0.02, 0.03}; // window size in TanY vs iteration
    static Double_t dTanY[20] = {0.02, 0.02, 0.02, 0.03};         // window size in TanY vs iteration
    ...
    // AZ-220623 static Double_t dTanY3[20] =  {0.01, 0.02, 0.02, 0.03}; // window size in TanY vs iteration
    static Double_t dTanY3[20] = {0.03, 0.02, 0.015, 0.015};       // AZ-220623 window size in TanY vs iteration
    ...
    // ptCut AZ-120623 static Double_t ptCut[20] =   {1.0,  0.3,  0.7,  0.1}; // min pxz
    static Double_t ptCut[20] = {1.0, 0.7, 0.3, 0.1};   // min pxz - 120623
    ...
    static Double_t curvSta[20] = {
        12.0, 12.0, 12.0, 7.5, 5.5, 4.5,
        3.6,  3.0,  3.0,  3.0, 3.0};   // AZ-310722 - max curvature in stations (for tracks starting in Si)
    static Double_t curvSta41[20] = {
        12.0, 12.0, 12.0, 12.0, 7.5, 5.5, 4.5,
        3.6,  3.0,  3.0,  3.0,  3.0};   // AZ-190625 - max curvature in stations (for tracks starting in Si) 4Si+VSP
    ...
    static Double_t phiXZ41[20] = {1.00, 0.60, 0.63, 0.61, 0.62, 0.95,
                                   0.74, 0.60, 0.51, 0.44, 0.39, 0.35};   // AZ-190625 - 4 Si 6_10_14_18 + VSP
    ...
    static Double_t tanXmax41[20] = {1.50, 0.68, 0.73, 0.69, 0.71, 1.40,
                                     0.91, 0.68, 0.56, 0.47, 0.41, 0.37};   // AZ-190625 - 4 Si 6_10_14_18 + VSP
    ...
    static Double_t zMean41[20] = {7.6,  17.6,  26.4,  35.1,  43.7,  63.6,
                                   94.2, 126.5, 156.8, 189.0, 219.2, 251.4};   // AZ-190625 - 4Si 6_10_14_18 + VSP
    //*/

    FairField* magField = FairRunAna::Instance()->GetField();
    if (magField == nullptr) {
        LOG(error) << "BmnStsVectorFinderV9::Init: magField has null pointer";
        return kERROR;
    }
    ...
    if (fNSi == 5)
        fPhiXZ = phiXZ41;   // AZ-190625 - 4Si+VSP
    fTanXmax = (fNSi == 4) ? tanXmax4 : tanXmax;
    if (fNSi == 5)
        fTanXmax = tanXmax41;   // AZ-190625 - 4Si+VSP
    fZmean = (fNSi == 4) ? zMean4 : zMean;
    if (fNSi == 5)
        fZmean = zMean41;   // AZ-190625 - 4Si+VSP
    // fVarx = dVarX;
    fCurvSta = curvSta;
    if (fNSi == 5)
        fCurvSta = curvSta41;
    ...
    // Read system info
    fTxBinw = 0.01;
    TString fileGraphs = gSystem->ExpandPathName("$VMCWORKDIR");
    fileGraphs += "/parameters/reco/";
    if (fNSi == 4)
        fileGraphs += "fitGraphs_8kG.txt";   // Run 8
    else
        fileGraphs += "fitGraphs_8kG_vsp.txt";   // AZ-220625 - Run 9
    ReadTxInfo(fileGraphs);
    ...
    if (fTMVAtracks)
        InitTMVAtracks();   // AZ-040525 - for tracks
    ...
    cout << "-I- BmnStsVectorFinderV9: Intialisation successfull" << endl;

    return kSUCCESS;
}
// -------------------------------------------------------------------------

void BmnStsVectorFinderV9::ReadTxInfo(TString fileName)
{
    // Read information about Tx windows

    ifstream fin;
    fin.open(fileName);
    if (fin.fail()) {
        cout << " !!! Error opening Tx file " << fileName << endl;
        exit(1);
    }
    TString chline;
    TObjArray* tokens = NULL;
    int ista0 = -1, ista2;
    vector<Float_t> params(10);
    ...
    while (1) {
        ...
        if (ntok == 4) {
            // Station info
            int j = 1;
            ista0 = ((TObjString*)tokens->UncheckedAt(j++))->String().Atoi();
            int ista1 = ((TObjString*)tokens->UncheckedAt(j++))->String().Atoi();
            ista2 = 100 * ista0 + ista1;
            if (fStatData.find(ista2) != fStatData.end())
                continue;
            // Float_t txmax = TMath::Abs (((TObjString*)tokens->UncheckedAt(j++))->String().Atof());
            fTxStep = ((TObjString*)tokens->UncheckedAt(j))->String().Atof();
            unordered_map<int, vector<Float_t>> mstat;
            fStatData[ista2] = mstat;
            // fTanXmax[ista0] = TMath::Abs(txmax); // average max Tx (for upper and lower parts)
            fTanXmax[ista0] = -9;
        } else {
            // int ibin = ((TObjString*)tokens->UncheckedAt(0))->String().Atoi();
            Float_t binc = ((TObjString*)tokens->UncheckedAt(0))->String().Atof();
            int ilr = ((TObjString*)tokens->UncheckedAt(1))->String().Atoi();
            Float_t binc1 = binc;
            if (binc1 > 90.0)
                binc1 -= 100.0;
            if (fTanXmax[ista0] < -8) {
                fTanXmax[ista0] = TMath::Abs(binc1 - fTxStep / 2);
                // cout << ista0 << " " << fTanXmax[ista0] << endl;
            }
            int ibin = FindBin(ista0, fTxStep, binc1);
            if (binc > 90.0)
                ibin += 1000;
            for (int j = 0; j < 5; ++j)
                params[ilr * 5 + j] = ((TObjString*)tokens->UncheckedAt(j + 2))->String().Atof();
            // if (ilr > 0) fStatData[ista2].insert(pair<int,vector<Float_t> >(ibin,params));
            if (ilr > 0)
                fStatData[ista2][ibin] = params;
        }
        if (tokens) {
            tokens->Delete();
            delete tokens;
    ...
    fin.close();
    cout << "-I- BmnStsVectorFinderV9: Station info has been read from " << fileName << endl;
}

// -----   Public method Exec   --------------------------------------------
void BmnStsVectorFinderV9::Exec(Option_t* opt)
{
    ...
    Int_t nHitsOut = 0;
    discarded = 0;
    // AZ-080623 BuildTrackCand();
    FillHitInfo();   // AZ-080623
    ...
    for (Int_t ipass = 0; ipass < nPass; ++ipass) {
        ...
        if (fVerbose > 0) {
            cout << "-I- BmnStsVectorFinderV9: start - " << nHits0 << ", end - " << nHits0 - nHitsOut << endl;
        }
...
Int_t BmnStsVectorFinderV9::ExcludeHits(Int_t minHits, TClonesArray* trArray)
{
...
set<Int_t> BmnStsVectorFinderV9::GetHitId(CbmStsHit* hit, Int_t& idmaxP)
{
...
set<Int_t> BmnStsVectorFinderV9::GetHitIdBmn(CbmStsHit* hit, Int_t& idmaxP)
{
    ...
    if (hit->GetRefIndex() == -1 || fStsPoints == nullptr || fSilPoints == nullptr)
        ...
        else if (hit->GetSystemId() == kVSP)
            p = (FairMCPoint*)fVspPoints->UncheckedAt(hit->GetRefIndex());
        else
...
// AZ-080623 void BmnStsVectorFinder::BuildTrackCand()
void BmnStsVectorFinderV9::FillHitInfo()   // AZ-080623
{
    // Fill hit information

    for (Int_t ist = 0; ist < fNsta; ++ist) {
        // AZ-080623 fmapPhx[ist].clear();
        fmapTx[ist].clear();
        fmapTy[ist].clear();
        fmapX[ist].clear();
        fmapY[ist].clear();
    ...
    // AZ-220725 fTxBinw = 0.01;   // bin width for hits
    ...
    for (Int_t ih = 0; ih < nHits; ++ih) {
        ...
        // if (hit->GetStationNr() > 6) continue;   // AZ-220226 - keep only 1'st GEM
        // if (hit->GetSystemId() != kGEM) continue; //!!!AZ-100123 - exclude Si
        //  else if (hit->GetSystemId() != kGEM && hit->GetStationNr() == 1) continue; //!!!AZ-110123 - exclude station
        //  1 else if (hit->GetSystemId() != kGEM && hit->GetStationNr() == 4) continue; //!!!AZ-110123 - exclude
        //  station 1 if (hit->GetSystemId() == kGEM && hit->GetStationNr() > 7 && TMath::Abs(hit->GetX()) < 10.0 &&
        //  TMath::Abs(hit->GetY()) < 15.0) continue; //!!!AZ-130123 - exclude beam region
        // if (hit->GetSystemId() == kGEM && hit->GetStationNr() > 10) continue;   // AZ-190226 - exclude 2 last
        // stations
        ...
        Double_t tanx = dx / dz;
        // Double_t tanx = dx / (fZmean[ista] - fXyzv[2]); //AZ-090623 - w.r.t. nominal vertex (mean station Z!!!)
        if (fPass >= 0)
            fmapHits[ih] = hitinfo(pos, TMath::ATan2(dx, dz) / fPhiXZ[ista] / fZmean[ista] / by,
                                   // AZ-070623 dx/dz/fTanXmax[ista], dy/dz/fTanXmax[ista]); //AZ-010123
                                   // tanx/fTanXmax[ista], dy/dz/fTanXmax[ista]); //AZ-070623
                                   // AZ-230623 tanx, dy/dz/fTanXmax[ista]); //AZ-080623
                                   tanx, dy / dz);   // AZ-230623
        fmapX[ista].insert(pair<Double_t, Int_t>(pos[0], ih));
        fmapY[ista].insert(pair<Double_t, Int_t>(pos[1], ih));
        // AZ-080623 fmapPhx[ista].insert(pair<Double_t,Int_t>(fmapHits[ih].phx,ih));
        fmapTx[ista].insert(pair<Double_t, Int_t>(fmapHits[ih].tx, ih));
        fmapTy[ista].insert(pair<Double_t, Int_t>(fmapHits[ih].ty, ih));
        // AZ-070623 - hit binning in Tx
        int ista2 = ista * 100 + ista + 1;
        int bin = FindBin(ista, fTxBinw, fmapHits[ih].tx);
        if (bin < 0)
            continue;   // AZ-270623
        int binStat = FindBin(ista, fTxStep, BinCenterTx(ista, bin, fTxBinw));
        if (dy > 0)
            binStat += 1000;   // AZ-270623
        if (fStatData[ista2].find(binStat) == fStatData[ista2].end())
            continue;
        // int bin = (tanx + fTanXmax[ista]) / fTxBinw;
        if (dy > 0)
            bin += 1000;
        // int bin = (tanxMean + fTanXmax[ista]) / fTxBinw;
        fTxBins[ista].insert(pair<int, int>(bin, ih));
    }   // for (Int_t ih = 0;
    /*
    ...
    if (fVerbose > 0) cout << "fNskips fPass " << fNskips[fPass] << " " << fPass << " stastop " << stastop << " fNsta-1
    " << fNsta-1 << endl;
...
void BmnStsVectorFinderV9::BuildDoublets()
{
    // Build doublets
    ...
    // Run8
    const unordered_map<string, float> scaleMap4 = {
        {"01", 1.9},  {"02", 2.8}, {"12", 1.0}, {"13", 1.5},   {"23", 0.65}, {"24", 1.4}, {"34", 0.95},
        {"35", 1.5},  {"45", 1.0}, {"46", 1.5}, {"56", 0.7},   {"57", 1.0},  {"67", 0.6}, {"68", 0.7},
        {"78", 0.35}, {"79", 0.5}, {"89", 0.3}, {"810", 0.35}, {"910", 0.25}};
    // Run 9
    const unordered_map<string, float> scaleMap5 = {
        {"01", 11.5}, {"02", 15.0}, {"12", 1.9},  {"13", 2.8},  {"23", 1.0},  {"24", 1.5},   {"34", 0.65},
        {"35", 1.4},  {"45", 0.95}, {"46", 1.5},  {"56", 1.0},  {"57", 1.5},  {"67", 0.7},   {"68", 1.0},
        {"78", 0.6},  {"79", 0.7},  {"89", 0.35}, {"810", 0.5}, {"910", 0.3}, {"911", 0.35}, {"1011", 0.25}};
    ...
    const unordered_map<string, float>& scaleMap = (fNSi == 4) ? scaleMap4 : scaleMap5;

    Int_t idmaxP = 0;

    for (Int_t ista = 0; ista < fNsta - 1; ++ista) {
        int nHits = fTxBins[ista].size();
        // cout << " nHits: " << ista << " " << nHits << endl;
        if (nHits == 0)
            continue;
        string ssta0 = to_string(ista);

        for (int istanext = ista + 1; istanext < ista + 3; ++istanext) {
            // for (int istanext = ista + 1; istanext < ista + 2; ++istanext) { // no jumps over station
            if (istanext > fNsta - 1)
                continue;
            if (fmapX[istanext].size() == 0)
                continue;
            Double_t dty = fdTanY[fPass];   //, dphx = fdTanX[fPass];
            // dty *= 5; //AZ-040822
            // dtx *= 2; //AZ-040822
            int nskips = istanext - (ista + 1);
            int ibin0 = -99, ista2 = ista * 100 + istanext;
            set<Int_t> setTx;
            Float_t tx = 0;
            string ssta2 = ssta0 + to_string(istanext);
            dty *= (scaleMap.find(ssta2)->second);

            for (unordered_multimap<int, int>::iterator mit = fTxBins[ista].begin(); mit != fTxBins[ista].end(); ++mit)
            {
                // AZ-070623 - Loop over binned hits
                int ih = mit->second;
                if (fmapHits[ih].used)
                    continue;   // used hit
                candvec aaa;
                ...
                int ibin = mit->first;
                if (ibin != ibin0) {
                    // Compute dTx constraints for this bin
                    ibin0 = ibin;
                    setTx.clear();
                    Float_t dtx[2] = {0};
                    tx = BinCenterTx(ista, ibin, fTxBinw);
                    if (fBy < 0.2) {
                        // AZ-140226 - for B = 0
                        dtx[0] = -fdTanXB0[fPass] * 2;
                        dtx[1] = fdTanXB0[fPass] * 2;
                    } else {
                        DTxWindow(ista2, tx, ibin / 1000, fPTcut[fPass], dtx);
                        // dtx[0] *= 2; dtx[1] *= 2; //
                        if (istanext < fNSi) {
                            dtx[0] *= 2;
                            dtx[1] *= 2;
                    ...
                    // else { dtx[0] *= 1.5; dtx[1] *= 1.5; }
                    // else { dtx[0] *= 1.5; dtx[1] *= 1.5; } // AZ-260625
                    ...
                    multimap<Double_t, Int_t>::iterator mitxb = fmapTx[istanext].lower_bound(tx + dtx[0]);
                    multimap<Double_t, Int_t>::iterator mitxe = fmapTx[istanext].upper_bound(tx + dtx[1]);
                    for (multimap<Double_t, Int_t>::iterator mit1 = mitxb; mit1 != mitxe; ++mit1)
                        if (fmapHits[mit1->second].used == 0)
                            setTx.insert(mit1->second);
                }

                Double_t ty = fmapHits[ih].ty;
                // AZ-230623 ty *= (fTanXmax[ista] / fTanXmax[istanext]); //AZ-280822
                ...
                set<Int_t> setTy, intersect;
                for (multimap<Double_t, Int_t>::iterator mit1 = mityb; mit1 != mitye; ++mit1)
                    if (fmapHits[mit1->second].used == 0)
                        setTy.insert(mit1->second);
                set_intersection(setTx.begin(), setTx.end(), setTy.begin(), setTy.end(),
                                 std::inserter(intersect, intersect.begin()));
                // if (fVerbose > 0) cout << " Intersect: " << ista << " " << setTx.size() << " " << setTy.size() << " "
                // << intersect.size() << endl;
                ...
                for (set<Int_t>::iterator sit = intersect.begin(); sit != intersect.end(); ++sit) {
                    ...
                    if (fExact) {
                        ...
                        set<Int_t> ids1 = GetHitId(*sit, idmaxP);
                    ...
                    // AZ-220623 aaa1.ty = ty - fmapHits[*sit].ty; //AZ-300922
                    aaa1.ty = (fmapHits[ih].xyz[1] - fmapHits[*sit].xyz[1]) / aaa1.lengxz;   // AZ-220623
                    aaa1.tx = tx - fmapHits[*sit].tx;   // AZ-010123 - for zero mag. field
            ...
            }   // for (unordered_multimap<int,int>::iterator mit = fTxBins[ista].begin();
...
int BmnStsVectorFinderV9::FindBin(int ista, Float_t binw, Float_t tx)
{
    // Find bin number

    return (tx + fTanXmax[ista]) / binw;
}

// -------------------------------------------------------------------------

Float_t BmnStsVectorFinderV9::BinCenterTx(int ista, int ibin, Float_t binw)
{
    // Get Tx bin center

    return -fTanXmax[ista] + binw * (ibin % 1000 + 0.5);
}

// -------------------------------------------------------------------------

void BmnStsVectorFinderV9::DTxWindow(int ista2, Float_t tx, int idu, Float_t curvCut, Float_t* dtx)
{
    // Compute dTx window

    // void DTxWindow(Double_t curv)
    // Float_t params[2][4] = { {-0.00995846, -81.1826, -181.066, -94.9851},
    //			    {0.0032616, 79.4583, 163.907, -415.319} };
    // Float_t tMinMax[2] = {-0.21, 0.1};
    TF1 ff("ff", "pol3", -1, 1);
    TVector3 xyz[3];

    int bintx = FindBin(ista2 / 100, fTxStep, tx) + 1000 * idu;
    // if (fStatData[ista2].find(bintx) == fStatData[ista2].end()) {cout << ista2 << " " << tx << " " << bintx << " " <<
    // fStatData[ista2].size() << " " << endl; exit(0);}
    vector<Float_t>& params = fStatData[ista2][bintx];
    Float_t tMinMax[2] = {params[4], params[9]};

    for (int ilr = 0; ilr < 2; ++ilr) {
        int ip = ilr * 5;
        ff.SetParameters(params[ip], params[ip + 1], params[ip + 2], params[ip + 3]);
        Float_t cmax = ff.Eval(tMinMax[ilr]);
        // if (cmax <= curv) { cout << tMinMax[ilr] << endl; continue; }
        if (cmax <= curvCut) {
            dtx[ilr] = tMinMax[ilr];
            continue;
        }
        xyz[1][0] = tMinMax[ilr] * curvCut / cmax;
        xyz[1][1] = ff.Eval(xyz[1][0]);
        Float_t dx = tMinMax[ilr] / 10;
        for (int j = -1; j < 2; j += 2) {
            xyz[j + 1][0] = xyz[1][0] + j * dx;
            xyz[j + 1][1] = ff.Eval(xyz[j + 1][0]);
        }
        // xyz[0].Print();
        // xyz[1].Print();
        // xyz[2].Print();
        //  Local parabolic approximation
        TVector3 cba = Parabola(xyz[0], xyz[1], xyz[2]);
        // cba.Print();
        Float_t d = cba[1] * cba[1] - 4 * cba[2] * (cba[0] - curvCut);
        // cout << d << endl;
        Float_t x1 = (-cba[1] + TMath::Sqrt(d)) / 2 / cba[2];
        Float_t x2 = (-cba[1] - TMath::Sqrt(d)) / 2 / cba[2];
        Float_t dthx = x1;
        if (x1 * x2 < 0) {
            if ((ilr == 0 && dthx > 0) || (ilr == 1 && dthx < 0))
                dthx = x2;
        } else
            dthx = TMath::Sign(TMath::Min(TMath::Abs(x1), TMath::Abs(x2)), x1);
        dtx[ilr] = dthx;
        if (ilr == 0)
            dtx[ilr] = TMath::Min(dtx[ilr], -0.01f);
        else
            dtx[ilr] = TMath::Max(dtx[ilr], 0.01f);
        // cout << x1 << " " << x2 << " " << dthx << endl;
    }
}

// -------------------------------------------------------------------------

Bool_t BmnStsVectorFinderV9::CheckVarx(Double_t dx, Double_t dz, Double_t tx, Double_t distxz, Double_t& varx)
{
...
void BmnStsVectorFinderV9::BuildTriplets()
{
    // Build triplets
    ...
    // AZ-140725 Double_t pxzCut = (fUseTMVA) ? 0.3 : 0.2; //AZ-041022 - pxz difference cut
    // pxzCut = 0.3; // AZ-220625
    // if (fBy < 0.2) pxzCut = 1.0; //AZ-261222 - zero magnetic field
    // const Float_t pxzScale4[19] = {5.0, 3.8, 3.0, 2.0, 1.2, 1.0, 0.9, 1.1, 1.0};        // AZ-260623 - Run 8
    // const Float_t pxzScale5[19] = {7.0, 7.0, 3.8, 3.0, 2.0, 1.2, 1.0, 0.9, 1.1, 1.0};   // AZ-220625 - Run 9
    // const Float_t dtymax9[19] = {0.014, 0.019, 0.020, 0.016, 0.009, 0.007, 0.007, 0.006, 0.006, 0.006, 0.006, 0.006};
    // // AZ-260625 - sigma
    const Float_t dtymax9[19] = {0.070, 0.095, 0.100, 0.080, 0.045, 0.035,
                                 0.035, 0.030, 0.030, 0.030, 0.03,  0.03};   // AZ-260625 - 5*sigma
    // const Float_t beamWidth9[19] = {0.19, 0.12, 0.10, 0.10, 0.11, 0.12, 0.14, 0.17, 0.19, 0.22}; // AZ-260624 - sigma
    // const Float_t beamWidth9[19] = {0.95, 0.60, 0.50, 0.50, 0.55, 0.60, 0.70, 0.85, 0.95, 1.10, 1.1, 1.1}; //
    // AZ-260624 - 5*sigma const Float_t beamWidth9[19] = {0.19, 0.12, 0.10, 0.09, 0.10, 0.10, 0.11, 0.13, 0.15, 0.19};
    // // AZ-260624 - sigma, fit with "w"
    const Float_t beamWidth9[19] = {0.95, 0.60, 0.50, 0.45, 0.50, 0.50,
                                    0.55, 0.65, 0.75, 0.95, 1.0,  1.0};   // AZ-260624 - 5*sigma

    // const Float_t *pxzScale = (fNSi == 4) ? pxzScale4 : pxzScale5; // AZ-230625
    const Float_t /**pxzScale = pxzScale5,*/ *dtymax = dtymax9, *beamWidth = beamWidth9;   // AZ-270625
    if (fNSi == 4) {                                                                       // AZ-270625
        // pxzScale = pxzScale4;
        dtymax = &dtymax9[1];
        beamWidth = &beamWidth9[1];
    }
    ...
    for (Int_t ista = 0; ista < fNsta - 2; ++ista) {
        ...
        for (mit = fMap2[ista].begin(); mit != fMap2[ista].end(); ++mit) {
            ...
            int nbr = 0, newtr = 0, newtr3 = 0;        //, mult2 = 0;
            ...
            for (mit1 = ret.first; mit1 != ret.second; ++mit1) {
                ...
                if (TMath::Abs(tany - aaa2.ty) > fdTanY3[fPass]) continue;
                ++mult2; // doublet multiplicity in triplets
            }
            }
            */
            ...
            for (mit1 = ret.first; mit1 != ret.second; ++mit1) {
                ...
                // AZ-230623 if (TMath::Abs(tany - aaa2.ty) > fdTanY3[fPass]) continue; //AZ-290822
                    ...
                    continue;   // AZ-010123 - for zero mag. field
                                /* AZ-090525
                                        if (fOut / 3 > 0) {   // AZ-260922 for writing out
                                            string code = to_string(aaa.stahit.begin()->second) + "-" +
                                   to_string(aaa.stahit.rbegin()->second)
                ...
                /* AZ-260625 - reldiff is not a good criterion for first stations and high momenta for a wide beam
                Float_t reldiff = TMath::Abs(aaa.momxz-aaa2.momxz) / (TMath::Abs(aaa.momxz)+TMath::Abs(aaa2.momxz));
                if (fBy > 0.2 && reldiff > 0.9) continue; //AZ-260623
                if (fBy > 0.2 && reldiff / pxzScale[ista] > pxzCut) continue; //AZ-260623
                //if (fBy < 0.2 && TMath::Abs(aaa.momxz-aaa2.momxz) / TMath::Sqrt(fZmean[ista]) > pxzCut) continue;
                //AZ-311222
                */
                // AZ-280625 if (TMath::Abs (aaa.ty - aaa2.ty) > dtymax[ista]) continue; // AZ-260625
                Double_t dtym = dtymax[ista];                    // AZ-280625
                int dsta = aaa2.stahit.rbegin()->first - ista;   // AZ-280625
                if (dsta > 2)
                    dtym *= (2.0 / dsta);   // AZ-280625
                if (TMath::Abs(aaa.ty - aaa2.ty) > dtym)
                    continue;   // AZ-260625

                // AZ-260625 Use parabolic approximation to find X-position of triplet at Ztarg
                map<int, int>::iterator mitt = aaa.stahit.begin();
                TVector3 points[3];
                points[0] = fmapHits[mitt->second].xyz;
                ++mitt;
                points[1] = fmapHits[mitt->second].xyz;
                points[2] = fmapHits[aaa2.stahit.rbegin()->second].xyz;
                for (int j3 = 0; j3 < 3; ++j3)
                    points[j3] -= fXyzv;
                TVector3 cba = Parabola(points[0], points[1], points[2], 2);
                Double_t dist = TMath::Abs(cba[0]) / TMath::Sqrt(points[0].Z());
                // cout << " ddd " << ista << " " << points[0].Z() << " " << points[1].Z() << " " << points[2].Z() << "
                // " << dist << endl;
                Double_t beamW = beamWidth[ista] * 2;   // AZ-290625
                if (dsta > 2)
                    beamW *= (2.0 / dsta);   // AZ-290625
                if (dist > beamW)
                    continue;
                ...
                for (map<Int_t, Int_t>::iterator mit2 = hitMap.begin(); mit2 != hitMap.end(); ++mit2) {
                    ...
                    hit->SetDx(0.08 / TMath::Sqrt(12.0) * 1.2);   // AZ-210125 - as in vers.1
                    /// hit->SetDx(0.05); //AZ-110123 - early estimate
                    // hit->SetDx(0.015);
                    hit->SetDy(0.1234);   // AZ-210125 - as in vers.1
                    // AZ-030123 if (hit->GetStationNr() <= 3) hit->SetDx(0.02/TMath::Sqrt(12.0));
                    if (hit->GetStationNr() <= fNSi)
                        hit->SetDx(0.02 / TMath::Sqrt(12.0));   // AZ-210125 - as in vers.1
                ...
                if (fOut / 3 > 0) {   // AZ-090525 for writing out
            ...
            // AZ-200525
            if (fUseTMVA && nbr > fNbranches) {
                multimap<Double_t, candvec> mapTmp;
                for (multimap<Double_t, candvec>::iterator it = mapDoublets.begin(); it != mapDoublets.end(); ++it) {
                    auto aaa1 = it->second;
                    // Double_t tmvaOut = TMVAOutput3 (aaa1, aaa2, mult2); // AZ-090525
                    //  AZ-280525 Double_t tmvaOut = TMVAOutput3 (aaa1, aaa, mult2); // AZ-090525
                    Double_t tmvaOut = TMVAOutput3(aaa1, aaa, nbr);   // AZ-280525
                    if (tmvaOut < -0.2) {
                        ++discarded;
                        continue;
                    }   // AZ-090525 - use TMVA BDTD method
                    mapTmp.insert(pair<Double_t, candvec>(-tmvaOut, aaa1));
                }
                mapDoublets.clear();
                mapDoublets = mapTmp;
            }
        ...
        if (fVerbose > 0)
            cout << " Triplet stat: " << ista << " " << ncand << " " << discarded << " " << fPass << endl;
    }   // for (Int_t ista = fNsta-1;
}

// -------------------------------------------------------------------------

void BmnStsVectorFinderV9::BuildTracks()
{
...
void BmnStsVectorFinderV9::ExtendTrack(candvec cand)
{
    ...
    // if (fBy < 0.2) pxzCut = 1.0; //AZ-261222 - zero magnetic field
    // const Float_t pxzCuts9pos[19] = {0.043, 0.046, 0.049, 0.060, 0.053, 0.047, 0.050, 0.055, 0.064}; // AZ-290625 -
    // sigma
    const Float_t pxzCuts9pos[19] = {0.215, 0.230, 0.245, 0.300, 0.265,
                                     0.235, 0.250, 0.275, 0.320};   // AZ-270625 - 5*sigma
    // const Float_t pxzCuts9neg[19] = {0.047, 0.043, 0.037, 0.034, 0.032, 0.027, 0.025, 0.024, 0.029}; // AZ-290625 -
    // sigma
    const Float_t pxzCuts9neg[19] = {0.235, 0.215, 0.185, 0.170, 0.160,
                                     0.135, 0.125, 0.120, 0.145};              // AZ-290625 - 5*sigma
    const Float_t* pxzCutsPos = (fNSi == 5) ? pxzCuts9pos : &pxzCuts9pos[1];   // AZ-290625
    const Float_t* pxzCutsNeg = (fNSi == 5) ? pxzCuts9neg : &pxzCuts9neg[1];   // AZ-290625
    ...
    */
    ...
    if (nTra == 0) {
        // No extension found
        if (int(cand.stahit.size()) < fNhitsMin[fPass])
        ...
        if (fCandCodes[cand.stahit.size()].find(code) == fCandCodes[cand.stahit.size()].end()) {
            ...
            // c2ndfMax) return;
            if (int(cand.stahit.size()) >= fNhitsMin[fPass] && cand.stahit.rbegin()->first != ista
    ...
    for (mit = ret.first; mit != ret.second; ++mit) {
        ...
        if (ok) {
            ...
            if (ok && cand.stahit.size() > 2) {
                ...
                //  AZ-270625 if (TMath::Abs(fBy) > 0.2 && dp / (TMath::Abs (cand.momxz) + TMath::Abs(bbb->momxz)) >
                //  pxzCut) { //AZ-311222
                int istabeg = cand.stahit.begin()->first;                                // AZ-290625
                pxzCut = (cand.momxz > 0) ? pxzCutsPos[istabeg] : pxzCutsNeg[istabeg];   // AZ-290625
                int dsta = bbb->stahit.rbegin()->first - istabeg;                        // AZ-290625
                if (dsta > 3)
                    pxzCut *= (3.0 / dsta);   // AZ-290625
                if (TMath::Abs(fBy) > 0.2
                    && dp / (TMath::Abs(cand.momxz) + TMath::Abs(bbb->momxz)) / TMath::Sqrt(TMath::Abs(1 / cand.momxz))
                           > pxzCut)
                {   // AZ-270625
        ...
        if (ok) {
            ...
            if (aaa.stahit.size() == 3) {
                ...
                if (ok) {
                    ...
                    //  AZ-270625 if (TMath::Abs(fBy) > 0.2 && dp / (TMath::Abs (cand.momxz) + TMath::Abs(aaa.momxz)) >
                    //  pxzCut) { //AZ-020123
                    int istabeg = cand.stahit.begin()->first;                                // AZ-290625
                    pxzCut = (cand.momxz > 0) ? pxzCutsPos[istabeg] : pxzCutsNeg[istabeg];   // AZ-290625
                    int dsta = aaa.stahit.rbegin()->first - istabeg;                         // AZ-290625
                    if (dsta > 3)
                        pxzCut *= (3.0 / dsta);   // AZ-290625
                    if (TMath::Abs(fBy) > 0.2
                        && dp / (TMath::Abs(cand.momxz) + TMath::Abs(aaa.momxz))
                                   / TMath::Sqrt(TMath::Abs(1 / cand.momxz))
                               > pxzCut)
                    {   // AZ-270625
                        // different curvatures
                        ok = 0;
...
Double_t BmnStsVectorFinderV9::FitTrack(candvec& cand)
{
    // Fit track candidate
...
// -------------------------------------------------------------------------
//*
Double_t BmnStsVectorFinderV9::FilterHit(candvec& cand, CbmStsTrack& track)
{
    ...
    */
    ...
    // Double_t chi20 = kftr.GetRefChi2();
    ...
    // Double_t dchi2 = kftr.GetRefChi2() - chi20;
...
void BmnStsVectorFinderV9::MakeStsTrack(candvec& cand, string& hitcode, CbmStsTrack& track)
{
    ...
    for (map<Int_t, Int_t>::iterator mit2 = hitMap.begin(); mit2 != hitMap.end(); ++mit2) {
        ...
        hit = (CbmStsHit*)fHitArray->UncheckedAt(mit2->second);
        // cout << " hit: " << mit2->first << " " << mit2->second << endl;
        hit->SetDx(0.08 / TMath::Sqrt(12.0));   // AZ-210125 - as in vers.1
        /// hit->SetDx(0.05); //AZ-110123 - early estimate
        // hit->SetDx(0.015);
        hit->SetDy(0.1234);   // AZ-210125 - as in vers.1
        // AZ-030123 if (hit->GetStationNr() <= 3) hit->SetDx(0.02/TMath::Sqrt(12.0));
        ...
        hit->SetDx(dx / TMath::Sqrt(12.0));   // AZ-210125 - as in vers.1
...
void BmnStsVectorFinderV9::ExtendTracks(Int_t ista)
{
...
void BmnStsVectorFinderV9::FinishEvent()
{
    // Finish event
    /*
    FairRootManager* ioman = FairRootManager::Instance();
    */
    if (fStream2out.is_open()) {
        fStream2out << " Event: " << fEvent << " " << fMap2Out.size() << endl;
        for (unordered_map<string, int>::iterator it = fMap2Out.begin(); it != fMap2Out.end(); ++it)
            fStream2out << it->first << " " << it->second << "\n";
    }
    if (fStream3out.is_open()) {
        fStream3out << " Event: " << fEvent << " " << fMap3Out.size() << endl;
        for (unordered_map<string, int>::iterator it = fMap3Out.begin(); it != fMap3Out.end(); ++it)
            fStream3out << it->first << " " << it->second << "\n";
    }
    ++fEvent;
}
...
void BmnStsVectorFinderV9::Finish()
{
    printf("Work time of BmnStsVectorFinderV9: %4.2f sec.\n", workTime);
    if (fStream2out.is_open())
        fStream2out.close();
    if (fStream3out.is_open())
        fStream3out.close();
...
// AZ-270625 TVector3 BmnStsVectorFinderV9::Parabola(TVector3& pos0, TVector3& pos1, TVector3& pos2)
TVector3 BmnStsVectorFinderV9::Parabola(TVector3& pos0, TVector3& pos1, TVector3& pos2, int ix)   // AZ-270625
{
    // Get parabolic track approximation from 3 points (X vs Z for ix = 2 or Y vs X for ix = 0)
    // y = a*x^2 + bx + c

    Double_t x2[3] = {pos0[2] - fXyzv[2], pos1[2] - fXyzv[2], pos2[2] - fXyzv[2]};
    Double_t y2[3] = {pos0[0] - fXyzv[0], pos1[0] - fXyzv[0], pos2[0] - fXyzv[0]};
    Double_t x0[3] = {pos0[0], pos1[0], pos2[0]};
    Double_t y0[3] = {pos0[1], pos1[1], pos2[1]};
    Double_t *x = x0, *y = y0;
    if (ix == 2) {
        x = x2;
        y = y2;
...
void BmnStsVectorFinderV9::FitTracks()
{
    // Fit tracks
    ...
    for (Int_t ista = 0; ista < fNsta; ++ista) {
        ...
        for (Int_t itra = 0; itra < ntra; ++itra) {
            ...
            /* AZ-140725
            Int_t same = 0; //, sta0 = hitMap.begin()->first;
...
void BmnStsVectorFinderV9::RemoveDoubles()
{
...
Bool_t BmnStsVectorFinderV9::AreTracksDoubles(CbmStsTrack& tr1, CbmStsTrack& tr2)
{
...
// -------------------------------------------------------------------------
/*
void BmnStsVectorFinderV9::RemoveFakes()
{
...
void BmnStsVectorFinderV9::ExcludeFakes()
{
    ...
    // AZ-280623 - kill short tracks (3 or 4 hits without consecutive triplet)
    int nKilled = 0;
    ncand = fVectorArray->GetEntriesFast();
    /* AZ-190125 - does not kill anything
    for (Int_t j = 0; j < ncand; ++j) {
      CbmStsTrack *tr = (CbmStsTrack*) fVectorArray->UncheckedAt(j);
      Int_t nhits = tr->GetNStsHits();
      // AZ-190125 if (nhits > 4) continue;
      if (nhits > 44) continue; // AZ-190125
      // AZ-190125 int sta0 = -1, ncons = 0;
      int sta0 = -1, ncons = 1; // AZ-190125

      for (Int_t jh = 0; jh < nhits; ++jh) {
        Int_t indx = tr->GetStsHitIndex(jh);
        CbmStsHit *hit = (CbmStsHit*) fHitArray->UncheckedAt(indx);
        int sta = hit->GetStationNr();
        if (jh == 0) sta0 = hit->GetStationNr();
        if (sta - sta0 < 2) {
       //fVectorArray->RemoveAt(j);
       ++ncons;
        } else ncons = 1;
        if (ncons >= 3) break;
        sta0 = sta;
      }
      if (ncons < 3) { fVectorArray->RemoveAt(j); ++nKilled; }
    }
    fVectorArray->Compress();
    cout << " !!! Killed w/out triplet: " << nKilled << endl;
    */
    // AZ-030723 - check short tracks for missing hits (remove if nmissMax hits in the acceptance are missing)
    ncand = fVectorArray->GetEntriesFast();
    nKilled = 0;
    ...
    for (Int_t j = 0; j < ncand; ++j) {
        ...
        // if (nhits > 4) continue;
        if (nhits > -4)
            continue;   // AZ-200125 - do not kill
        // if (nhits > 44) continue;
        set<int> setStat;

        for (int ih = 0; ih < nhits; ++ih) {
            int indx = tr->GetStsHitIndex(ih);
            CbmStsHit* hit = (CbmStsHit*)fHitArray->UncheckedAt(indx);
            setStat.insert(hit->GetStationNr());
        }

        CbmKFTrack kftr = CbmKFTrack(*tr);
        fitter.SetKFHits(kftr, tr);
        FairTrackParam param;

        int firstSta = *setStat.begin(), nmisSi = 0, nmisGem = 0, nmiss = 0;

        // Go upstream

        for (int ista = firstSta - 2; ista >= 0; --ista) {
            nmiss = 0;
            if (ista >= fNSi) {
                BmnGemStripStation* stat = fGemStationSet->GetStation(ista - fNSi);
                kftr.Smooth(stat->GetZPosition());
                ...
                for (int imod = 0; imod < stat->GetNModules(); ++imod) {
                    ...
                    // AZ-180125 if (mod->IsPointInsideModule(param.GetX(),param.GetY())) {
                    if (mod->IsPointInsideModule(param.GetX(), param.GetY(), kFALSE)) {   // AZ-180125
                        ++nmiss;
                        ++nmisSi;
                        // cout << " +++ " << param.GetX() << " " << param.GetY() << endl;
                        break;
            ...
            if (nmiss == 0)
                break;   // no need for checking other stations
        }   // for (int ista = firstSta - 2;

        // Check downstream stations
        nmiss = 0;

        for (int ista = firstSta; ista <= fNsta; ++ista) {
            if (setStat.find(ista + 1) != setStat.end())
                continue;
            nmiss = 0;
            if (ista >= fNSi) {
                BmnGemStripStation* stat = fGemStationSet->GetStation(ista - fNSi);
            ...
            if (nmiss == 0)
                break;   // no need for checking other stations
        }   // for (int ista = firstSta; ista <= fNsta;
        ...
        tr->SetFlag(nmisSi * 10 + nmisGem);

    }   // for (Int_t j = 0; j < ncand;
    // cout << " !!! Killed due to missing: " << nKilled << endl;
    fVectorArray->Compress();
...
Double_t BmnStsVectorFinderV9::DxVsMom(Int_t ista, candvec& vec)
{
...
Double_t BmnStsVectorFinderV9::Proxim(Double_t phi0, Double_t phi)
{
...
// -------------------------------------------------------------------------
// AZ-280722 Double_t BmnStsVectorFinderV9::LinearFit(CbmStsTrack *tr, Int_t newtr, Double_t& ty)
Double_t BmnStsVectorFinderV9::LinearFit(candvec& cand,
                                         candvec& cand2,
                                         CbmStsTrack* tr,
                                         Int_t newtr,
...
Double_t BmnStsVectorFinderV9::Curv3(candvec& cand1, candvec& cand2, candvec& cand3, int newtr)
{
    ...
    //  first = fPass;
    //  AZ-270625 if (first) {
    // if (cand3.stahit.size() <= 2 && first) { // AZ-270625 - for doublets
    if (cand3.stahit.size() <= 2) {   // AZ-270625 - for doublets
        // first = 0;
    ...
    // AZ-310722 ++mit;
    //  AZ-270625 if (cand3.stahit.size() > 2) ++mit; //AZ-310722
    ...
    for (; mit != cand3.stahit.end(); ++mit) {
        ...
        // AZ-270625 points3[indx] -= points3[0];
        if (indx)
            points3[indx] -= points3[0];   // AZ-270625
    ...
    // AZ-270625 Double_t pxz = by * rad * sign;
...
set<int> BmnStsVectorFinderV9::KalmanWindow(candvec& cand, int hitIndx)
{
...
std::string BmnStsVectorFinderV9::MakeCode(candvec& cand)
{
    // Make track code
...
void BmnStsVectorFinderV9::PrintHits(candvec& cand)
{
    // Print hits

    for (map<int, int>::iterator mit = cand.stahit.begin(); mit != cand.stahit.end(); ++mit) {
        int ista = mit->first, ih = mit->second;
        CbmStsHit* hit = (CbmStsHit*)fHitArray->UncheckedAt(ih);
        // cout << mit->first << "(" << hit->GetZ() << "):" << mit->second << " - ";
        printf("%d(%5.2f,%5.2f,%5.2f):%d", ista, hit->GetZ(), fmapHits[ih].tx, fmapHits[ih].ty, ih);
        auto ret = fHit2id.equal_range(ih);
        for (unordered_multimap<Int_t, Int_t>::iterator mit1 = ret.first; mit1 != ret.second; ++mit1) {
            if (mit1 == ret.first)
                cout << "*";
            else
                cout << ":";
...
void BmnStsVectorFinderV9::InitTMVA3()
{
    ...
    fReadersTMVA3.insert(pair<int, TMVA::Reader*>(-13, nullptr));   // pass3 for negative
    fReadersTMVA3.insert(pair<int, TMVA::Reader*>(-11, nullptr));
    fReadersTMVA3.insert(pair<int, TMVA::Reader*>(-10, nullptr));
    fReadersTMVA3.insert(pair<int, TMVA::Reader*>(10, nullptr));
    fReadersTMVA3.insert(pair<int, TMVA::Reader*>(11, nullptr));
    fReadersTMVA3.insert(pair<int, TMVA::Reader*>(13, nullptr));

    // --- Create the Reader objects

    for (auto mit = fReadersTMVA3.begin(); mit != fReadersTMVA3.end(); ++mit) {
        TMVA::Reader* reader = new TMVA::Reader("!Color:!Silent");
        fReadersTMVA3[mit->first] = reader;
        ...
        reader->AddVariable("tanx", &var[iii++]);
        reader->AddVariable("tany", &var[iii++]);
        reader->AddVariable("zb", &var[iii++]);
        reader->AddVariable("dty", &var[iii++]);
        // reader->AddVariable( "1/pxz1",                 &var[iii++] );
        // reader->AddVariable( "log(abs(pxz1))",         &var[iii++] );
        reader->AddVariable("log(abs(pxz2))", &var[iii++]);
        reader->AddVariable("dpxz", &var[iii++]);
        reader->AddVariable("corrq[0]", &var[iii++]);
        reader->AddVariable("corrq[1]", &var[iii++]);
        reader->AddVariable("corrq[2]", &var[iii++]);
        // reader->AddVariable( "log(mult+rndm)",         &var[iii++] );
        reader->AddVariable("log(mult)", &var[iii++]);

        // --- Book the MVA methods

        TString dir = "datasetTMVA3/weights";
        dir += mit->first;
        dir += "/";
    ...
    }   // for (auto mit = fReadersTMVA3.begin();
}

// -------------------------------------------------------------------------

Float_t BmnStsVectorFinderV9::TMVAOutput3(candvec& aaa1, candvec& aaa2, int mult)
{
    // Compute TMVA output for triplets: aaa1 - triplet, aaa2 - first doublet

    Float_t* var = fVarTMVA;
    map<Int_t, Int_t>& hitMap = aaa1.stahit;
    CbmStsHit* hit[3] = {nullptr};
    int iii = 0, indx = 0, inds[3];
    inds[0] = inds[1] = inds[2] = 0;

    /// hitMap ---- aaa.stahit
    for (map<Int_t, Int_t>::iterator mit = hitMap.begin(); mit != hitMap.end(); ++mit) {
        inds[indx] = mit->second;
        hit[indx++] = (CbmStsHit*)fHitArray->UncheckedAt(mit->second);
    }

    Double_t dx = hit[1]->GetX() - hit[0]->GetX();
    Double_t dz = hit[1]->GetZ() - hit[0]->GetZ();
    Double_t lenxz1 = TMath::Sqrt(dx * dx + dz * dz);
    Double_t dx1 = hit[2]->GetX() - hit[1]->GetX();
    Double_t dz1 = hit[2]->GetZ() - hit[1]->GetZ();
    Double_t lenxz2 = TMath::Sqrt(dx1 * dx1 + dz1 * dz1);
    Double_t dty = (hit[2]->GetY() - hit[1]->GetY()) / lenxz2 - (hit[1]->GetY() - hit[0]->GetY()) / lenxz1;
    Double_t pxz1 = aaa1.momxz;   // triplet curvature
    Double_t dpxz = (aaa2.momxz - pxz1) / (TMath::Abs(aaa2.momxz) + TMath::Abs(pxz1));

    var[iii++] = hit[0]->GetX() / hit[0]->GetZ();
    var[iii++] = hit[0]->GetY() / hit[0]->GetZ();
    var[iii++] = hit[0]->GetZ();
    var[iii++] = dty;
    // var[iii++] = pxz1;
    var[iii++] = TMath::Log(TMath::Abs(1 / pxz1));
    var[iii++] = dpxz;
    var[iii++] = hit[0]->GetSignalDiv();
    var[iii++] = hit[1]->GetSignalDiv();
    var[iii++] = hit[2]->GetSignalDiv();
    var[iii++] = TMath::Log(mult);

    int iread = (fPass == 2) ? 1 : fPass;   // fPass==2 has the same triplet cuts as fPass==1
    iread += 10;
    if (pxz1 < 0)
        iread = -iread;
    Float_t tmvaout = fReadersTMVA3[iread]->EvaluateMVA("BDTD method");
    /*
    if (fOutf) {
    for (int j = 0; j < iii; ++j) *fOutf << var[j] << " ";
    *fOutf << mult << " " << tmvaout << " " << iread << " " << inds[0] << " " << inds[1] << " "
           << inds[2] << " " << fEvNo << endl;
    }
    */
    return tmvaout;
...
void BmnStsVectorFinderV9::InitTMVA2()
{
...
Float_t BmnStsVectorFinderV9::TMVAOutput2(candvec& aaa1)
{
...
void BmnStsVectorFinderV9::InitTMVAtracks()
{
    // Initialize TMVA package for rejecting fake tracks (4- and 5-hits)
    ...
    Use["BDTD"] = 1;   // decorrelation + Adaptive Boost
    Float_t* var = fVarTMVA;
    fTmvaReaders.insert(pair<int, TMVA::Reader*>(-5, nullptr));
    fTmvaReaders.insert(pair<int, TMVA::Reader*>(-4, nullptr));
    fTmvaReaders.insert(pair<int, TMVA::Reader*>(4, nullptr));
    fTmvaReaders.insert(pair<int, TMVA::Reader*>(5, nullptr));

    // --- Create the Reader objects

    for (auto mit = fTmvaReaders.begin(); mit != fTmvaReaders.end(); ++mit) {
        TMVA::Reader* reader = new TMVA::Reader("!Color:!Silent");
        int nhits = mit->first;
        fTmvaReaders[nhits] = reader;
        ...
        // FIXME
        int iii = 0;
        // reader->AddVariable( "sqrt(xb*xb+yb*yb)",                  &var[iii++] );
        reader->AddVariable("atan2(yb,xb)", &var[iii++]);
        // reader->AddVariable( "zb",                                 &var[iii++] );
        reader->AddVariable("tx", &var[iii++]);
        // reader->AddVariable( "ty",                                 &var[iii++] );
        reader->AddVariable("log(abs(momr))", &var[iii++]);
        reader->AddVariable("chi2", &var[iii++]);
        // reader->AddVariable( "log(sqrt(xextr*xextr+yextr*yextr))", &var[iii++] );
        // reader->AddVariable( "atan2(yextr,xextr)",                 &var[iii++] );
        reader->AddVariable("corrq[0]", &var[iii++]);
        reader->AddVariable("corrq[1]", &var[iii++]);
        reader->AddVariable("corrq[2]", &var[iii++]);
        reader->AddVariable("corrq[3]", &var[iii++]);
        if (TMath::Abs(nhits) == 5)
            reader->AddVariable("corrq[4]", &var[iii++]);
        reader->AddVariable("corrq3", &var[iii++]);
        // reader->AddVariable( "nmiss+rndm",                         &var[iii++] );
        reader->AddVariable("nseqmx", &var[iii++]);
        // reader->AddVariable( "leng",                               &var[iii++] );
        reader->AddVariable("corrn[0]", &var[iii++]);
        reader->AddVariable("corrn[1]", &var[iii++]);
        reader->AddVariable("corrn[2]", &var[iii++]);
        reader->AddVariable("corrn[3]", &var[iii++]);
        if (TMath::Abs(nhits) == 5)
            reader->AddVariable("corrn[4]", &var[iii++]);
        reader->AddVariable("nsigem", &var[iii++]);

        // --- Book the MVA methods

        TString dir = gSystem->Getenv("VMCWORKDIR");
        // TString dir    = "datasetTMVA1/weights";
        dir += "/parameters/reco/datasetTMVAV8/weights";
        // dir += nhits;
        dir += (nhits < 10) ? TMath::Abs(nhits) : nhits / 10;
        if (nhits < 0)
            dir += "neg";
        else
            dir += "pos";
    ...
    }   // for (auto mit = fTmvaReaders.begin();
}

// -------------------------------------------------------------------------

void BmnStsVectorFinderV9::TMVAOutputTra()
{
    // Compute TMVA output for tracks

    // Fill cluster overlap map
    map<Int_t, set<Int_t>> mClusTr;
    int nTracks = fVectorArray->GetEntriesFast();

    for (int itr = 0; itr < nTracks; ++itr) {
        CbmStsTrack* tr = (CbmStsTrack*)fVectorArray->UncheckedAt(itr);
        int nhit = tr->GetNStsHits();
        if (nhit < 4)
            continue;   // exclude 3-hit tracks

        for (int ih = 0; ih < nhit; ++ih) {
            CbmStsHit* hit = (CbmStsHit*)fHitArray->UncheckedAt(tr->GetStsHitIndex(ih));
            // Check clusters
            ...
            for (Int_t side = 0; side < 2; ++side) {
                ...
                mClusTr[iclus].insert(itr);
            }
        }
    }   // for (int itr = 0; itr < ntracks;

    Float_t* var = fVarTMVA;

    for (int itr = 0; itr < nTracks; ++itr) {
        CbmStsTrack* track = (CbmStsTrack*)fVectorArray->UncheckedAt(itr);
        int nhit = track->GetNStsHits();
        // if (nhit < 4 || nhit > 5) return 1.0;
        if (nhit < 4 || nhit > 5) {
            track->SetB(1.0);
            continue;
        }
        const FairTrackParam* param = track->GetParamFirst();
        Float_t momr = 1. / param->GetQp();
        int iq = (momr >= 0) ? 1 : -1;
        TVector3 posb, pose, mom3;
        Float_t corrq[19], corrn[19] = {0}, corrn3 = 0, corrq3 = 0;
        int nmiss = 0, ista0 = -1, nsigem = 0, nseqmx = 1, nseq = 1;

        for (int ih = 0; ih < nhit; ++ih) {
            CbmStsHit* hit = (CbmStsHit*)fHitArray->UncheckedAt(track->GetStsHitIndex(ih));
            Int_t ista = hit->GetStationNr() - 1;
            if (ih == 0) {
                hit->Position(posb);
                ista0 = ista;
            ...
            corrq[ih] = hit->GetSignalDiv();
            corrq3 += TMath::Abs(corrq[ih]);
            ista0 = ista;

            for (Int_t side = 0; side < 2; ++side) {
                Int_t iclus = hit->GetDigi(side);
                int over = mClusTr[iclus].size();
                corrn[ih] += (over - 1);
            }
            corrn3 += corrn[ih];
            if (ista < 4)
                ...
                nsigem += 1;
        }   // for (int ih = 0; ih < nhit;

        nseqmx = TMath::Max(nseqmx, nseq);
        pose -= posb;
        // Float_t leng = pose.Mag() / (nhit-1);

        // Extrapolate track to the target
        CbmKFTrack trKF = CbmKFTrack(*track);
        FairTrackParam par;
        // track.Extrapolate(xyzv[2]);
        trKF.Extrapolate(0.0);
        trKF.GetTrackParam(par);
        par.Momentum(mom3);
        momr = TMath::Sqrt(mom3.X() * mom3.X() + mom3.Z() * mom3.Z());
        par.Position(mom3);

        int iii = 0;
        // var[iii++] = TMath::Sqrt (xb*xb+yb*yb);
        // var[iii++] = posb.Pt();
        // var[iii++] = TMath::ATan2 (yb, xb);
        var[iii++] = posb.Phi();
        // var[iii++] = zb;
        // var[iii++] = posb.Z();
        var[iii++] = param->GetTx();
        var[iii++] = TMath::Log(TMath::Abs(momr));
        var[iii++] = track->GetChi2() / track->GetNDF();
        // var[iii++] = TMath::Log (TMath::Sqrt (xextr*xextr+yextr*yextr));
        // var[iii++] = TMath::ATan2 (yextr, xextr);
        // var[iii++] = TMath::Log (mom3.Pt());
        // var[iii++] = mom3.Phi();
        var[iii++] = corrq[0];
        var[iii++] = corrq[1];
        var[iii++] = corrq[2];
        ...
        // return fTmvaReaders[nhit*iq]->EvaluateMVA("BDTD method");
        track->SetB(fTmvaReaders[nhit * iq]->EvaluateMVA("BDTD method"));
    }   // for (int itr = 0; itr < nTracks;
}

// -------------------------------------------------------------------------

void BmnStsVectorFinderV9::GoToTarget()
{
    // Evaluate vertex coordinates

    int ntracks = fVectorArray->GetEntriesFast();
    Double_t xv = 0, yv = 0, wwwxtot = 0, wwwytot = 0;

    for (int itr = 0; itr < ntracks; ++itr) {
        CbmStsTrack* tr = (CbmStsTrack*)fVectorArray->UncheckedAt(itr);
        FairTrackParam param = *(tr->GetParamFirst());
        fitter.Extrapolate(&param, fXyzv[2], &param);
        // Double_t wwwx = 1 / TMath::Sqrt(param.GetCovariance(0,0));
        // Double_t wwwy = 1 / TMath::Sqrt(param.GetCovariance(1,1));
        Double_t wwwx = 1 / param.GetCovariance(0, 0);
        Double_t wwwy = 1 / param.GetCovariance(1, 1);
        xv += (param.GetX() * wwwx);
        yv += (param.GetY() * wwwy);
        wwwxtot += wwwx;
        wwwytot += wwwy;
        // cout << itr << " " << tr->GetNStsHits() << " " << param.GetX() << " " <<
        // TMath::Sqrt(param.GetCovariance(0,0)) << " "
        //    << param.GetY() << " " << TMath::Sqrt(param.GetCovariance(1,1)) << " " << tr->GetParamFirst()->GetZ() <<
        //    endl;
    }
    cout << " Vertex coords: " << fEvent << " " << fPass << " " << xv / wwwxtot << " " << yv / wwwytot << endl;
}

// -------------------------------------------------------------------------

ClassImp(BmnStsVectorFinderV9)
