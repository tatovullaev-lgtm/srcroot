...
#ifndef BMNSTSVECTORFINDERV9_H
#define BMNSTSVECTORFINDERV9_H 1
...
class BmnStsVectorFinderV9 : public FairTask
{
  ...
  public:
    ...
    int GetNofSiStations() const { return fNSi; }    // AZ-280725

  private:
    // void ReadMatBudget();
    void ReadTxInfo(TString fileName);
    ...
    // AZ-080623 void BuildTrackCand();
    void FillHitInfo();   // AZ-080623
    void BuildDoublets();
    int FindBin(int ista, Float_t binw, Float_t tx);
    Float_t BinCenterTx(int ista, int ibin, Float_t binw);
    void DTxWindow(int ista2, Float_t tx, int idu, Float_t curvCut, Float_t* dtx);
    ...
    TVector3 Parabola(TVector3& pos0, TVector3& pos1, TVector3& pos2, int ix = 0);
    void FitTracks();
    void RemoveDoubles();
    ...
    Float_t TMVAOutput3(candvec& aaa1, candvec& aaa2, int mult2);   // AZ-300922
    void InitTMVA2();                                               // AZ-221022
    Float_t TMVAOutput2(candvec& aaa1);                             // AZ-241022
    void InitTMVAtracks();                                          // AZ-040525
    void TMVAOutputTra();                                           // AZ-040525
    void GoToTarget();                                              // AZ-240625

    /** Input / output array of CbmStsHits **/
    TClonesArray *fClusArray[4], *fHitArray, *fTrackArray, *fDigiMatches, *fStsPoints, *fSilPoints, *fVspPoints;
    ...
    // AZ-080623 std::multimap<Double_t,Int_t> fmapPhx[19], fmapTy[19], fmapX[19], fmapY[19];
    ...
    std::unordered_map<std::string, int> fMap2Out;   // AZ-210922 - doublets for writing out
    // AZ-240623 std::map<std::string,int> *fMap2OutPtr; //AZ-210922 - doublets for writing out
    std::unordered_map<std::string, int> fMap3Out;   // AZ-260922 - triplets for writing out
    // AZ-240623 std::map<std::string,int> *fMap3OutPtr; //AZ-260922 - triplets for writing out
    std::ofstream fStream2out;   // 240623
    std::ofstream fStream3out;   // 240623
    ...
    // AZ-070623 - binned hits in Tx
    std::unordered_multimap<int, int> fTxBins[19];
    std::unordered_map<int, std::unordered_map<int, std::vector<Float_t>>> fStatData;
    Float_t fTxStep;
    Float_t fTxBinw;
    ...
    int fTMVAtracks;        // AZ-040525
    int fNSi;               // AZ-051022
    Float_t fVarTMVA[20];   // AZ-300922
    // TMVA::Reader* fReaderTMVA3;   // AZ-300922
    std::map<int, TMVA::Reader*> fReadersTMVA3;   // AZ-080525 - for triplets
    TMVA::Reader* fReaderTMVA2;                   // AZ-221022
    std::map<int, TMVA::Reader*> fTmvaReaders;    // AZ-040525 - for tracks
    // std::ofstream *fOutf; // AZ-290525

    TString fGemConfigFile;
    TString fSilConfigFile;
    BmnGemStripStationSet* fGemStationSet;   //! Entire GEM detector
    BmnSiliconStationSet* fSilStationSet;    //! Entire SILICON detector

    BmnStsVectorFinderV9(const BmnStsVectorFinderV9&);
    BmnStsVectorFinderV9& operator=(const BmnStsVectorFinderV9&);

    ClassDef(BmnStsVectorFinderV9, 0);
};
