...
{
   //cout << " L1Algo::BranchFitterFast " << endl; exit(1); //AZ-220223
...
#ifdef AZ
  //AZ-230223
  int ihp = vStsHits2Unus[t.StsHits[iFirstHit]];
  x0 = (*vStsHitPointsUnused)[ihp].X();
  y0 = (*vStsHitPointsUnused)[ihp].Y();
#else
...
#ifdef AZ
  //AZ-230223
  ihp = vStsHits2Unus[t.StsHits[iFirstHit+step]];
  x1 = (*vStsHitPointsUnused)[ihp].X();
  y1 = (*vStsHitPointsUnused)[ihp].Y();
#else
...
#ifdef AZ
  //AZ-230223
  ihp = vStsHits2Unus[t.StsHits[iFirstHit+2*step]];
  x2 = (*vStsHitPointsUnused)[ihp].X();
  y2 = (*vStsHitPointsUnused)[ihp].Y();
#else
  ...
  for( int i = iFirstHit + step; step*i <= step*iLastHit; i += step){
...
#ifdef AZ
    //AZ-260223
    ihp = vStsHits2Unus[t.StsHits[i]];
    L1HitPoint &point = (*vStsHitPointsUnused)[ihp];
    x = point.X();
    y = point.Y();
    fvec u = static_cast<fscal> (point.U());
    fvec v = static_cast<fscal> (point.V());
    fvec du = static_cast<fscal> (point.DU());
    fvec dv = static_cast<fscal> (point.DV());
#else
    fvec u = static_cast<fscal>( vStsStrips[hit.f] );
    fvec v = static_cast<fscal>( vStsStripsB[hit.b] );
    fvec du  = static_cast<fscal>( vStsStrips[hit.f].errf() ); //AZ
    fvec dv  = static_cast<fscal>( vStsStripsB[hit.b].errf() ); //AZ
#endif
    ...
    du *= du; //AZ
    dv *= dv; //AZ
    L1Filter( T, sta.frontInfo, u, w, &du); //AZ
    L1Filter( T, sta.backInfo,  v, w, &dv); //AZ
    fB0 = fB1;
    fB1 = fB2;
    fz0 = fz1;
    fz1 = fz2;
...
{
   //cout << " L1Algo::FindMoreHits " << endl; exit(1); //AZ-220223
...
//  int ista = GetFStation(vSFlag[vStsHits[t.StsHits[iFirstHit]].f]) + 2*step; // current station. set to the end of track
  ...
  for( ; (ista < NStations) && (ista >= 0); ista += step ){ // CHECKME why ista2?
    ...
    while( area.GetNext( ih ) ) {
      ...
      xh = point.X(), yh = point.Y(), zh = point.Z();
#else
...
#ifdef AZ
    //AZ-260223
    L1HitPoint &point = (*vStsHitPointsUnused)[iHit_best];
    x = point.X(), y = point.Y();
    fvec u = static_cast<fscal> (point.U());
    fvec v = static_cast<fscal> (point.V());
    fvec du = static_cast<fscal> (point.DU());
    ...
    fvec du = static_cast<fvec>(vStsStrips[hit.f].errf()); //AZ
    fvec dv = static_cast<fvec>(vStsStripsB[hit.b].errf()); //AZ
    StripsToCoor(u, v, x, y, sta);
#endif
    z = vStsZPos[hit.iz];
