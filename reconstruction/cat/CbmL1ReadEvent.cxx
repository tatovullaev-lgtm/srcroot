...
struct TmpHit{ // used for sort Hits before writing in the normal arrays
  ...
  double du_front, du_back; //AZ - errors of strips
  double x, y;            // position of hit
  int iMC;                // index of MCPoint in the vMCPoints array
  double strip1, strip2;  //AZ-140223 - COGs of clusters
...
struct TmpStrip{
  fscal u;
  fscal du; //AZ
  fscal strip; //AZ-140223 - COG of cluster
  int iStation;
  int iSector, iStrip;
  bool isStrip;
...
void CbmL1::ReadEvent()
{
  ...
  algo->vSFlagB.clear();
#ifdef AZ
  algo->vAllHitPoints.clear(); //AZ-240223
#endif
...
//       MC.z = th.iStation; // for sort in right order
  ...
  if( listStsHits ){
    ...
    for (int j=0; j < nEnt; j++ ){
      ...
      TmpHit th;
      {
        ...
        th.u_front = th.x*st.frontInfo.cos_phi[0] + th.y*st.frontInfo.sin_phi[0];
        th.u_back  = th.x* st.backInfo.cos_phi[0] + th.y* st.backInfo.sin_phi[0];
	th.strip1 = mh->GetStrip(0); //AZ-140223
	th.strip2  = mh->GetStrip(1); //AZ-140223
  ...
  for ( int ih = 0; ih<nHits; ih++ ){
...
#ifdef AZ
      if (fabs (s.strip - th.strip1) > 1.e-4) continue; //AZ-140223
#else
...
#ifdef AZ
      if (fabs (s.strip - th.strip2) > 1.e-4) continue; //AZ-140223
#else
    ...
    if( th.indStripF<0 ){
      ...
      tmp.u = th.u_front;
      tmp.du = th.du_front; //AZ
      tmp.isStrip = th.isStrip;
      tmp.strip = th.strip1; //AZ-140223
    ...
    if( th.indStripB<0 ){
      ...
      tmp.isStrip = th.isStrip;
      tmp.u = th.u_back;
      tmp.du = th.du_back; //AZ
      tmp.strip = th.strip2; //AZ-140223
      tmpStripsB.push_back(tmp);
      th.indStripB = NStripsB++;
    }
  } // ih
  //cout << " xxx " << nHits << " " << tmpStrips.size() << " " << tmpStripsB.size() << endl; //AZ-140223
...
//   }
  ...
  for( int i=0; i<NStrips; i++ ){
    ...
    algo->vStsStrips.push_back(L1Strip(ts.u,ts.du)); //AZ
  ...
  for( int i=0; i<NStripsB; i++ ){
    ...
    algo->vStsStripsB.push_back(L1Strip(ts.u,ts.du)); //AZ
...
#ifdef AZ
    //AZ-240223 - save hit point
    L1HitPoint hp (fscal(th.x), fscal(th.y), fscal(z_tmp), fscal(th.u_back), fscal(th.u_front),
		   fscal(th.du_back), fscal(th.du_front));
    algo->vAllHitPoints.push_back(hp);
#endif
