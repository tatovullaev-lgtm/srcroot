...
InitStatus CbmL1::Init()
{
    ...
    for (Int_t ist = 0; ist < NStation; ist++) {
        ...
        if (ist < NMvdStations) {
            ...
            if (fVerbose)
                cout << " AZ - station " << ist << " " << f_phi * TMath::RadToDeg() << " " << b_phi * TMath::RadToDeg()
                     << " " << Xmax << " " << Ymax << endl;
            Xmax = TMath::Abs(Xmax);   // AZ-040623
            Ymax = TMath::Abs(Ymax);   // AZ-040623
        }
...

    {
        ...
        if (fSTAPDataMode >= 2) {   // 2,3
            ...
            if (ind2 != ind)
                cout << "-E- CbmL1: Read geometry from file " << fSTAPDataDir + "geo_algo.txt was NOT successful."
                     << endl;
        };
    }
    ...
    if (fMatBudgetFileName != "") {
        ...
        for (int j = 0, iSta = algo->NMvdStations; iSta < algo->NStations; iSta++, j++) {
            ...
            if (!hStaRadLen) {
                cout << "L1: incorrect " << fMatBudgetFileName << " file. No " << name << endl;
                exit(1);
            }
            ...
            for (int iB = 0; iB < NBins; iB++) {
                ...
                for (int iB2 = 0; iB2 < NBins; iB2++) {
                    algo->fRadThick[iSta].table[iB][iB2] =
                        0.01 * hStaRadLen->GetBinContent(iB, iB2);   // 0.0034;//0.003209;//
...
void CbmL1::Reconstruct()
{
    ...
    if (0) {   // correct hits on MC // dbg
        ...
        for (unsigned int iH = 0; iH < algo->vStsHits.size(); ++iH) {
            ...
            if (vStsHits[iH].mcPointIds.size() == 0)
                continue;
...
#if 1   // GAUSS
            algo->vStsStrips[h.f] = idet * (+sta.yInfo.sin_phi[0] * mcp.x - sta.xInfo.cos_phi[0] * mcp.y)
                                    + random.Gaus(0, sqrt(sta.frontInfo.sigma2)[0]);
            algo->vStsStripsB[h.b] = idet * (-sta.yInfo.cos_phi[0] * mcp.x + sta.xInfo.sin_phi[0] * mcp.y)
                                     + random.Gaus(0, sqrt(sta.backInfo.sigma2)[0]);
#else   // UNIFORM
            algo->vStsStrips[h.f] =
                idet * (+sta.yInfo.sin_phi[0] * mcp.x - sta.xInfo.cos_phi[0] * mcp.y)
                + random.Uniform(-sqrt(sta.frontInfo.sigma2)[0] * 3.5, sqrt(sta.frontInfo.sigma2)[0] * 3.5);
            algo->vStsStripsB[h.b] =
                idet * (-sta.yInfo.cos_phi[0] * mcp.x + sta.xInfo.sin_phi[0] * mcp.y)
                + random.Uniform(-sqrt(sta.backInfo.sigma2)[0] * 3.5, sqrt(sta.backInfo.sigma2)[0] * 3.5);
#endif
    ...
    if (fVerbose > 1)
        cout << "L1 Track finder..." << endl;
    algo->CATrackFinder();
    //  IdealTrackFinder();
    if (fVerbose > 1)
        cout << "L1 Track finder ok" << endl;
...
void CbmL1::WriteSTAPAlgoData()   // must be called after ReadEvent
{
    ...
    if (1) {
        ...
        if (fVerbose >= 4)
            cout << "vStsStrips[" << n << "]" << " have been written." << endl;
        // write vStsStripsB
        ...
        if (fVerbose >= 4)
            cout << "vStsStripsB[" << n << "]" << " have been written." << endl;
        // write vStsZPos
        ...
        if (fVerbose >= 4)
            cout << "vStsZPos[" << n << "]" << " have been written." << endl;
        // write vSFlag
        n = algo->vSFlag.size();
        fadata << n << endl;
        ...
        if (fVerbose >= 4)
            cout << "vSFlag[" << n << "]" << " have been written." << endl;
        // write vSFlagB
        n = algo->vSFlagB.size();
        fadata << n << endl;
        for (int i = 0; i < n; i++) {
        ...
        if (fVerbose >= 4)
            cout << "vSFlagB[" << n << "]" << " have been written." << endl;
        // write vStsHits
        ...
        if (fVerbose >= 4)
            cout << "vStsHits[" << n << "]" << " have been written." << endl;
        // write StsHitsStartIndex and StsHitsStopIndex
        n = 20;
        for (int i = 0; i < n; i++) {
            if (algo->MaxNStations + 1 > i)
...
void CbmL1::WriteSTAPPerfData()   // must be called after ReadEvent
{
    ...
    if (1) {
        ...
        if (fVerbose >= 4)
            cout << "vMCPoints[" << n << "]" << " have been written." << endl;
        ...
        if (fVerbose >= 4)
            cout << "vMCTracks[" << n << "]" << " have been written." << endl;
        ...
        if (fVerbose >= 4)
            cout << "vHitMCRef[" << n << "]" << " have been written." << endl;
        ...
        if (fVerbose >= 4)
            cout << "vHitStore[" << n << "]" << " have been written." << endl;
    ...
    cout << "-I- CbmL1: Data for performance of event number " << vNEvent << " have been written in file "
         << fpdata_name << endl;
    vNEvent++;
...
void CbmL1::ReadSTAPAlgoData()
{
    ...
    if (1) {
        ...
        fadata >> nEv;
        if (nEv != nEvent)
            cout << "-E- CbmL1: Can't read event number " << nEvent << " from file " << fadata_name << endl;
        ...
        if (fVerbose >= 4)
            cout << "vStsStrips[" << n << "]" << " have been read." << endl;
        ...
        if (fVerbose >= 4)
            cout << "vStsStripsB[" << n << "]" << " have been read." << endl;
        // read algo->vStsZPos
        fadata >> n;
        for (int i = 0; i < n; i++) {
            fscal element;
        ...
        if (fVerbose >= 4)
            cout << "vStsZPos[" << n << "]" << " have been read." << endl;
        // read algo->vSFlag
        fadata >> n;
        for (int i = 0; i < n; i++) {
            int element;
        ...
        if (fVerbose >= 4)
            cout << "vSFlag[" << n << "]" << " have been read." << endl;
        // read algo->vSFlagB
        fadata >> n;
        for (int i = 0; i < n; i++) {
            int element;
        ...
        if (fVerbose >= 4)
            cout << "vSFlagB[" << n << "]" << " have been read." << endl;
        // read algo->vStsHits
        fadata >> n;
        ...
        if (fVerbose >= 4)
            cout << "vStsHits[" << n << "]" << " have been read." << endl;
        ...
        for (int i = 0; i < n; i++) {
            ...
            fadata >> tmp;
            if (algo->MaxNStations + 1 > i)
                algo->StsHitsStartIndex[i] = tmp;
        };
        for (int i = 0; i < n; i++) {
            int tmp;
            fadata >> tmp;
            if (algo->MaxNStations + 1 > i)
                algo->StsHitsStopIndex[i] = tmp;
        };
...
void CbmL1::ReadSTAPPerfData()
{
    ...
    if (1) {
        ...
        if (nEv != nEvent)
            cout << "-E- CbmL1: Performance: can't read event number " << nEvent << " from file " << "data_perfo.txt"
                 << endl;
        // vMCPoints
        int n;   // number of elements
        fpdata >> n;
        ...
        if (fVerbose >= 4)
            cout << "vMCPoints[" << n << "]" << " have been read." << endl;
        ...
        if (fVerbose >= 4)
            cout << "vMCTracks[" << n << "]" << " have been read." << endl;
        ...
        if (fVerbose >= 4)
            cout << "vHitMCRef[" << n << "]" << " have been read." << endl;
        ...
        if (fVerbose >= 4)
            cout << "vHitStore[" << n << "]" << " have been read." << endl;
...
void CbmL1::WriteSIMDKFData()
{
    ...
    for (vector<CbmL1Track>::iterator RecTrack = vRTracks.begin(); RecTrack != vRTracks.end(); ++RecTrack) {
        ...
        for (int i = 0; i < NHits; i++)
            Tracks << "     " << st[i] << " " << x[i] << " " << y[i] << " " << z[i] << " " << AngleXAxis << " "
                   << AngleYAxis << endl;
        Tracks << endl;
        ...
        for (int iPoint = 0; iPoint < NMCPoints; iPoint++) {
            ...
            McTracksOut << "     " << MCPoint.iStation << " " << MCPoint.xOut << " " << MCPoint.yOut << " "
                        << MCPoint.zOut << " " << MCPoint.pxOut << " " << MCPoint.pyOut << " " << MCPoint.pzOut << endl;
