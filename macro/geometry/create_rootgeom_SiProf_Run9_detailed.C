/*
 * Use this macro to create detailed ROOT geometry of Silicon Beam Profilometer
 * (SiProf) for the RUN-9 configuration (2 stations)
 *
 * Author: Baranov D.
 * Created: 25.04.2025
 *
...
const Double_t ZStationRotations[NStations][NMaxModules] = {
    {-0.111},   // st. 0 (-(90 - 89.889))
    {+0.080},   // st. 1 (-(90 - 90.080))
};
...
void create_rootgeom_SiProf_Run9_detailed()
{
...
TGeoVolume* CreateVacuumTube(TString vacTube_name)
{
    ...
    // shapes
    TGeoShape* vacTubeS =
        new TGeoTube(TString("vacTubeS") += TString("_") + vacTube_name, 19.6 * 0.5, 20.0 * 0.5, 20.0 * 0.5);
    TGeoShape* vacTubeHoleS =
        new TGeoTube(TString("vacTubeHoleS") += TString("_") + vacTube_name, 0.0, 15.0 * 0.5, 10. * 0.5);

    TGeoShape* sideTubeS =
        new TGeoTube(TString("sideTubeS") += TString("_") + vacTube_name, 14.6 * 0.5, 15.0 * 0.5, 20.8 * 0.5);
    TGeoShape* sideTubeCutS =
        new TGeoTube(TString("sideTubeCutS") += TString("_") + vacTube_name, 0.0, 19.6 * 0.5, 20.0 * 0.5);

    TGeoShape* vacTubeFlangeS =
        new TGeoTube(TString("vacTubeFlangeS") += TString("_") + vacTube_name, 20.0 * 0.5, 24.0 * 0.5, 1.2 * 0.5);
    TGeoShape* sideTubeFlangeS =
        new TGeoTube(TString("sideTubeFlangeS") += TString("_") + vacTube_name, 15.0 * 0.5, 18.0 * 0.5, 1.2 * 0.5);
    ...
    vacTubeWithHoleS->SetName(TString("vacTubeWithHoleS") += TString("_") + vacTube_name);
    {
        ...
        expression += TString("_") + vacTube_name;
    ...
    TGeoVolume* vacTubeWithHoleV =
        new TGeoVolume(TString("vacTubeWithHoleV") += TString("_") + vacTube_name, vacTubeWithHoleS);
    TGeoVolume* sideTubeWithCutV =
        new TGeoVolume(TString("sideTubeWithCutV") += TString("_") + vacTube_name, sideTubeWithCutS);
    TGeoVolume* vacTubeFlangeV =
        new TGeoVolume(TString("vacTubeFlangeV") += TString("_") + vacTube_name, vacTubeFlangeS);
    TGeoVolume* sideTubeFlangeV =
        new TGeoVolume(TString("sideTubeFlangeV") += TString("_") + vacTube_name, sideTubeFlangeS);

    // volume medium
    TGeoMedium* vacTubeWithHoleV_medium = nullptr;
    if (vacTube_name.Contains("station0"))
        vacTubeWithHoleV_medium = pMedSteel;
    if (vacTube_name.Contains("station1"))
