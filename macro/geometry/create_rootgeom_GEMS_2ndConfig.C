...
const Double_t XDistanceSensModule = 5.0;
...
void create_rootgeom_GEMS_2ndConfig() {
    ...
    const TString geoDetectorVersion = "2ndConfig";
...
void CreateStation_Two66x41Plane_Intersec(TGeoVolume* mother_volume, TString station_name, Double_t x_position, Double_t y_position, Double_t z_position, Double_t IsInverted) {
    TGeoVolume *stationA = new TGeoVolumeAssembly(station_name);
    stationA->SetMedium(pMedAir);
...
*/
    ...
    moduleV->SetTransparency(20);
    composite_frameV->SetTransparency(20);
    ...
    //Station position
    Double_t station_shift = dZSensitiveVolume*0.5;
    TGeoCombiTrans *station_pos = new TGeoCombiTrans();
    if(IsInverted) {
        station_pos->RotateX(180.0);
        station_shift += dZSensitiveVolume;
    }
    station_pos->SetTranslation(x_position, y_position, z_position+station_shift);
    mother_volume->AddNode(stationA, 0, station_pos);
}

void CreateStation_Two66x41Plane_Distance(TGeoVolume* mother_volume, TString station_name, Double_t x_position, Double_t y_position, Double_t z_position) {
    TGeoVolume *stationA = new TGeoVolumeAssembly(station_name);
    stationA->SetMedium(pMedAir);
...
*/
    ...
    TGeoCombiTrans *module_left_position = new TGeoCombiTrans();
    //module_left_position->ReflectX(true);
    module_left_position->SetTranslation(XModuleSize_StationPrototype*0.5+XDistanceSensModule*0.5, 0.0, 0.0);

    TGeoCombiTrans *module_right_position = new TGeoCombiTrans();
    module_right_position->ReflectX(true);
    module_right_position->SetTranslation(-XModuleSize_StationPrototype*0.5-XDistanceSensModule*0.5, 0.0, 0.0);
