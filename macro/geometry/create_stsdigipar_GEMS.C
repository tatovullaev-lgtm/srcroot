...
// ======                         Main function                           =====
// ============================================================================
void create_stsdigipar_GEMS(const char* geoTag = gkGeoTag,
						   const char* digiTag = gkDigiTag)
{
  ...
  // ---> GEM STS
  if (!geoMan->cd("GEMS_0")) {
    cout << "-E- Mother volume GEMS not found " << endl;
    exit(0);
  }

  TGeoNode* sts = geoMan->GetCurrentNode();
  if ( ! TString(sts->GetName()).Contains("GEMS") ) {
    cout << "-E- Illegal GEMS node name " << sts->GetName() << endl;
    exit(0);
  }
  ...
  for (Int_t iCaveD = 0; iCaveD < sts->GetNdaughters(); iCaveD++) {
	...
	continue;
      ...
      //moduleNr++;
      //fprintf(parFile, "%4d %4d\n", moduleNr, GetNofDaughters(module, "Senso"));
	...
	if ( ! TString(sensor->GetName()).Contains("Senso") ) {
	  geoMan->CdUp();
	  continue;
	}
...
Int_t GetNofModules(TGeoNode* station) {
  ...
  Int_t nModules = station->GetNdaughters();
