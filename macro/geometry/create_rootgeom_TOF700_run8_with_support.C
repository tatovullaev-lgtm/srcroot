...
Double_t xoffs, xoffs2, yoffs, zoffs;
...
void create_rootgeom_TOF700_run8_with_support() {
    ...
    TOF2_Nmodules = readGeom((char *)"TOF700_geometry_run8.txt");
    // -------   Geometry file name (output)   ----------------------------------
    TString geoDetectorName = "tof700";
    TString geoDetectorVersion = "run8_with_support";
...
int readGeom(char *geomfile)
{
	...
	if (strstr(geomfile,"_run8")) is8 = 1;
	if (is8 == 0)
	{
	...
	{
	    printf("TOF700 geometry file %s - first line read error!\n", fname);
	    return 0;
	};
	if (sscanf(line1,"%lf %lf %lf %lf\n", &xoffs, &xoffs2, &yoffs, &zoffs) != 4)
	{
	    printf("Wrong first line (not 4 numbers) in TOF700 geometry file %s\n", fname);
	    return 0;
	}
	printf("Zoffs = %f\n", zoffs);
