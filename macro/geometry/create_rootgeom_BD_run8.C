...
void create_rootgeom_BD_run8() {
	...
	TGeoTube *tubepbContainerS = new TGeoTube("tubepbContainerS", 7.6/2, 8/2, 18.8/2);
	TGeoTubeSeg *Seg_tube = new TGeoTubeSeg("Seg_tube", Seg_tube_Min_dim/2, Seg_tube_Max_dim/2, Seg_tube_Thick/2, Seg_tube_Min_phi, Seg_tube_Max_phi);   
    ...
    tubepb_prot->SetLineColor(kRed);
	
	TGeoVolume *seg_tube_prot = new TGeoVolume("seg_tube_prot", Seg_tube);
    seg_tube_prot->SetMedium(pMedlead);
    seg_tube_prot->SetLineColor(kRed);
	...
	BD->AddNode(seg_tube_prot, 44, Seg_tube_trans);	
