...
Double_t xoffs, xoffs2, yoffs, zoffs, dxcorr, dycorr, dzcorr;
...
Double_t G10LayerThickness_TOF2 =
    ChamberThickness_TOF2 - AlLayerThickness_TOF2 - GasLayerThickness_TOF2 - 2. * GlassLayerThickness_TOF2;
...
void create_rootgeom_TOF701_run8()
{
    ...
    TOF2_Nmodules = readGeom((char*)"TOF701_geometry_run8_v2.txt");
    // -------   Geometry file name (output)   ----------------------------------
    TString geoDetectorName = "tof700";
    TString geoDetectorVersion = "run8";
    TString geoFileName = geoPath + "/geometry/" + geoDetectorName + "_" + geoDetectorVersion + "_v2.root";
    // --------------------------------------------------------------------------
    // Modules Y centers
    Double_t dy = 0.;
    ...
    for (int i = 0; i < TOF2_Nmodules; i++) {
        if (TOF2_X_center[i] < xmin)
            xmin = TOF2_X_center[i];
        if (TOF2_Y_center[i] < ymin)
            ymin = TOF2_Y_center[i];
        if (TOF2_Z_center[i] < zmin)
            zmin = TOF2_Z_center[i];
        if (TOF2_X_center[i] > xmax)
            xmax = TOF2_X_center[i];
        if (TOF2_Y_center[i] > ymax)
            ymax = TOF2_Y_center[i];
        if (TOF2_Z_center[i] > zmax)
            zmax = TOF2_Z_center[i];
    }
    ...
    Double_t TOF2_XWidth =
        TMath::Abs(TOF2_X_max) > TMath::Abs(TOF2_X_min) ? (2 * TMath::Abs(TOF2_X_max)) : (2 * TMath::Abs(TOF2_X_min));
    Double_t TOF2_YWidth =
        TMath::Abs(TOF2_Y_max) > TMath::Abs(TOF2_Y_min) ? (2 * TMath::Abs(TOF2_Y_max)) : (2 * TMath::Abs(TOF2_Y_min));
    Double_t TOF2_ZWidth =
        TMath::Abs(TOF2_Z_max) > TMath::Abs(TOF2_Z_min) ? (2 * TMath::Abs(TOF2_Z_max)) : (2 * TMath::Abs(TOF2_Z_min));
    ...
    Double_t GasLayerPos =
        AlLayerThickness_TOF2 + GlassLayerThickness_TOF2 + GasLayerThickness_TOF2 / 2. - ZWidthOfModule_TOF2 / 2.;
    sprintf(name, "GasLayerPosTOF2_trans");
    TGeoTranslation* GasLayerPosTOF2_trans = new TGeoTranslation(name, 0., 0., GasLayerPos);
    Double_t GlassLayer2Pos = AlLayerThickness_TOF2 + GlassLayerThickness_TOF2 + GasLayerThickness_TOF2
                              + GlassLayerThickness_TOF2 / 2. - ZWidthOfModule_TOF2 / 2.;
    ...
    Double_t G10LayerPos = AlLayerThickness_TOF2 + GlassLayerThickness_TOF2 + GasLayerThickness_TOF2
                           + GlassLayerThickness_TOF2 + G10LayerThickness_TOF2 / 2. - ZWidthOfModule_TOF2 / 2.;
    ...
    int nstr[2] = {32, 16};
    for (int i = 0; i < 2; i++) {
        TOF2ModuleS[i] = new TGeoBBox(nTOF2ModuleS[i], XWidthOfModule_TOF2[i] / 2 + 0.001,
                                      YWidthOfModule_TOF2[i] / 2 + 0.001, ZWidthOfModule_TOF2 / 2 + 0.001);
        TOF2GlassVolumeS[i] = new TGeoBBox(nTOF2GlassVolumeS[i], XActWidthOfModule_TOF2[i] / 2,
                                           YActWidthOfModule_TOF2[i] / 2, GlassLayerThickness_TOF2 / 2);
        TOF2ActiveGasVolumeS[i] = new TGeoBBox(nTOF2ActiveGasVolumeS[i], XActWidthOfModule_TOF2[i] / 2,
                                               YActWidthOfModule_TOF2[i] / 2, GasLayerThickness_TOF2 / 2);
        TOF2G10VolumeS[i] = new TGeoBBox(nTOF2G10VolumeS[i], XActWidthOfModule_TOF2[i] / 2,
                                         YActWidthOfModule_TOF2[i] / 2, G10LayerThickness_TOF2 / 2);
...
int readGeom(char* geomfile)
{
    ...
    if (strlen(geomfile) == 0) {
        printf("TOF701 geometry file name not defined!\n");
        return 0;
    }
    if (strstr(geomfile, "_run8"))
        is8 = 1;
    if (is8 == 0) {
        printf("Geometry file for early period (%s), but script for period 8 !\n", geomfile);
        return 0;
    };
    ...
    fg = fopen(fname, "r");
    if (fg == NULL) {
        printf("TOF701 geometry file %s open error!\n", fname);
        return 0;
    }
    if (!fgets(line1, sizeof(line1), fg)) {
        printf("TOF701 geometry file %s - first line read error!\n", fname);
        return 0;
    };
    if (sscanf(line1, "%lf %lf %lf %lf\n", &xoffs, &xoffs2, &yoffs, &zoffs) != 4) {
        printf("Wrong first line (not 4 numbers) in TOF701 geometry file %s\n", fname);
        return 0;
    }
    printf("Zoffs = %f\n", zoffs);
    for (int i = 0; i < TOF2_MAX_CHAMBERS; i++)
        nstrips[i] = 0;
    int c = 0;
    while (fscanf(fg, "%s %d %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", &ic[0], &n, &step, &sy, &sx, &x, &y, &z, &dxcorr,
                  &dycorr, &dzcorr)
           == 11)
    {
        ...
        for (int ns = n - 1; ns >= 0; ns--) {
            if (z < 100.)
                xcens[c][ns] = -x / 10. + xoffs + dxcorr / 10.;
            else
                xcens[c][ns] = -x / 10. + xoffs2 + dxcorr / 10.;
            ycens[c][ns] = y / 10. + yoffs - (n - ns - 1) * step / 10. + dycorr / 10.;
