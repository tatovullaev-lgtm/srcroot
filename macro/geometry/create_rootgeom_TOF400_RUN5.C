...
void MakeBMNROOT_TOFv3()
{
    ...
    // first box
    topTof->AddNode(vDetector, 1, new TGeoTranslation("t1", +tofX, 0 + 2 * DetYsize - 2 * tofYoverlap, tofZ_1box + tofZshift)); // right side, x > 0
    topTof->AddNode(vDetector, 2, new TGeoTranslation("t1", +tofX, 0 + DetYsize - tofYoverlap, tofZ_1box));
    topTof->AddNode(vDetector, 3, new TGeoTranslation("t1", +tofX, 0, tofZ_1box + tofZshift));
    topTof->AddNode(vDetector, 4, new TGeoTranslation("t1", +tofX, 0 - DetYsize + tofYoverlap, tofZ_1box));
    topTof->AddNode(vDetector, 5, new TGeoTranslation("t1", +tofX, 0 - 2 * DetYsize + 2 * tofYoverlap, tofZ_1box + tofZshift));
