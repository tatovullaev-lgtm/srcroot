...
Double_t Zdc_X_size = 77.5; //155./2.
Double_t Zdc_Y_size = 52.5; //105./2.
Double_t Zdc_Z_size = 62.4; //10 sect

Double_t Zdc_X_size_NICA = 37.5; //75./2.
Double_t Zdc_Y_size_NICA = 52.5; //105./2.

//Modules NICA
Double_t Zdc_NColumns_NICA = 5; 
Double_t Zdc_NRows_NICA = 7; 
...
//Modules
Double_t Zdc_Y_UP = 2.5;//cm
Double_t Zdc_NColumns = 2; 
Double_t Zdc_NRows = 5; 
Double_t Zdc_module_X_size = 10.; //20./2.
Double_t Zdc_module_Y_size = 10.; //20./2.
Double_t Zdc_module_Z_size = 62.4; //10 sect

//Fe slices
Double_t Zdc_Fe_X_size = 9.9; //(20.-2.*0.1)/2.
Double_t Zdc_Fe_Y_size = 9.85; //(20.-2.*0.15)/2.
Double_t Zdc_Fe_Z_size = 1.; //2./2.

//Pb slices
Double_t Zdc_NPb_in_module = 59;
Double_t Zdc_Pb_X_size = 9.9; //(20.-2.*0.1)/2.
Double_t Zdc_Pb_Y_size = 9.85; //(20.-2.*0.15)/2.
Double_t Zdc_Pb_Z_size = 0.8; //1.6/2.

//Scin. slices
//Double_t Zdc_NSc_in_module = 1;//put into Tyvec
Double_t Zdc_Sc_X_size = 9.88; //(20.-2.*0.1-2.*0.02)/2.
Double_t Zdc_Sc_Y_size = 9.83; //(20.-2.*0.15-2.*0.02)/2.
Double_t Zdc_Sc_Z_size = 0.2; //0.4/2.

//Tyvec slices
Double_t Zdc_NTyvec_in_module = 60;
Double_t Zdc_Tyvec_X_size = 9.9; //(20.-2.*0.1)/2.
Double_t Zdc_Tyvec_Y_size = 9.85; //(20.-2.*0.15)/2.
...
void FillModule(TGeoVolume* zdcModuleV);

void create_rootgeom_FHCal_oldnames_CBM_20mods_NICA_34mods_54mods_hole_v1(Double_t ZDC_Xpos=65.3,Double_t ZDC_Ypos=-0.8,Double_t ZDC_Zpos=977.8,Double_t rotY=1.6) {
  ...
  const TString geoDetectorName = "FHCal_CBM_20mods_NICA_34mods_54mods_hole";
  ...
  sXshift_1 += Xshift_1;
  Int_t Xshift_2 = Int_t(100*(ZDC_Xpos-Int_t(ZDC_Xpos)))+1; //cm
  sXshift_2 += Xshift_2;
  ...
  Int_t Xshift_1 = Int_t(ZDC_Xpos); //cm
  sXshift_1 += Xshift_1;
  //Int_t Xshift_2 = ZDC_Xpos-ZDC_Xpos; //to make "0" //cm
  Int_t Xshift_2 = Int_t(100*(ZDC_Xpos-Int_t(ZDC_Xpos))); //cm
  //Int_t Xshift_2 = Int_t(100*(ZDC_Xpos-Int_t(ZDC_Xpos)))+1; //cm
  //Int_t Xshift_3 = Int_t(0.5+100*(ZDC_Xpos-Int_t(ZDC_Xpos))); //cm
  sXshift_2 += Xshift_2;
  //sXshift_2 += Xshift_3;
  */
  ...
  Int_t Yshift_2 = TMath::Abs(Int_t(10*(ZDC_Ypos))); //cm
...
// -------------- Create geometry and global top volume ---------------------
  ...
  TGeoVolume* top = new TGeoVolumeAssembly(BmnFHCalGeoPar::fTopVolumeName);
  top->SetMedium(pMedAir);
  gGeoManager->SetTopVolume(top);
  //gGeoMan->SetTopVisible(1);

// Define ZDC Geometry
    TGeoVolume *FHCAL_common = new TGeoVolumeAssembly(BmnFHCalGeoPar::fCommonVolumeName);
    FHCAL_common->SetMedium(pMedAir);
    //ZDC_common->SetMedium(pMedVacuum);

    TGeoBBox *ZDCS = new TGeoBBox(BmnFHCalGeoPar::fDetectorVolumeName, Zdc_X_size, Zdc_Y_size, Zdc_Z_size);
    TGeoVolume *ZDCV = new TGeoVolume(BmnFHCalGeoPar::fDetectorVolumeName, ZDCS);
    ZDCV->SetMedium(pMedAir);

    TGeoBBox *zdcModuleS = new TGeoBBox(BmnFHCalGeoPar::fhcalMod_name_large, Zdc_module_X_size, Zdc_module_Y_size, Zdc_module_Z_size);
    TGeoVolume *zdcModuleV = new TGeoVolume(BmnFHCalGeoPar::fhcalMod_name_large, zdcModuleS);
    zdcModuleV->SetMedium(pMedIron);

    TGeoBBox *zdcModuleS_NICA = new TGeoBBox(BmnFHCalGeoPar::fhcalMod_name_small, Zdc_module_X_size_NICA, Zdc_module_Y_size_NICA, Zdc_module_Z_size_NICA);
    TGeoVolume *zdcModuleV_NICA = new TGeoVolume(BmnFHCalGeoPar::fhcalMod_name_small, zdcModuleS_NICA);
    zdcModuleV_NICA->SetMedium(pMedIron);

//Fill ZDC with NICA modules
    Float_t xCur = Zdc_X_size_NICA + Zdc_module_X_size_NICA;//37.5+7.5=45. cm
    Float_t yCur = Zdc_Y_size_NICA + Zdc_module_Y_size_NICA;//52.5+7.5=59. cm

    Int_t iMod=-1, modNb=-1;
    for(Int_t iy=0; iy<Zdc_NRows_NICA; iy++) {//7
      yCur=yCur - 2.*Zdc_module_Y_size_NICA;
      for(Int_t ix=0; ix<Zdc_NColumns_NICA; ix++) {//5
	xCur=xCur - 2.*Zdc_module_X_size_NICA;
	
	TGeoTranslation *zdcModuleV_position_NICA = new TGeoTranslation(xCur,yCur,-(Zdc_module_Z_size-Zdc_module_Z_size_NICA));
	iMod=iy*Zdc_NColumns_NICA + ix;
	
	if(iMod<17) modNb=iMod+1;  //mods 1-17
	if(iMod==17) continue;
	//if(iMod==17) modNb=55;
	if(iMod>17) modNb=iMod;  //mods 18-54
...
//Fill ZDC with modules
    Float_t xCur_right=0;
    xCur = Zdc_X_size + Zdc_module_X_size;//77.5+10=87.5 cm
    yCur = Zdc_Y_size + Zdc_module_X_size_NICA;//52.5-7.5=60. cm
    iMod=-1; modNb=-1; 

    TGeoTranslation *zdcModuleV_position = new TGeoTranslation(xCur,yCur,0.);

    for(Int_t iy=0; iy<Zdc_NRows; iy++) {//5
      yCur=yCur - 2.*Zdc_module_Y_size;
      for(Int_t ix=0; ix<Zdc_NColumns; ix++) {//2
	xCur=xCur - 2.*Zdc_module_X_size;
	
	iMod=(iy*Zdc_NColumns + ix) + 34;
	modNb=iMod+1; 

	zdcModuleV_position = new TGeoTranslation(xCur,yCur,0.);
	ZDCV->AddNode(zdcModuleV,modNb,zdcModuleV_position);
	cout <<"CBM mods Left iMod,modNb,xxxx,yyyy " <<iMod+1 <<" " <<modNb <<" " <<xCur <<" " <<yCur <<"======= " <<endl;	  	  
	if(ix==0) modNb=iMod+1+11;
	if(ix==1) modNb=iMod+1+9;
	xCur_right = -xCur;
	zdcModuleV_position = new TGeoTranslation(xCur_right,yCur,0.);
	ZDCV->AddNode(zdcModuleV,modNb,zdcModuleV_position);
	cout <<"CBM mods right iMod,modNb,xxxx,yyyy " <<iMod+1 <<" " <<modNb <<" " <<xCur_right <<" " <<yCur <<endl;	  
	
	if(ix==Zdc_NColumns-1) xCur=Zdc_X_size + Zdc_module_X_size;;	
      }//for(Int_t ix==0; ix<7; ix++)
    }//for(Int_t iy==0; iy<7; iy++)
...
//Adding the zdc mother volume to the global TOP Volume
    ...
    FHCAL_common->AddNode(ZDCV, 1, zdcZDC);//ZDC right

    top->AddNode(FHCAL_common, 0);
...
Double_t Zdc_VRFM_Y_size = 8.6;
Double_t Zdc_VRFM_Z_size = 1.;

Double_t Zdc_VRPM_X_size = 0.1; //Pb
Double_t Zdc_VRPM_Y_size = 8.6;
Double_t Zdc_VRPM_Z_size = 0.8;

Double_t Zdc_VRYM_X_size = 0.01; //Tyvec_Y
Double_t Zdc_VRYM_Y_size = 8.6;
Double_t Zdc_VRYM_Z_size = 0.22;

Double_t Zdc_VRAM_X_size = 0.09; //Tyvec_A
Double_t Zdc_VRAM_Y_size = 8.6;
Double_t Zdc_VRAM_Z_size = 0.01;

Double_t Zdc_VRSM_X_size = 0.09; //Scint
Double_t Zdc_VRSM_Y_size = 8.6;
Double_t Zdc_VRSM_Z_size = 0.2;

//Shapes
 TGeoShape *FeS = new TGeoBBox(BmnFHCalGeoPar::Fe_name, Zdc_Fe_X_size, Zdc_Fe_Y_size, Zdc_Fe_Z_size);
 TGeoVolume *FeV = new TGeoVolume(BmnFHCalGeoPar::Fe_name, FeS);
 TGeoMedium *Fe_medium = pMedIron; //set medium
 if(Fe_medium) FeV->SetMedium(Fe_medium);
 else Fatal("Main", "Invalid medium for Fe!");
 
 TGeoShape *PbS = new TGeoBBox(BmnFHCalGeoPar::Pb_name, Zdc_Pb_X_size, Zdc_Pb_Y_size, Zdc_Pb_Z_size);
 TGeoVolume *PbV = new TGeoVolume(BmnFHCalGeoPar::Pb_name, PbS);
 TGeoMedium *Pb_medium = pMedLead; //set medium
 if(Pb_medium) PbV->SetMedium(Pb_medium);
 else Fatal("Main", "Invalid medium for Pb!");

 TGeoShape *zdcSensitiveVolumeS = new TGeoBBox(BmnFHCalGeoPar::SensitiveVolume_name, Zdc_Sc_X_size, Zdc_Sc_Y_size, Zdc_Sc_Z_size);
 TGeoVolume *zdcSensitiveVolumeV = new TGeoVolume(BmnFHCalGeoPar::SensitiveVolume_name, zdcSensitiveVolumeS);
 ...
 TGeoShape *TyvecS = new TGeoBBox(BmnFHCalGeoPar::Tyvec_name, Zdc_Tyvec_X_size, Zdc_Tyvec_Y_size, Zdc_Tyvec_Z_size);
 TGeoVolume *TyvecV = new TGeoVolume(BmnFHCalGeoPar::Tyvec_name, TyvecS);
...
//Slot's shapes
 TGeoShape *slot_FeS = new TGeoBBox(BmnFHCalGeoPar::slot_Fe_name, Zdc_VRFM_X_size, Zdc_VRFM_Y_size, Zdc_VRFM_Z_size);
 TGeoVolume *slot_FeV = new TGeoVolume(BmnFHCalGeoPar::slot_Fe_name, slot_FeS);
 ...
 TGeoShape *slot_PbS = new TGeoBBox(BmnFHCalGeoPar::slot_Pb_name, Zdc_VRPM_X_size, Zdc_VRPM_Y_size, Zdc_VRPM_Z_size);
 TGeoVolume *slot_PbV = new TGeoVolume(BmnFHCalGeoPar::slot_Pb_name, slot_PbS);
 ...
 TGeoShape *slot_ScS = new TGeoBBox(BmnFHCalGeoPar::slot_Sc_name, Zdc_VRSM_X_size, Zdc_VRSM_Y_size, Zdc_VRSM_Z_size);
 TGeoVolume *slot_ScV = new TGeoVolume(BmnFHCalGeoPar::slot_Sc_name, slot_ScS);
 ...
 TGeoShape *slot_Tyvec_YS = new TGeoBBox(BmnFHCalGeoPar::slot_Tyvec_Y_name, Zdc_VRYM_X_size, Zdc_VRYM_Y_size, Zdc_VRYM_Z_size);
 TGeoVolume *slot_Tyvec_YV = new TGeoVolume(BmnFHCalGeoPar::slot_Tyvec_Y_name, slot_Tyvec_YS);
 ...
 TGeoShape *slot_Tyvec_AS = new TGeoBBox(BmnFHCalGeoPar::slot_Tyvec_A_name, Zdc_VRAM_X_size, Zdc_VRAM_Y_size, Zdc_VRAM_Z_size);
 TGeoVolume *slot_Tyvec_AV = new TGeoVolume(BmnFHCalGeoPar::slot_Tyvec_A_name, slot_Tyvec_AS);
...
//Shapes
 TGeoShape *FeS_NICA = new TGeoBBox(BmnFHCalGeoPar::Fe_name_NICA, Zdc_Fe_X_size, Zdc_Fe_Y_size, Zdc_Fe_Z_size);
 TGeoVolume *FeV_NICA = new TGeoVolume(BmnFHCalGeoPar::Fe_name_NICA, FeS_NICA);
 ...
 TGeoShape *PbS_NICA = new TGeoBBox(BmnFHCalGeoPar::Pb_name_NICA, Zdc_Pb_X_size, Zdc_Pb_Y_size, Zdc_Pb_Z_size);
 TGeoVolume *PbV_NICA = new TGeoVolume(BmnFHCalGeoPar::Pb_name_NICA, PbS_NICA);
 ...
 TGeoShape *zdcSensitiveVolumeS_NICA = new TGeoBBox(BmnFHCalGeoPar::SensitiveVolume_name_NICA, Zdc_Sc_X_size, Zdc_Sc_Y_size, Zdc_Sc_Z_size);
 TGeoVolume *zdcSensitiveVolumeV_NICA = new TGeoVolume(BmnFHCalGeoPar::SensitiveVolume_name_NICA, zdcSensitiveVolumeS_NICA);
 ...
 TGeoShape *TyvecS_NICA = new TGeoBBox(BmnFHCalGeoPar::Tyvec_name_NICA, Zdc_Tyvec_X_size, Zdc_Tyvec_Y_size, Zdc_Tyvec_Z_size);
 TGeoVolume *TyvecV_NICA = new TGeoVolume(BmnFHCalGeoPar::Tyvec_name_NICA, TyvecS_NICA);
