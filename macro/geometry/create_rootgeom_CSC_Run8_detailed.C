...
const Double_t XStationPositions[NStations] = {110.0 /*0*/, -110.0 /*1*/, 130.0 /*2*/,
                                               -130.0 /*3*/};                                 // geometry center
const Double_t YStationPositions[NStations] = {0.0 /*0*/, 0.0 /*1*/, 0.0 /*2*/, 0.0 /*3*/};   // geometry center
const Double_t ZStationPositions[NStations] = {410.0 /*0*/, 410.0 /*1*/, 470.0 /*2*/,
                                               470.0 /*3*/};   // outer side nearest to the target
...
void create_rootgeom_CSC_Run8_detailed()
{
    ...
    for (Int_t istation = 0; istation < NStations; ++istation) {
        ...
        module0_transform->SetTranslation(XModuleShifts[stationNum][0], YModuleShifts[stationNum][0],
                                          ZModuleShifts[stationNum][0] + 0.5 * ZGasSize);
        ...
        module1_transform->SetTranslation(XModuleShifts[stationNum][1], YModuleShifts[stationNum][1],
                                          ZModuleShifts[stationNum][1] + 0.5 * ZGasSize);
...
TGeoVolume* CreateModule(TString module_name)
{
    ...
    // shapes
    TGeoShape* moduleS =
        new TGeoBBox(TString("moduleS") += TString("_") + module_name, XGasSize * 0.5, YGasSize * 0.5, ZGasSize * 0.5);
    TGeoShape* sensS =
        new TGeoBBox(TString("sensS") += TString("_") + module_name, XSensSize * 0.5, YSensSize * 0.5, ZSensSize * 0.5);
...
TGeoVolume* CreateFrameForModule(TString frame_name)
{
    ...
    fiberGlassPanelV_transf[2]->SetDz(
        -(ZGasSize * 0.5 + fiberGlassPanel_ZSize + honeyCombPanel_ZSize + fiberGlassPanel_ZSize * 0.5));
    ...
    fixingPlateV_transf[0]->SetDz(
        -(ZGasSize * 0.5 + fixingPlate_ZSize * 0.5 + spacerBedInnerHole_ZSize + 0.005 /*shift*/));
