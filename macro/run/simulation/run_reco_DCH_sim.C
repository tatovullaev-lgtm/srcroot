...
void run_reco_DCH_sim(TString inputFileName ="$VMCWORKDIR/macro/run/Sim_For_Decay_FullGeom_WO_Target_Field_Scaled_Real_Beam_wo_Tof700.root",
                  TString srcdstFileName = "$VMCWORKDIR/macro/run/Reco_DCH_Sim_For_Decay_FullGeom_WO_Target_Field_Scaled_Real_Beam_wo_Tof700.root",
                  Int_t nStartEvent = 0, Int_t nEvents = 0)
{
    ...
    fRunAna->SetEventHeader(new DstEventHeader());
     cout << "Macro started successfully." << endl;
    ...
    Int_t run_period = 8, run_number = 1080;
...
//        gRandom->SetSeed(0);//from here
	//   TString geoFileName = "full_geometry_johansson.root";// Form("current_geo_file_%d.root", UInt_t(gRandom->Integer(UINT32_MAX)));
...
//    fRunAna->AddTask(innerTF);
    ...
    if(!isExp) {
     ...
     // dchTF->SetTransferFunction("pol_coord_Run9_SRC_LH.txt");
      dchTF->SetTransferFunction("pol_coord_Run9_SRC_1800A.txt");
    ...
    //BmnGlobalTracking* glTF = new BmnGlobalTracking(isField, 1, kFALSE);
