...
void run_sim_src_decay(TString inFile = "", TString outFile = "$VMCWORKDIR/macro/run/Sim_For_Decay_FullGeom_WO_Target_Field_Scaled_10Be_Beam.root", Int_t nStartEvent = 0, Int_t nEvents = 100000, Bool_t useRealEffects = kFALSE) {
    TStopwatch timer;
    timer.Start();
    gDebug = 0;
   ...
   // primGen->SetBeam(0.5, -4.6, 0.0, 0.0);
	...
	double A = 10.;
	double Z = 4.;
	double P = 3.721;//3.662;//without target 3.693; //before target  //3.75;//3.997433; //per nucleon after MWPC2
	...
	// 			P*A-27, P*A+21, // Min and Max Total momentum, momentum sigma in GeV/c, middle of LH2 
	// 			0.5, -4.6, -500, //X, Y, Z (in cm) of the vertex
	// 			3., 3., //  max vertex deviation in X and Y unifrom  (in cm)
	// 			0.0,  0.0, // X angle, Y angle [deg], 
	// 			1.15, 1.15); //Max XA deviation [deg], max YA deviation [deg] uniform
    MpdSRCIonGenerator *fSRCgen =
		new MpdSRCIonGenerator(Z, A, 1, //Z, A, muliplicity of an ion in one event
				P*A*0.995, P*A*1.005, // Min and Max Total momentum, momentum sigma in GeV/c, middle of LH2 
				0.5, -4.6, -500, //X, Y, Z (in cm) of the vertex
				0.65, 0.85, //  max vertex deviation in X and Y unifrom  (in cm)
				0.0,  0.0, // X angle, Y angle [deg], 
				0.0032, 0.0025); //Max XA deviation [deg], max YA deviation [deg] uniform
//  	MpdSRCIonGenerator *fSRCgen =
//		new MpdSRCIonGenerator(Z, A, 1, //Z, A, muliplicity of an ion in one event
...
//	
    ...
    Double_t fieldScale = 0.9/*0.85*/; // 1900A - 112V 1650A ~ 100.5V -> fieldScale = 100.5V / 112V ~ 0.897
    BmnFieldMap* magField = new BmnNewFieldMap("FieldMap_1900.root");
    magField->SetScale(fieldScale);
    fRun->SetField(magField);

    // Double_t fieldScale = 1650. / 900./1.0612;
