...
#include "macro/run/geometry_run/geometry_src_run8_CSC_protons.C"

#define BOX     // Choose generator: SRC URQMD QGSM HSD BOX PART ION
...
void run_sim_src1(TString inFile = "", TString outFile = "$VMCWORKDIR/macro/run/SimFile.root", Int_t nStartEvent = 0, Int_t nEvents = 1, Bool_t useRealEffects = kFALSE) {
    TStopwatch timer;
    timer.Start();
    gDebug = 0;
    ...
    primGen->SetBeam(0.5, -4.7, 0.0, 0.0); //0.5, -4.7 vertex XY
    primGen->SetTarget(-576.2, 0.0); // -576.2=Vertex Z
