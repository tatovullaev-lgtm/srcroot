...
void run_sim_src_Si(TString inFile = "", TString outFile = "$VMCWORKDIR/macro/run/Sim_Si.root", Int_t nStartEvent = 0, Int_t nEvents = 1000000, Bool_t useRealEffects = kFALSE) {
    TStopwatch timer;
    timer.Start();
    gDebug = 0;
   ...
   // primGen->SetBeam(0.5, -4.6, 0.0, 0.0);
	...
	double A = 12.;
	double Z = 6.;
	double P = 3.662;//3.662;//without target 3.693; //before target  //3.75;//3.997433; //per nucleon after MWPC2
	...
	MpdSRCIonGenerator *fSRCgen =
		new MpdSRCIonGenerator(Z, A, 1, //Z, A, muliplicity of an ion in one event
				P*A, P*A, // Min and Max Total momentum, momentum sigma in GeV/c, middle of LH2 
				0.5, -4.6, -1000, //X, Y, Z (in cm) of the vertex
				1., 1., //  max vertex deviation in X and Y unifrom  (in cm)
				0.0,  0.0, // X angle, Y angle [deg], 
				0.15, 0.1); //Max XA deviation [deg], max YA deviation [deg] uniform
