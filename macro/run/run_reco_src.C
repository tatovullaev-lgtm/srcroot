...
void run_reco_src(TString inputFileName ="/zfs/scratch/lhep-src/srcroot_1/4032_digi_test.root",
                  TString srcdstFileName = "/zfs/scratch/lhep-src/srcroot_1/4032_reco_test.root",
                  Int_t nStartEvent = 0, Int_t nEvents = 0)
{
    ...
    Int_t run_period = 8, run_number = 5162;
...
//        gRandom->SetSeed(0);//from here
	  TString geoFileName = "Geometry_correctTOF400_12_2023.root";// Form("current_geo_file_%d.root", UInt_t(gRandom->Integer(UINT32_MAX)));
...
//    fRunAna->AddTask(VF);
    ...
    BmnScWallAnalyzer* scwall = new BmnScWallAnalyzer(); 
    fRunAna -> AddTask(scwall); 
