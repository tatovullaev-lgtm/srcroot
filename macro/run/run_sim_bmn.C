                 ...
                 Bool_t saveGeoInfo = kFALSE)
{
    ...
    // Set the BM@N geometry
#ifdef USE_GEODB
    GeoSetup gSetup;
    int errStatus = gSetup.LoadSetupToFairRunSim("run8_dev_25.11.01");   // run8_25.09.0, run9_dev_25.12.10
    if (errStatus != 0) {
        cout << "ERROR: Geometry not loaded. Error status " << errStatus << endl;
        exit(-1);
    }
#else
    geometry(fRun);   // load BM@N geometry from macro file
#endif
    ...
    primGen->SetBeam(0.4376, 0.1239 - 0.3, 0.4537, 0.6925);   // (beamX0, beamY0, beamSigmaX, beamSigmaY)
...
#ifdef USE_GEODB
    // if GetMagneticField(0) -> get default value from the Geometry Database
    ...
    fHodoDigit->SetConfig("HODO_Q_calibration_period8.json");
    fHodoDigit->SetScale(1337.8);   // [Z^2/GeV] set Xe peak to 54*54
    fHodoDigit->SetThreshold(0.);   // [Z^2] we see noise below this in data
