...
//
void drawMultCorrBMN(TString fileName = "../run/bmndst_4699.root", Int_t nEvents = 10000) {
    ...
    TClonesArray* fdHits = nullptr;
    out->SetBranchAddress("BmnFdHit", &fdHits);
    ...
    TString names[nDet] = {"FD", "SIL", "GEM", "CSC", "TOF400", "TOF700", "DCH1", "DCH2"};
    ...
    for (Int_t iDet = 0; iDet < nDet; iDet++)
        for (Int_t jDet = 0; jDet < nDet; jDet++) {
            ...
            hist[iDet][jDet] = new TH2F(histName, "", 40, 0, 40, 40, 0, 40);
        }
    ...
    for (Int_t iEv = 0; iEv < nEvents; iEv++) {
        ...
        Int_t nFd = (fdHits) ? fdHits->GetEntriesFast() : 0;
        ...
        Int_t nTof400 = (tof400Hits) ? tof400Hits->GetEntriesFast() : 0;
        ...
        Int_t nItem[nDet] = {nFd, nSil, nGem, nCsc, nTof400, nTof700, nDch1, nDch2};
