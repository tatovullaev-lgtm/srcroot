...
bool usePreviousIfMissing = true;
enum enumTracking
{
    VectorFinder,
    L1Tracking,
...
void run_reco_bmn(TString inFileName = "tcp://localhost:5555",
                  ...
                  enumTracking tracking = VectorFinder)
{
    ...
    if (!BmnFunctionSet::CheckFileExist(inputFileName, 1))
        exit(-1);
    isExp = (BmnFunctionSet::isSimulationFile(inputFileName)
             == 0);   // flag for hit finder (to create digits or take them from data-file)
#endif
    ...
    Int_t run_period = 9, run_number = 9865;
    Double_t fieldScale = 0.;

#ifdef OnlineMode
    //    fFileSource = new BmnMQSource("tcp://bmn-mon-1.he.jinr.ru:5555", kTRUE);
    fFileSource = new BmnDecoSource(inFileName);
    bool srcInitResult = fFileSource->Init();
    if (!srcInitResult) {
        cout << "ERROR: Online digi source init failed!" << endl;
        exit(-8);
    }
    //    fFileSource = new BmnDecoSource("tcp://bmn-mon-1.he.jinr.ru:5555");
    //            fFileSource = new BmnDecoSource("tcp://localhost:5555");
    {
#else
...
#ifdef OnlineMode
    fRunAna->SetSink(new BmnMQSink(6666));
#else
    ...
    //    if (!isExp)
    cscHM->SetCurrentConfig(BmnCSCConfiguration::Run9);   // set explicitly
    if (isExp)
