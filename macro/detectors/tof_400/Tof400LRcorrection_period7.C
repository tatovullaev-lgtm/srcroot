...
/* 
 * File:   Tof400LRCorrection_period7.C
 * Author: mikhailr
 *
 * Created on March 10, 2018, 9:55 AM
 */
const Int_t nDet = 20;

/*const Double_t LR[nDet] = {0.07, -0.01, 2.18, 6.64, 6.57, -0.05, 0.03, 2.12, 6.49, 6.49, 
                        0.093, 0.055, 2.21, 6.59, 6.51, 0.00, -0.03, 2.07, 6.47, 6.52};*/

const Int_t minimum[nDet] = {40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40};

void Tof400LRcorrection_period7(TString file = "") {

  /*  ofstream f_call;
    TString NameCallFile = "Tof400LRcorr_RUN7.dat";
    ...
    for (Int_t Plane = 0; Plane < 20; Plane++) {
        for (Int_t Str = 0; Str < 48; Str++)
            f_call << Plane << "\t" << Str << "\t" << LR[Plane] << "\t" << LR[Plane] << endl;
    }//*/
       ...
       TString Name1, Name2;
       TH1I ***hDtLR = new TH1I**[nDet];
       TH1I ***hDifDtLR = new TH1I**[nDet];

       FileIn->cd("ToF");
       for (Int_t Plane = 0; Plane < nDet; Plane++)
       {
           ...
           FileIn->cd(Form("ToF/Plane_%d/Detector", Plane));
           gDirectory->pwd();
           //gDirectory->ls();
           for (Int_t Str = 0; Str < 48; Str++)
           {
       ...
       outName.Replace(Point, 15, "_LRcorr_RUN7.root");
