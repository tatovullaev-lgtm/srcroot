//Macro 06.12.2021
/*
 * Визуальная и параметрическая проверка SiBT-конфигураций, включающая:
 *  - проверка расположения точки на станции, модуле, слое
 ...
 * помощи SHIFT+Mouse_click автоматическим расчетом хитов-пересечений
 *
...
BmnSiBTStation *g_current_station = 0;
...
//Выводить информацию в поток о всех хитах на станции при добавлении новой точки
Bool_t g_ShowIntersectionPointInfo = true;

void DrawCreatedStationFront(BmnSiBTStation *station, vector<TLorentzVector> &points);
void DrawCreatedStationTop(BmnSiBTStation *station, vector<TLorentzVector> &points);

void TestSiBTConfig_Visual() {

    //Enable transparency OpenGL via gStyle
    gStyle->SetCanvasPreferGL(1);


    BmnSiBTStationSet *StationSet = new BmnSiBTStationSet("../../parameters/SiBT/XMLConfigs/SiBTRun8.xml");
    ...
    if(1) {
        ...
        if(station_num != -1) {
            BmnSiBTStation *station = StationSet->GetStation(station_num);
...
void DrawCreatedStationFront(BmnSiBTStation *station, vector<TLorentzVector> &points) {
    ...
    BmnSiBTModule **Modules = new BmnSiBTModule*[NModules];
    ...
    */
    ...
    for(Int_t im = 0; im < NModules; im++) {
        ...
        for(Int_t ilayer = 0; ilayer < NLayers; ++ilayer) {
            ...
            BmnSiBTLayer layer = Modules[im]->GetStripLayer(ilayer);
...
//void RecognizeModuleNumFromPointAndDrawIt() {
    ...
    if(event == kButton1Shift) { //mouse click with shift button
        ...
        if(module_num != -1) {
            ...
            BmnSiBTModule *module = g_current_station->GetModule(module_num);
            Int_t NLayers = module->GetNStripLayers();
        ...
        if(module_num != -1) {
            ...
            for(Int_t im = 0; im < NModules; ++im) {
                BmnSiBTModule *module = g_current_station->GetModule(im);
