 ...
 * Author: D. Baranov
 *
 * Визуальная и параметрическая проверка CSC-конфигураций, включающая:
 *  - проверку расположения точки на станции, модуле, слое;
 *  - размеры модуля, слоя и его визуальное представление (фронтальный и верхний виды);
 *  - расположение мертвых зон, наклона стрипов, их границы нумерации и направление;
 *
 * Есть возможность интерактивного добаление точек на фронтальной плоскости при
 * помощи SHIFT+Mouse_click с автоматическим расчетом хитов-пересечений
 *
 * Обнуление информации на плоскости и станции
 */
...
BmnCSCStation *g_current_station = 0;
...
//Возможность добавления точек мышкой
Bool_t g_EnableAddingPoints = true;

//Рисовать добавленные мышкой точки на канвах отдельных слоев
Bool_t g_DrawOnSeparateLayers = false;

//Выводить информацию в поток обо всех хитах на станции при добавлении новой точки
Bool_t g_ShowIntersectionPointInfo = true;

void DrawCreatedStationFront(BmnCSCStation *station, vector<TLorentzVector> &points);
void DrawCreatedStationTop(BmnCSCStation *station, vector<TLorentzVector> &points);

void TestCSCConfig_Visual() {
    ...
    //Create configuration from an xml-file
    BmnCSCStationSet *StationSet = new BmnCSCStationSet(gPathDetectorConfig);

    //Определение принадлежности по конкретной точке ---------------------------
    if(1) {

        //Тестовая точка (без обработкой станцией - просто определение станции по ее положению)
        //Double_t x = +115.31016; //don't forget to invert
        //Double_t y = -25.76025;
        //Double_t z = -446.18716;

        Double_t x = 0.0; //don't forget to invert
        Double_t y = 0.0;
        Double_t z = 816.0085 + 2.5;
        ...
        if(station_num != -1) {
            BmnCSCStation *station = StationSet->GetStation(station_num);
...
void DrawCreatedStationFront(BmnCSCStation *station, vector<TLorentzVector> &points) {
    ...
    BmnCSCModule **Modules = new BmnCSCModule*[NModules];
    ...
    */
    ...
    for(Int_t im = 0; im < NModules; im++) {
        ...
        for(Int_t ilayer = 0; ilayer < NLayers; ++ilayer) {
            ...
            BmnCSCLayer layer = Modules[im]->GetStripLayer(ilayer);
...
//void RecognizeModuleNumFromPointAndDrawIt() {
    ...
    if(event == kButton1Shift) { //mouse click with shift button
        ...
        if(module_num != -1) {
            ...
            BmnCSCModule *module = g_current_station->GetModule(module_num);
            Int_t NLayers = module->GetNStripLayers();
        ...
        if(module_num != -1) {
            ...
            for(Int_t im = 0; im < NModules; ++im) {
                BmnCSCModule *module = g_current_station->GetModule(im);
