/*
 * Macro:
 * Visual test of your VSP configuration, including:
...
BmnVSPStation *g_current_station = 0;
...
void DrawCreatedStationFront(BmnVSPStation *station, vector<TLorentzVector> &points);
void RecognizeModuleNumFromPointAndDrawIt(TCanvas *station_front_canv);
void DrawCreatedStationTop(BmnVSPStation *station, vector<TLorentzVector> &points);
//------------------------------------------------------------------------------

void TestVSPConfig_Visual() {
    ...
    if(station_num != -1) {
        BmnVSPStation *station = StationSet->GetVSPStation(station_num);
...
void DrawCreatedStationFront(BmnVSPStation *station, vector<TLorentzVector> &points) {
    ...
    BmnVSPModule **Modules = new BmnVSPModule*[NModules];
    ...
    for(Int_t im = 0; im < NModules; im++) {
        ...
        for(Int_t ilayer = 0; ilayer < NLayers; ++ilayer) {
            ...
            BmnVSPLayer layer = Modules[im]->GetStripLayer(ilayer);
...
//void RecognizeModuleNumFromPointAndDrawIt() {
    ...
    if(event == kButton1Shift) { //mouse click with shift button
        ...
        for(Int_t imod = 0; imod < g_current_station->GetNModules(); ++imod) {
            BmnVSPModule *module = g_current_station->GetModule(imod);
        ...
        if(module_num != -1) {
            ...
            BmnVSPModule *module = g_current_station->GetModule(module_num);
            Int_t NLayers = module->GetNStripLayers();
        ...
        if(module_num != -1) {
            ...
            for(Int_t im = 0; im < NModules; ++im) {
                BmnVSPModule *module = g_current_station->GetModule(im);
