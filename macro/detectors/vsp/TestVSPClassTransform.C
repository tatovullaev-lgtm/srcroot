/*
 * This macro for testing configurations of VSP detector
 * which have module transformations the XML-parameter file.
 *
...
BmnVSPStationSet *gStationSet;
BmnVSPTransform *gTransfClass;
...
void TestVSPClassTransform() {
    ...
    Int_t NEvents_fVSPMCPoints = fVSPMCPoints_Tree->GetEntries();


    // Input hits for VSP
    TClonesArray* fVSPHits_Array = new TClonesArray("");
    TString fVSPHits_FileName = "$VMCWORKDIR/macro/run/bmndst";
        fVSPHits_FileName += file_prefix;
        fVSPHits_FileName += ".root";
    TFile *fVSPHits_File = new TFile(fVSPHits_FileName, "READ");
    TTree *fVSPHits_Tree = (TTree*) fVSPHits_File->Get("bmndata");
    fVSPHits_Tree->GetBranch("BmnVSPHit")->SetAutoDelete(kTRUE);
    fVSPHits_Tree->SetBranchAddress("BmnVSPHit", &fVSPHits_Array);
    Int_t NEvents_fVSPHits = fVSPHits_Tree->GetEntries();
...
//------------------------------------------------------------------------------
    ...
    Double_t zmin_view = +5.0*scale_coeff; //was -100
    Double_t zmax_view = +10.0*scale_coeff;
...
//------------------------------------------------------------------------------
    ...
    gStationSet = new BmnVSPStationSet(gPathDetectorConfig);

    //Current transform configuration
    gTransfClass = new BmnVSPTransform();
    ...
    for(Int_t istation = 0; istation < gStationSet->GetNStations(); ++istation) {
        ...
        for(Int_t imodule = 0; imodule < gStationSet->GetStation(istation)->GetNModules(); ++imodule) {
            ...
            if(visual_status == true) {
                ...
                BmnVSPModule *module = gStationSet->GetStation(istation)->GetModule(imodule);
...
;
    //DrawBranchPoints(fVSPMCPoints_Tree, fVSPMCPoints_Array, 0, NEvents_fVSPMCPoints);
    //DrawBranchHits(fVSPHits_Tree, fVSPHits_Array, 0, NEvents_fVSPMCPoints);

    DrawBranchPoints(fVSPMCPoints_Tree, fVSPMCPoints_Array, 0, 1);
    DrawBranchHits(fVSPHits_Tree, fVSPHits_Array, 0, 1);
...
void DrawBranchPoints(TTree *points_tree, TClonesArray *points_array, Int_t first_event, Int_t last_event) {
    ...
    for(Int_t ievent = first_event; ievent < last_event; ++ievent) {
        ...
        for(Int_t ipoint = 0; ipoint < npoints; ++ipoint) {
            ...
            BmnVSPPoint *mc_point = (BmnVSPPoint*) points_array->At(ipoint);
...
void DrawBranchHits(TTree *points_tree, TClonesArray *points_array, Int_t first_event, Int_t last_event) {
    ...
    for(Int_t ievent = first_event; ievent < last_event; ++ievent) {
        ...
        for(Int_t ihit = 0; ihit < npoints; ++ihit) {
            ...
            BmnVSPHit *hit = (BmnVSPHit*) points_array->At(ihit);
