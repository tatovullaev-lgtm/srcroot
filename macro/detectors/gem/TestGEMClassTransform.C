/*
 * This macro for testing configurations of the GEM detector
...
BmnGemStripStationSet *gStationSet;
BmnGemStripTransform *gTransfClass;
...
void TestGEMClassTransform() {

//------------------------------------------------------------------------------
    TString file_prefix = "";
    //file_prefix = "_CC_4gev_b0_16fm";

    // Input MC-points for GEM (called StsPoint)
    TClonesArray* fGemMCPoints_Array = new TClonesArray("");
    TString fGemMCPoints_FileName = "$VMCWORKDIR/macro/run/bmnsim";
        fGemMCPoints_FileName += file_prefix;
        fGemMCPoints_FileName += ".root";
    TFile *fGemMCPoints_File = new TFile(fGemMCPoints_FileName, "READ");
    TTree *fGemMCPoints_Tree = (TTree*) fGemMCPoints_File->Get("bmndata");
    fGemMCPoints_Tree->GetBranch("StsPoint")->SetAutoDelete(kTRUE);
    fGemMCPoints_Tree->SetBranchAddress("StsPoint", &fGemMCPoints_Array);
    Int_t NEvents_fGemMCPoints = fGemMCPoints_Tree->GetEntries();

    // Input hits for GEM
    TClonesArray* fGemHits_Array = new TClonesArray("");
    TString fGemHits_FileName = "$VMCWORKDIR/macro/run/bmndst";
        fGemHits_FileName += file_prefix;
        fGemHits_FileName += ".root";
    TFile *fGemHits_File = new TFile(fGemHits_FileName, "READ");
    TTree *fGemHits_Tree = (TTree*) fGemHits_File->Get("bmndata");
    fGemHits_Tree->GetBranch("BmnGemStripHit")->SetAutoDelete(kTRUE);
    fGemHits_Tree->SetBranchAddress("BmnGemStripHit", &fGemHits_Array);
    Int_t NEvents_fGemHits = fGemHits_Tree->GetEntries();
...
//------------------------------------------------------------------------------
    ...
    Double_t zmin_view = 0.0; //was -100
...
//------------------------------------------------------------------------------
    ...
    //gPathDetectorConfig += "GemRunSRCSpring2018.xml";
    gPathDetectorConfig += "GemRun8.xml";


    //Current configuration
    gStationSet = new BmnGemStripStationSet(gPathDetectorConfig);


    //Current transform configuration
    gTransfClass = new BmnGemStripTransform();
    ...
    for(Int_t istation = 0; istation < gConfig_visual.size(); ++istation) {
        gConfig_visual[istation].resize(gStationSet->GetGemStation(istation)->GetNModules(), false); //default: inactive
    }

    for(Int_t istation = 0; istation < gStationSet->GetNStations(); ++istation) {
        for(Int_t imodule = 0; imodule < gStationSet->GetGemStation(istation)->GetNModules(); ++imodule) {
    ...
    for(Int_t istation = 0; istation < gStationSet->GetNStations(); ++istation) {
        ...
        for(Int_t imodule = 0; imodule < gStationSet->GetGemStation(istation)->GetNModules(); ++imodule) {
            ...
            if(visual_status == true) {
                ...
                BmnGemStripModule *module = gStationSet->GetGemStation(istation)->GetModule(imodule);
                ...
                Double_t z_start_pos_module = gStationSet->GetGemStation(istation)->GetModule(imodule)->GetZStartModulePosition();
                Double_t z_reg_module = gStationSet->GetGemStation(istation)->GetModule(imodule)->GetZPositionRegistered();
    ...
    //DrawBranchPoints(fGemMCPoints_Tree, fGemMCPoints_Array, 0, NEvents_fGemMCPoints);
    //DrawBranchHits(fGemHits_Tree, fGemHits_Array, 0, NEvents_fGemMCPoints);

    DrawBranchPoints(fGemMCPoints_Tree, fGemMCPoints_Array, 0, 1);
    DrawBranchHits(fGemHits_Tree, fGemHits_Array, 0, 1);
...
void DrawBranchPoints(TTree *points_tree, TClonesArray *points_array, Int_t first_event, Int_t last_event) {
    ...
    for(Int_t ievent = first_event; ievent < last_event; ++ievent) {
        ...
        for(Int_t ipoint = 0; ipoint < npoints; ++ipoint) {
            ...
            CbmStsPoint *mc_point = (CbmStsPoint*) points_array->At(ipoint);
            ...
            if(station_num < gStationSet->GetNStations()) {
                if(module_num < gStationSet->GetGemStation(station_num)->GetNModules()) {
            ...
            if(current_station_num >= 0) current_module_num = gStationSet->GetGemStation(current_station_num)->GetPointModuleOwnership(-xloc, yloc, zloc);
...
void DrawBranchHits(TTree *points_tree, TClonesArray *points_array, Int_t first_event, Int_t last_event) {
    ...
    for(Int_t ievent = first_event; ievent < last_event; ++ievent) {
        ...
        for(Int_t ihit = 0; ihit < npoints; ++ihit) {
            ...
            BmnGemStripHit *hit = (BmnGemStripHit*) points_array->At(ihit);
