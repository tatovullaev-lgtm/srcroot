...
BmnGemStripTransform::BmnGemStripTransform() {
...
void BmnGemStripTransform::PrintTest() {
    for(Int_t iStation = 0; iStation < transform_stack.size(); ++iStation) {
        cout << " iStation[" << iStation << "]:\n";

        for(Int_t iModule = 0; iModule < transform_stack[iStation].size(); ++iModule) {
            cout << "   iModule[" << iModule << "]:\n";

            for(Int_t iTransform = 0; iTransform < transform_stack[iStation][iModule].size(); ++iTransform) {
...
void BmnGemStripTransform::Reset() {
    transform_stack.clear();
}

Bool_t BmnGemStripTransform::LoadFromXMLFile(TString xml_config_file) {
    ...
    if(parse_status != 0) {
        std::cerr << "Error: An error occured when parsing the file! (in BmnGemStripTransform)\n";
        return false;
    }
...
Plane3D::Point BmnGemStripTransform::ApplyTransforms(Plane3D::Point point, Int_t station, Int_t module) {
...
Plane3D::Point BmnGemStripTransform::ApplyInverseTransforms(Plane3D::Point point, Int_t station, Int_t module) {
    Plane3D::Point tpoint = point; //transformed point
