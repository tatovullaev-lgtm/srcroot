...
Bool_t BmnSiliconModule::IsPointInsideModule(Double_t x, Double_t y, Bool_t isLocal = false) {
...
Bool_t BmnSiliconModule::IsPointInsideModule(Double_t x, Double_t y, Double_t z, Bool_t isLocal = false) {
...
Bool_t BmnSiliconModule::AddRealPointFullOne_WithIncline(Double_t x, Double_t y, Double_t z,
                               Double_t px, Double_t py, Double_t pz, Double_t signal, Int_t refID) {
...
Bool_t BmnSiliconModule::AddRealPointFull(Double_t x, Double_t y, Double_t z,
                               Double_t px, Double_t py, Double_t pz, Double_t signal, Int_t refID) {

    //Convert coodinates if rotation around Z axis is used ---------------------
    Double_t x_loc = ConvertRotatedToLocalX(x, y);
    Double_t y_loc = ConvertRotatedToLocalY(x, y);
    Double_t z_loc = z;
    ...
    if( IsPointInsideModule(x_loc, y_loc, true) ) {
        ...
        if(z_distance_from_in_to_out <= 0.0) {
            if(Verbosity) cerr << "WARNING: BmnSiliconModule::AddRealPointFull(): Point (" << x << " : " << y << " : " << z << ") has a track with an incorrect length\n";
            return false;
        }
        ...
        for(Int_t icollpoint = 0; icollpoint < n_collisions; ++icollpoint) {
            ...
            if(chargedCarrierCloudRadius <= 0.0) {
                if(Verbosity) cerr << "WARNING: BmnSiliconModule::AddRealPointFull(): incorrect chargedCarrierCloudRadius!\n";
                return false;
            }
        ...
        if(Verbosity) cerr << "WARNING: BmnSiliconModule::AddRealPointFull(): Point (" << x << " : " << y << " : " << z << ") is out of the readout plane or inside a dead zone\n";
        return false;
    }
}
