...
void BmnSiliconTransform::PrintTest() {
...
void BmnSiliconTransform::Reset() {
    transform_stack.clear();
}

Bool_t BmnSiliconTransform::LoadFromXMLFile(TString xml_config_file) {
    ...
    if(parse_status != 0) {
        std::cerr << "Error: An error occured when parsing the file! (in BmnSiliconTransform)\n";
        return false;
    }
...
Plane3D::Point BmnSiliconTransform::ApplyTransforms(Plane3D::Point point, Int_t station, Int_t module) {
...
Plane3D::Point BmnSiliconTransform::ApplyInverseTransforms(Plane3D::Point point, Int_t station, Int_t module) {
    Plane3D::Point tpoint = point; //transformed point
