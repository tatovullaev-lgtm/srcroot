    ...
    , Modules(NULL)
{
    ...
    if (!create_status) {
        std::cerr << "Error: There are problems with creation of the configuration from XML node (in BmnSiBTStation)\n";
    }
...
Int_t BmnSiBTStation::GetPointModuleOwnership(Double_t xcoord, Double_t ycoord, Double_t zcoord)
{
    for (Int_t imodule = 0; imodule < NModules; ++imodule) {
        if (Modules[imodule]->IsPointInsideModule(xcoord, ycoord, zcoord))
            return imodule;
    }
...
void BmnSiBTStation::DefineStationBorders()
{
    ...
    for (Int_t i = 0; i < NModules; ++i) {
        if (Modules[i]->GetXMinModule() < XMinStation)
            XMinStation = Modules[i]->GetXMinModule();
        if (Modules[i]->GetXMaxModule() > XMaxStation)
            XMaxStation = Modules[i]->GetXMaxModule();
        if (Modules[i]->GetYMinModule() < YMinStation)
            YMinStation = Modules[i]->GetYMinModule();
        if (Modules[i]->GetYMaxModule() > YMaxStation)
            YMaxStation = Modules[i]->GetYMaxModule();
...
Bool_t BmnSiBTStation::CreateConfigurationFromXMLNode(TXMLNode* node)
{
    NModules = CountNumberOfModules(node);

    Modules = new BmnSiBTModule*[NModules];
...
Int_t BmnSiBTStation::CountNumberOfModules(TXMLNode* node)
{
    Int_t module_cnt = 0;
    node = node->GetChildren();
    while (node) {
...
Bool_t BmnSiBTStation::ParseModule(TXMLNode* node, Int_t iModule)
{
    ...
    Modules[iModule] = new BmnSiBTModule(ZPosition + ZShiftOfModules[iModule], ThicknessOfModules[iModule]);
...
BmnSiBTLayer BmnSiBTStation::ParseLayer(TXMLNode* node, Int_t iLayer, Int_t iModule)
{
    ...
    StripNumberingDirection strip_direction = LeftToRight;
    ...
    BmnSiBTLayer layer(zone_number, layer_type, xsize, ysize, XShiftOfModules[iModule] + XPosition + xorig,
                       YShiftOfModules[iModule] + YPosition + yorig, pitch, adeg);

    layer.SetStripNumberingOrder(strip_direction);

    layer.SetStripNumberingBorders(
        XShiftOfModules[iModule] + XPosition + lx_border, YShiftOfModules[iModule] + YPosition + ly_border,
        XShiftOfModules[iModule] + XPosition + rx_border, YShiftOfModules[iModule] + YPosition + ry_border);
...
DeadZoneOfStripLayer BmnSiBTStation::ParseDeadZone(TXMLNode* node, Int_t iModule)
{
            ...
            if (node->HasAttributes()) {
                ...
                while ((attr = (TXMLAttr*)next()) != nullptr) {
                    if (strcmp(attr->GetName(), "x") == 0) {
                        xpoints[currentDeadPointNum] =
                            -atof(attr->GetValue()) + XShiftOfModules[iModule] + XPosition;   // inverted
                    }
...
Int_t BmnSiBTStation::CountDeadZonePoints(TXMLNode* node)
{
