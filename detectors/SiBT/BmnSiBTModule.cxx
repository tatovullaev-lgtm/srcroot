...
BmnSiBTModule::BmnSiBTModule(Double_t z_start_pos, Double_t module_thick)
{
...
Bool_t BmnSiBTModule::SetStripSignalInLayer(Int_t layer_num, Int_t strip_num, Double_t signal)
{
...
Bool_t BmnSiBTModule::AddStripSignalInLayer(Int_t layer_num, Int_t strip_num, Double_t signal)
{
    if (layer_num >= 0 && layer_num < (Int_t)StripLayers.size())
...
Bool_t BmnSiBTModule::SetStripMatchInLayer(Int_t layer_num, Int_t strip_num, BmnMatch strip_match)
{
    if (layer_num >= 0 && layer_num < (Int_t)StripLayers.size())
...
Double_t BmnSiBTModule::GetStripSignalInLayer(Int_t layer_num, Int_t strip_num)
{
    if (layer_num >= 0 && layer_num < (Int_t)StripLayers.size())
...
BmnMatch BmnSiBTModule::GetStripMatchInLayer(Int_t layer_num, Int_t strip_num)
{
    if (layer_num >= 0 && layer_num < (Int_t)StripLayers.size())
...
Bool_t BmnSiBTModule::IsPointInsideModule(Double_t x, Double_t y)
{
    for (size_t ilayer = 0; ilayer < StripLayers.size(); ++ilayer) {
...
Bool_t BmnSiBTModule::IsPointInsideModule(Double_t x, Double_t y, Double_t z)
{
                                          ...
                                          Int_t refID)
{
    ...
    if (IsPointInsideModule(x, y)) {
        ...
        if (Verbosity)
            cerr << "WARNING: BmnSiBTModule::AddRealPointFullOne(): Point (" << x << " : " << y << " : " << z
                                         ...
                                         Int_t refID)
{
    ...
    if (IsPointInsideModule(x, y)) {
        ...
        if (Verbosity)
            cerr << "WARNING: BmnSiBTModule::AddRealPointSimple(): Point (" << x << " : " << y
                 << ") is out of the readout plane or inside a dead zone\n";
        return false;
    }
}
...
void BmnSiBTModule::DefineModuleBorders()
{
