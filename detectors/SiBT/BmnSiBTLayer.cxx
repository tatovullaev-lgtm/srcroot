...
BmnSiBTLayer::BmnSiBTLayer(Int_t zone_number, StripLayerType layer_type,
...
void BmnSiBTLayer::SetLayerSizes(Double_t xsize, Double_t ysize, Double_t xorig, Double_t yorig) {
    XMinLayer = xorig;
    XMaxLayer = xorig + xsize;
    YMinLayer = yorig;
    YMaxLayer = yorig + ysize;
...
void BmnSiBTLayer::SetAngleDeg(Double_t deg) {
    if(Abs(deg) <= 90.0) {
        ...
        AngleDeg = 0.0;
        if(Verbosity) cerr << "WARNING: BmnSiBTLayer::SetAngleDeg(): the strip angle is incorrect and has been set to zero\n";
    }
...
Bool_t BmnSiBTLayer::SetStripNumberingBorders(Double_t x_left, Double_t y_left, Double_t x_right, Double_t y_right) {
...
Bool_t BmnSiBTLayer::SetStripNumberingBorders(StripBorderPoint left, StripBorderPoint right) {
...
Bool_t BmnSiBTLayer::AddDeadZone(Int_t n_points, Double_t *x_points, Double_t *y_points) {
...
Bool_t BmnSiBTLayer::IsPointInsideDeadZones(Double_t x, Double_t y) {
...
Bool_t BmnSiBTLayer::IsPointInsideStripLayer(Double_t x, Double_t y) {
...
Bool_t BmnSiBTLayer::SetStripSignal(Int_t strip_num, Double_t signal) {
...
Bool_t BmnSiBTLayer::AddStripSignal(Int_t strip_num, Double_t signal) {
    if (strip_num >= 0 && strip_num < (Int_t)Strips.size()) {
        Strips[strip_num] += signal;
        return true;
    }
...
Bool_t BmnSiBTLayer::SetStripMatch(Int_t strip_num, BmnMatch strip_match) {
...
Bool_t BmnSiBTLayer::AddLinkToStripMatch(Int_t strip_num, Double_t weight, Int_t refID) {
    if (strip_num >= 0 && strip_num < (Int_t)StripMatches.size()) {
...
Double_t BmnSiBTLayer::ConvertNormalPointToStripX(Double_t x, Double_t y) {
...
Double_t BmnSiBTLayer::ConvertNormalPointToStripY(Double_t x, Double_t y) {
    Double_t XRotationCenter;
    Double_t YRotationCenter;
...
Double_t BmnSiBTLayer::ConvertPointToStripPosition(Double_t x, Double_t y) {
...
void BmnSiBTLayer::FindClustersAndStripHits() {
 ...
 void BmnSiBTLayer::MakeStripHit(StripCluster &cluster, vector<Double_t> &AnalyzableStrips, Int_t &curcnt) {
...
//    }
 ...
 void BmnSiBTLayer::SmoothStripSignal(vector<Double_t>& AnalyzableStrips, Int_t NIterations, Int_t SmoothWindow, Double_t Weight) {
