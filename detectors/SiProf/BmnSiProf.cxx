...
BmnSiProf::BmnSiProf()
: FairDetector("SiProf", kTRUE) {
    fPointCollection = new TClonesArray("BmnSiProfPoint");
    fPosIndex = 0;
    fVerboseLevel = 1;
    ResetParameters();
}

BmnSiProf::BmnSiProf(const char* name, Bool_t active)
...
Bool_t BmnSiProf::ProcessHits(FairVolume* vol) {
    ...
    // Create BmnSiProfPoint at EXIT of active volume;
	...
	fPosOut = pos - corr_step;
        ...
        TRegexp expr = "^SiProfSensor_module[0-9]+_station[0-9]+$";
        ...
        /*BmnSiProfPoint *p = */AddHit(fTrackID, fVolumeID,
                                fPosIn, fPosOut,
                                fMomIn, fMomOut,
        ...
        ((CbmStack*)gMC->GetStack())->AddPoint(kSiProf);
    }
...
void BmnSiProf::Register() {
    FairRootManager::Instance()->Register("SiProfPoint", "SiProf", fPointCollection, kTRUE);
}
...
void BmnSiProf::Print(Option_t*) const {
    ...
    cout << "-I- BmnSiProf: " << nHits << " points registered in this event." << endl;
...
void BmnSiProf::CopyClones(TClonesArray* cl1, TClonesArray* cl2, Int_t offset) {
    Int_t nEntries = cl1->GetEntriesFast();
    cout << "-I- BmnSiProf: " << nEntries << " entries to add." << endl;
    TClonesArray& clref = *cl2;
    BmnSiProfPoint* oldpoint = NULL;

    for(Int_t i = 0; i < nEntries; i++) {
        oldpoint = (BmnSiProfPoint*) cl1->At(i);
        Int_t index = oldpoint->GetTrackID() + offset;
        oldpoint->SetTrackID(index);
        new (clref[fPosIndex]) BmnSiProfPoint(*oldpoint);
        fPosIndex++;
    }

    cout << "-I- BmnSiProf: " << cl2->GetEntriesFast() << " merged entries."  << endl;
}
...
void BmnSiProf::ConstructGeometry() {
    ...
    if( fileName.EndsWith(".root") ) {
        LOG(info) << "Constructing SiProf geometry from ROOT file " << fileName.Data();
        ConstructRootGeometry();
    }

    else if ( fileName.EndsWith(".gdml") ) {
        LOG(info) << "Constructing SiProf geometry from GDML file " << fileName.Data();
        ConstructGDMLGeometry();
    }

    else {
        LOG(fatal) << "Geometry format of SiProf file " << fileName.Data() << " not supported.";
    }
}

// -----   ConstructGDMLGeometry   ---------------------------------------------
void BmnSiProf::ConstructGDMLGeometry() {
    TFile *old = gFile;
    TGDMLParse parser;
    TGeoVolume* gdmlTop;
...
Bool_t BmnSiProf::CheckIfSensitive(std::string name) {
    TString tsname = name;
    if(tsname.Contains("SiProfSensor")) {
        return kTRUE;
    }
    return kFALSE;
}
...
//------------------------------------------------------------------------------
BmnSiProfPoint* BmnSiProf::AddHit(Int_t trackID, Int_t detID,
    ...
    new(clref[size]) BmnSiProfPoint(trackID, detID,
                                        posIn, posOut, momIn, momOut,
                                        time, length, eLoss,
                                        isPrimary, charge, pdgId);

    BmnSiProfPoint *hit = (BmnSiProfPoint*) clref.At(clref.GetEntriesFast() - 1);
