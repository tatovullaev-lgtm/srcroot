...
BmnSiProfStation::BmnSiProfStation(TXMLNode *stationNode, Int_t iStation,
  ...
  Modules(NULL)
{
    ...
    if(!create_status) {
        std::cerr << "Error: There are problems with creation of the configuration from XML node (in BmnSiProfStation)\n";
    }
...
Int_t BmnSiProfStation::AddPointToStation(Double_t xcoord, Double_t ycoord, Double_t zcoord,
...
Int_t BmnSiProfStation::GetPointModuleOwnership(Double_t xcoord, Double_t ycoord, Double_t zcoord) {
    for(Int_t imodule = 0; imodule < NModules; ++imodule) {
...
Bool_t BmnSiProfStation::CreateConfigurationFromXMLNode(TXMLNode *node) {
    NModules = CountNumberOfModules(node);

    Modules = new BmnSiProfModule* [NModules];
...
Int_t BmnSiProfStation::CountNumberOfModules(TXMLNode *node) {
    Int_t module_cnt = 0;
    node = node->GetChildren();
    while(node) {
        if( strcmp(node->GetNodeName(), "Module") == 0 ) {
...
Bool_t BmnSiProfStation::ParseModule(TXMLNode *node, Int_t iModule) {
    ...
    Modules[iModule] = new BmnSiProfModule(ZPosition+ZShiftOfModules[iModule], ThicknessOfModules[iModule]);
...
BmnSiProfLayer BmnSiProfStation::ParseLayer(TXMLNode *node, Int_t iLayer, Int_t iModule) {
    ...
    BmnSiProfLayer layer(zone_number, layer_type,
                      xsize, ysize,
...
DeadZoneOfStripLayer BmnSiProfStation::ParseDeadZone(TXMLNode *node, Int_t iModule) {
...
Int_t BmnSiProfStation::CountDeadZonePoints(TXMLNode *node) {
