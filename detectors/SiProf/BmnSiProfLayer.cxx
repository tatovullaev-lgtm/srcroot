...
BmnSiProfLayer::BmnSiProfLayer() {
    ...
    Pitch = 0.0475; //cm
    AngleDeg = 90.0; //in degrees from a vertical line where a plus value is clockwise
...
BmnSiProfLayer::BmnSiProfLayer(Int_t zone_number, StripLayerType layer_type,
...
void BmnSiProfLayer::SetLayerSizes(Double_t xsize, Double_t ysize, Double_t xorig, Double_t yorig) {
    XMinLayer = xorig;
    XMaxLayer = xorig + xsize;
    YMinLayer = yorig;
    YMaxLayer = yorig + ysize;
...
void BmnSiProfLayer::SetAngleDeg(Double_t deg) {
    if(Abs(deg) <= 90.0) {
        ...
        AngleDeg = 0.0;
        if(Verbosity) cerr << "WARNING: BmnSiProfLayer::SetAngleDeg(): the strip angle is incorrect and has been set to zero\n";
    }
...
Bool_t BmnSiProfLayer::SetStripNumberingBorders(Double_t x_left, Double_t y_left, Double_t x_right, Double_t y_right) {
...
Bool_t BmnSiProfLayer::SetStripNumberingBorders(StripBorderPoint left, StripBorderPoint right) {
...
Bool_t BmnSiProfLayer::AddDeadZone(Int_t n_points, Double_t *x_points, Double_t *y_points) {
...
Bool_t BmnSiProfLayer::IsPointInsideDeadZones(Double_t x, Double_t y) {
...
Bool_t BmnSiProfLayer::IsPointInsideStripLayer(Double_t x, Double_t y) {
...
Bool_t BmnSiProfLayer::SetStripSignal(Int_t strip_num, Double_t signal) {
...
Bool_t BmnSiProfLayer::AddStripSignal(Int_t strip_num, Double_t signal) {
    if (strip_num >= 0 && strip_num < (Int_t)Strips.size()) {
        Strips[strip_num] += signal;
        return true;
    }
...
Bool_t BmnSiProfLayer::SetStripMatch(Int_t strip_num, BmnMatch strip_match) {
...
Bool_t BmnSiProfLayer::AddLinkToStripMatch(Int_t strip_num, Double_t weight, Int_t refID) {
    if (strip_num >= 0 && strip_num < (Int_t)StripMatches.size()) {
...
Double_t BmnSiProfLayer::ConvertNormalPointToStripX(Double_t x, Double_t y) {
...
Double_t BmnSiProfLayer::ConvertNormalPointToStripY(Double_t x, Double_t y) {
    Double_t XRotationCenter;
    Double_t YRotationCenter;
...
Double_t BmnSiProfLayer::ConvertPointToStripPosition(Double_t x, Double_t y) {
...
void BmnSiProfLayer::FindClustersAndStripHits() {
 ...
 void BmnSiProfLayer::MakeStripHit(StripCluster &cluster, vector<Double_t> &AnalyzableStrips, Int_t &curcnt) {
...
//    }
 ...
 void BmnSiProfLayer::SmoothStripSignal(vector<Double_t>& AnalyzableStrips, Int_t NIterations, Int_t SmoothWindow, Double_t Weight) {
