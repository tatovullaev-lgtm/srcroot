...
BmnSiProfStationSet::BmnSiProfStationSet(TString xml_config_file, map<Int_t, TVector3>* shifts)
    : NStations(0)
    , XStationPositions(NULL)
    , YStationPositions(NULL)
    , ZStationPositions(NULL)
    ...
    , fStatShifts(shifts)
{
    ...
    if (!create_status) {
        std::cerr << "Error: There are problems with creation of the configuration from XML (in BmnSiProfStationSet)\n";
        throw(StationSet_Exception("Error in the constructor BmnSiProfStationSet()"));
    }
}
...
Int_t BmnSiProfStationSet::GetPointStationOwnership(Double_t xcoord, Double_t ycoord, Double_t zcoord)
{
    // for z-positions and z-shifts of all modules in a station
    for (Int_t iStation = 0; iStation < NStations; iStation++) {
        Int_t NModules = Stations[iStation]->GetNModules();
...
Bool_t BmnSiProfStationSet::CreateConfigurationFromXMLFile(TString xml_config_file)
{
    ...
    if (parse_status != 0) {
        std::cerr << "Error: An error occured when parsing the file! (in BmnSiProfStationSet)\n";
        return false;
    }
    ...
    if (strcmp(node->GetNodeName(), "StationSet") != 0) {
        std::cerr << "Error: Incorrect name of the root element! (in BmnSiProfStationSet)\n";
        return false;
    }

    NStations = CountNumberOfStations(node);

    Stations = new BmnSiProfStation*[NStations];
...
Int_t BmnSiProfStationSet::CountNumberOfStations(TXMLNode* node)
{
    Int_t station_cnt = 0;
    node = node->GetChildren();
    while (node) {
...
Bool_t BmnSiProfStationSet::ParseStation(TXMLNode* node, Int_t iStation)
{
    ...
    Stations[iStation] = new BmnSiProfStation(node, iStation, XStationPositions[iStation] + dx,
                                              YStationPositions[iStation] + dy, ZStationPositions[iStation] + dz);

    return true;
}

unique_ptr<BmnSiProfStationSet> BmnSiProfStationSet::Create(Int_t period, Int_t stp)
{
    TString gPathConfig = getenv("VMCWORKDIR");
    std::ifstream f(gPathConfig + "/config/bmnconf.json");
    json data = json::parse(f);
    json jss = data["Detectors"]["siprof"]["StationSet"];
    ...
    return std::make_unique<BmnSiProfStationSet>(gPathCscConfig + xmlConfFileName);
}
