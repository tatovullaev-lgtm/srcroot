...
CbmStsParRootFileIo::CbmStsParRootFileIo(FairParRootFile* f)
...
Bool_t CbmStsParRootFileIo::init(FairParSet* pPar, Int_t* set)
{
    ...
    if (pFile) {
        if (!strcmp(name, "StsDigiPar"))
            return read(pPar);   // read((CbmStsDigiPar*)pPar,set);
    }
    cerr << "initialization of " << name << " not possible from ROOT file!" << endl;
    return kFALSE;
}

Bool_t CbmStsParRootFileIo::read(FairParSet* pPar)
{
