...
BmnVSPStationSet::BmnVSPStationSet(TString xml_config_file, map<Int_t, TVector3>* shifts)
    : NStations(0)
    , XStationPositions(NULL)
    , YStationPositions(NULL)
    , ZStationPositions(NULL)
    ...
    , fStatShifts(shifts)
{
    ...
    if (!create_status) {
        std::cerr << "Error: There are problems with creation of the configuration from XML (in BmnVSPStationSet)\n";
    }
}
...
{
    if (VSPStations && station_num >= 0 && station_num < NStations) {
        return VSPStations[station_num];
    } else {
        throw(StationSet_Exception("Error in the function GetVSPStation()"));
    }
}
                                            ...
                                            Int_t refID)
{
    ...
    if (station != -1) {
        if (VSPStations[station]->AddPointToStation(xcoord, ycoord, zcoord, px, py, pz, dEloss, refID) != -1)
            return true;
        else
            return false;
    }
...
Int_t BmnVSPStationSet::CountNAddedToDetectorPoints()
{
    Int_t points_sum = 0;
    for (Int_t iStation = 0; iStation < NStations; iStation++) {
        points_sum += VSPStations[iStation]->CountNAddedToStationPoints();
    }
    return points_sum;
}
...
{
    for (Int_t iStation = 0; iStation < NStations; iStation++) {
        VSPStations[iStation]->ProcessPointsInStation();
    }
}
...
Int_t BmnVSPStationSet::CountNProcessedPointsInDetector()
{
    Int_t points_sum = 0;
    for (Int_t iStation = 0; iStation < NStations; iStation++) {
        points_sum += VSPStations[iStation]->CountNProcessedPointInStation();
    }
    return points_sum;
}
...
Int_t BmnVSPStationSet::GetPointStationOwnership(Double_t zcoord)
{
    ...
    for (Int_t iStation = 0; iStation < NStations; iStation++) {
        Int_t NModules = VSPStations[iStation]->GetNModules();
        for (Int_t iModule = 0; iModule < NModules; ++iModule) {
            if (VSPStations[iStation]->GetModule(iModule)->IsPointInsideZThickness(zcoord)) {
                return iStation;
            }
        }
    }
...
Int_t BmnVSPStationSet::GetPointStationOwnership(Double_t xcoord, Double_t ycoord, Double_t zcoord)
{
    // for z-positions and z-shifts of all modules in a station
    for (Int_t iStation = 0; iStation < NStations; iStation++) {
        Int_t NModules = VSPStations[iStation]->GetNModules();
        for (Int_t iModule = 0; iModule < NModules; ++iModule) {
            // if( VSPStations[iStation]->GetModule(iModule)->IsPointInsideZThickness(zcoord) ) {
            if (VSPStations[iStation]->GetModule(iModule)->IsPointInsideModule(xcoord, ycoord, zcoord)) {
                return iStation;
            }
        }
    }
...
Bool_t BmnVSPStationSet::CreateConfigurationFromXMLFile(TString xml_config_file)
{
    ...
    if (parse_status != 0) {
        std::cerr << "Error: An error occured when parsing the file! (in BmnVSPStationSet)\n";
        return false;
    }
    ...
    if (strcmp(node->GetNodeName(), "StationSet") != 0) {
        std::cerr << "Error: Incorrect name of the root element! (in BmnVSPStationSet)\n";
        return false;
    }

    NStations = CountNumberOfStations(node);

    VSPStations = new BmnVSPStation*[NStations];
...
Int_t BmnVSPStationSet::CountNumberOfStations(TXMLNode* node)
{
    Int_t station_cnt = 0;
    node = node->GetChildren();
    while (node) {
...
Bool_t BmnVSPStationSet::ParseStation(TXMLNode* node, Int_t iStation)
{
    ...
    VSPStations[iStation] = new BmnVSPStation(node, iStation, XStationPositions[iStation] + dx,
                                              YStationPositions[iStation] + dy, ZStationPositions[iStation] + dz);
    return true;
}

unique_ptr<BmnVSPStationSet> BmnVSPStationSet::Create(Int_t period, Int_t stp)
{
    TString gPathConfig = getenv("VMCWORKDIR");
    std::ifstream f(gPathConfig + "/config/bmnconf.json");
    json data = json::parse(f);
    json jss = data["Detectors"]["vsp"]["StationSet"];
    ...
    return std::make_unique<BmnVSPStationSet>(gPathCscConfig + xmlConfFileName);
}