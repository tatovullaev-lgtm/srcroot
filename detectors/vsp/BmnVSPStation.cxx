...
BmnVSPStation::BmnVSPStation(TXMLNode *stationNode, Int_t iStation,
  ...
  Modules(NULL)
{
    ...
    if(!create_status) {
        std::cerr << "Error: There are problems with creation of the configuration from XML node (in BmnVSPStation)\n";
    }
...
Int_t BmnVSPStation::AddPointToStation(Double_t xcoord, Double_t ycoord, Double_t zcoord,
    ...
    if(module >= 0) {
        ...
        //if(Modules[module]->AddRealPointFullOne_WithIncline(xcoord, ycoord, zcoord, px, py, pz, dEloss, refID)) {
...
Int_t BmnVSPStation::GetPointModuleOwnership(Double_t xcoord, Double_t ycoord, Double_t zcoord) {
    for(Int_t imodule = 0; imodule < NModules; ++imodule) {
        if( Modules[imodule]->IsPointInsideModule(xcoord, ycoord, zcoord, false) ) return imodule;
    }
    return -1;
}
...
Bool_t BmnVSPStation::CreateConfigurationFromXMLNode(TXMLNode *node) {
    NModules = CountNumberOfModules(node);

    Modules = new BmnVSPModule* [NModules];
...
Int_t BmnVSPStation::CountNumberOfModules(TXMLNode *node) {
    Int_t module_cnt = 0;
    node = node->GetChildren();
    while(node) {
        if( strcmp(node->GetNodeName(), "Module") == 0 ) {
...
Bool_t BmnVSPStation::ParseModule(TXMLNode *node, Int_t iModule) {
    ...
    Modules[iModule] = new BmnVSPModule(ZPosition+ZShiftOfModules[iModule]);
...
BmnVSPLayer BmnVSPStation::ParseLayer(TXMLNode *node, Int_t iLayer, Int_t iModule) {
    ...
    BmnVSPLayer layer(zone_id, layer_type, first_strip_number,
                          xsize, ysize,
...
DeadZoneOfStripLayer BmnVSPStation::ParseDeadZone(TXMLNode *node, Int_t iModule) {
...
Int_t BmnVSPStation::CountDeadZonePoints(TXMLNode *node) {
