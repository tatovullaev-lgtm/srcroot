#ifndef BMNVSPSTATION_H
#define BMNVSPSTATION_H
...
class BmnVSPStation
{
  protected:
    ...
    BmnVSPModule **Modules; //[NModules] modules in the station [array]

  public:
    /* Constructor */
    BmnVSPStation();
    BmnVSPStation(TXMLNode *stationNode, Int_t iStation,
  ...
  private:
    ...
    BmnVSPLayer ParseLayer(TXMLNode *node, Int_t iLayer, Int_t iModule);
    DeadZoneOfStripLayer ParseDeadZone(TXMLNode *node, Int_t iModule);
    Int_t CountDeadZonePoints(TXMLNode *node);

  ClassDef(BmnVSPStation, 1);
};

#endif /* BMNVSPSTATION_H */
