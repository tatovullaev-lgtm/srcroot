...
BmnVSP::BmnVSP()
: FairDetector("VSP", kTRUE) {
    fPointCollection = new TClonesArray("BmnVSPPoint");
    fPosIndex = 0;
    fVerboseLevel = 1;
    ResetParameters();
}

BmnVSP::BmnVSP(const char* name, Bool_t active)
...
Bool_t BmnVSP::ProcessHits(FairVolume* vol) {
    ...
    // Create BmnVSPPoint at EXIT of active volume;
	...
	fPosOut = pos - corr_step;
        ...
        TRegexp expr = "^moduleV_module[0-9]+_station[0-9]+_VSP$";
        ...
        BmnVSPPoint* p = AddHit(fTrackID, fVolumeID,
            fPosIn, fPosOut,
            fMomIn, fMomOut,
        ...
        ((CbmStack*)gMC->GetStack())->AddPoint(kVSP);
    }
...
void BmnVSP::Register() {
    FairRootManager::Instance()->Register("VSPPoint", "VSP", fPointCollection, kTRUE);
}
...
void BmnVSP::Print(Option_t*) const {
    ...
    cout << "-I- BmnVSP: " << nHits << " points registered in this event." << endl;
...
void BmnVSP::CopyClones(TClonesArray* cl1, TClonesArray* cl2, Int_t offset) {
    Int_t nEntries = cl1->GetEntriesFast();
    cout << "-I- BmnVSP: " << nEntries << " entries to add." << endl;
    TClonesArray& clref = *cl2;
    BmnVSPPoint* oldpoint = NULL;

    for(Int_t i = 0; i < nEntries; i++) {
        oldpoint = (BmnVSPPoint*) cl1->At(i);
        Int_t index = oldpoint->GetTrackID() + offset;
        oldpoint->SetTrackID(index);
        new (clref[fPosIndex]) BmnVSPPoint(*oldpoint);
        fPosIndex++;
    }

    cout << "-I- BmnVSP: " << cl2->GetEntriesFast() << " merged entries."  << endl;
}
...
void BmnVSP::ConstructGeometry() {
    ...
    if( fileName.EndsWith(".root") ) {
        LOG(info) << "Constructing VSP geometry from ROOT file " << fileName.Data();
        ConstructRootGeometry();
    }

    else if ( fileName.EndsWith(".gdml") ) {
        LOG(info) << "Constructing VSP geometry from GDML file " << fileName.Data();
        ConstructGDMLGeometry();
    }

    else {
        LOG(fatal) << "Geometry format of VSP file " << fileName.Data() << " not supported.";
    }
}

// -----   ConstructGDMLGeometry   ---------------------------------------------
void BmnVSP::ConstructGDMLGeometry() {
    TFile *old = gFile;
    TGDMLParse parser;
    TGeoVolume* gdmlTop;
...
//------------------------------------------------------------------------------
BmnVSPPoint* BmnVSP::AddHit(Int_t trackID, Int_t detID,
    ...
    return new(clref[size]) BmnVSPPoint(trackID, detID,
