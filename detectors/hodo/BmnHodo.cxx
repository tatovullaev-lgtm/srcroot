...
Bool_t BmnHodo::ProcessHits(FairVolume* vol) {
  ...
  if (gMC->CurrentVolID(copyNoStickSens) == fHodoStickSensVolId)
 {
    ...
    copyNoSTICKSENSCom = sticksens; copyNoSTICKCom = stick;  
  }
...
// Create BmnZdcPoint
	...
	if(copyNoSTICKSENSCom==sticksens && copyNoSTICKCom==stick) {
	  if ( !GetHit(sticksens,stick) ) {
	    AddHit(fTrackID, ivol, sticksens, stick, TVector3(tPos.X(), tPos.Y(), tPos.Z()),TVector3(tMom.Px(), tMom.Py(), tMom.Pz()),time, length, fELoss);
	  }
	  else {
	    GetHit(sticksens,stick)->AddSTICK(fTrackID, ivol, sticksens, stick, TVector3(tPos.X(), tPos.Y(), tPos.Z()),TVector3(tMom.Px(), tMom.Py(), tMom.Pz()),time, length, fELoss);
	  }
...
BmnHodoPoint* BmnHodo::AddHit(Int_t trackID, Int_t detID, Int_t copyNo, Int_t copyNoMother, TVector3 pos, TVector3 mom, Double_t time, Double_t length, Double_t eLoss) {
