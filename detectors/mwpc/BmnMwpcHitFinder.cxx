...
InitStatus BmnMwpcHitFinder::Init() {
  ...
  for (int i=0; i < kNChambers; ++i) {
    ...
    if (fDebug) {
      ...
      h = new TH1D(Form("Num_layers_out_beam_Ch%d",  i), Form("Num_layers_out_beam_Ch%d; NFired_layers; Events",  i), 7, 0.0, 7.0);fList.Add(h);hNum_layers_out_beam_Ch.push_back(h);
      ...
      h2=  new TH2D(Form("ClustSizePlCh%d", i), Form("ClustSizePlCh%d", i), 6, 0.,6, 40, 0, 40);fList.Add(h2);hClustSizePlCh.push_back(h2);
      h = new TH1D(Form("dt_fastest_other_Ch%d", i), Form("dt_fastest_other_Ch%d", i), 100, 0.,200.);fList.Add(h);hdt_fastest_other_Ch.push_back(h);
      h = new TH1D(Form("dw_fastest_other_Ch%d", i), Form("dw_fastest_other_Ch%d", i), 20, 0.,20.);fList.Add(h);hdw_fastest_other_Ch.push_back(h);
      h = new TH1D(Form("NclustCh%d", i), Form("NclustCh%d", i), 20, 0.,20.);fList.Add(h);hNclustCh.push_back(h);
      

      if (i < 2 ){
        h = new TH1D(Form("sum_time_Ch%d", i), Form("sum of the time wires per cluster Ch%d;ns", i), 200, 0.,400.);
        h2 = new TH2D(Form("dt_dwire_Ch%d",  i), Form("dt vs dwire Ch%d;dwire;dt",  i), 20, 0.,20, 20, 0, 20);
      }else{
        h = new TH1D(Form("sum_time_Ch%d", i), Form("sum of the time wires per cluster Ch%d;ns", i), 500, 0.,1000.);
        h2 = new TH2D(Form("dt_dwire_Ch%d",  i), Form("dt vs dwire Ch%d;dwire;dt",  i), 20, 0.,20, 100, 0, 100);
      }
      fList.Add(h);hsum_time_Ch.push_back(h);
      fList.Add(h2);hdt_dwire_Ch.push_back(h2);
      
      h = new TH1D(Form("fired_wire_Ch%d", i),Form("fired_wire_Ch%d;Wires;Events", i), kNWires*6, 0., kNWires*6);fList.Add(h);hfired_wire_Ch.push_back(h);
      
      h = new TH1D(Form("WiresXm_Ch%d", i),Form("WiresXm_Ch%d", i),kNWires, 0., kNWires);fList.Add(h);hWiresXm.push_back(h);
      h = new TH1D(Form("WiresVm_Ch%d", i),Form("WiresVm_Ch%d", i),kNWires, 0., kNWires);fList.Add(h);hWiresVm.push_back(h);
      h = new TH1D(Form("WiresUp_Ch%d", i),Form("WiresUp_Ch%d", i),kNWires, 0., kNWires);fList.Add(h);hWiresUp.push_back(h);
      h = new TH1D(Form("WiresXp_Ch%d", i),Form("WiresXp_Ch%d", i),kNWires, 0., kNWires);fList.Add(h);hWiresXp.push_back(h);
      h = new TH1D(Form("WiresVp_Ch%d", i),Form("WiresVp_Ch%d", i),kNWires, 0., kNWires);fList.Add(h);hWiresVp.push_back(h);
      h = new TH1D(Form("WiresUm_Ch%d", i),Form("WiresUm_Ch%d", i),kNWires, 0., kNWires);fList.Add(h);hWiresUm.push_back(h);
  ...
  if (fDebug) {
    ...
    hfiredWireCh01 = new TH2D("hfiredWireCh01", "fired Wires Ch1 vs Ch0", 100, 0, 200, 100, 0, 200); fList.Add(hfiredWireCh01);
    hfiredWireCh23 = new TH2D("hfiredWireCh23", "fired Wires Ch3 vs Ch2", 100, 0, 200, 100, 0, 200); fList.Add(hfiredWireCh23);
    hfiredWireCh12 = new TH2D("hfiredWireCh12", "fired Wires Ch2 vs Ch1", 100, 0, 200, 100, 0, 200); fList.Add(hfiredWireCh12);
  ...
  if (fRunPeriod == 8) { //SRC
    //normal Z
    // //ch0
    // shift[0][0] += 0.001-0.01-0.0021  -0.027; //Ax
    // shift[0][2] += 0.001-0.00-0.0039  -0.0005; //Ay
    // shift[0][1] += -0.16+0.8222+0.01703+0.00028 +0.00089; //x
    // shift[0][3] +=  0.05+0.6-0.9853+0.45+0.42+0.01416-0.000103  -0.002055; //y
    // //ch1
    // shift[1][0] += 0.000-0.01-0.006584  -0.041; //Ax
    // shift[1][2] += 0.000-0.00-0.0006807 +0.015; //Ay
    // shift[1][1] += 0.000+0.08401+0.7577+0.03111+0.00131 +0.00338; //x
    // shift[1][3] += 0.000+0.5104+0.1317-0.115-0.003093-0.000109  -0.000102; //y
    // //ch2
    // shift[2][0] += 0.001+0.03+0.02739-0.000225  +0.02; //Ax
    // shift[2][2] += 0.001+0.03+0.03308-0.00039 +0.061; //Ay
    // shift[2][1] += -0.04-0.512-0.06476-0.0008322-0.00749-0.001362+0.000227-0.000454 ; //x
    // shift[2][3] += -0.4-0.2+0.1971+0.01691+0.0015 ; //y
    // //ch3
    // shift[3][0] += 0.000+0.03+0.03924-0.000191  +0.025; //Ax
    // shift[3][2] += 0.000+0.03+0.03904-0.000257  +0.021; //Ay
    // shift[3][1] += 0.000-0.53-0.03777-0.001819  ; //x
    // shift[3][3] += 0.000+0.2463+0.01361+0.00042 ; //y

    // Z fixed
    //ch0
    shift[0][0] += 0-0.02495+0.00023-0.00001; //Ax
    shift[0][2] += 0-0.001709+0.00001+0; //Ay
    shift[0][1] += 0+1.027+0.02727+0.00083; //x
    shift[0][3] += 0+0.6223+0.01231+0.00033; //y
    //ch1
    shift[1][0] += 0-0.01044+0.00009+0; //Ax
    shift[1][2] += 0-0.001562+0.00001+0; //Ay
    shift[1][1] += 0+0.4617+0.01395+0.00037; //x
    shift[1][3] += 0+0.4184+0.00865+0.00025; //y
    //ch2
    shift[2][0] += 0+0.05864-0.00048+0.00001; //Ax
    shift[2][2] += 0+0.06445-0.00076+0; //Ay
    shift[2][1] += 0-0.5479-0.06818-0.00893-0.00143; //x
    shift[2][3] += 0-0.3523-0.02948-0.00241-0.00022; //y
    //ch3
    shift[3][0] += 0+0.06939-0.00034+0; //Ax
    shift[3][2] += 0+0.06923-0.00045+0; //Ay
    shift[3][1] += 0-0.53-0.03777-0.00181-0.00010; //x
    shift[3][3] += 0+0.2463+0.01361+0.00042; //y
  }
  ...
  if(fRunPeriod == 8 ) { //SRC
    Beam_wires_min[2][0] = 35; Beam_wires_max[2][0] = 63;//x-
    Beam_wires_min[2][1] = 37; Beam_wires_max[2][1] = 58;//v-
    Beam_wires_min[2][2] = 38; Beam_wires_max[2][2] = 53;//u+
    Beam_wires_min[2][3] = 35; Beam_wires_max[2][3] = 58;//x+
    Beam_wires_min[2][4] = 39; Beam_wires_max[2][4] = 58;//v+
    Beam_wires_min[2][5] = 43; Beam_wires_max[2][5] = 55;//u-
    
    Beam_wires_min[3][0] = 33; Beam_wires_max[3][0] = 65;
    Beam_wires_min[3][1] = 33; Beam_wires_max[3][1] = 59;
    Beam_wires_min[3][2] = 36; Beam_wires_max[3][2] = 57;
    Beam_wires_min[3][3] = 34; Beam_wires_max[3][3] = 61;
    Beam_wires_min[3][4] = 38; Beam_wires_max[3][4] = 60;
    Beam_wires_min[3][5] = 42; Beam_wires_max[3][5] = 59;
  }
...
void BmnMwpcHitFinder::Exec(Option_t* opt) {
  ...
  for (Int_t iChamber = 0; iChamber < kNChambers; iChamber++) {
    ...
    */
    ...
    if (fDebug) hNbest_Ch.at(iChamber) -> Fill(Nbest_seg[iChamber]);
...
void BmnMwpcHitFinder::ReadWires(Double_t ***DigitsArray_, Int_t **iw_Ch_, vector<MC_points> & vec){
  ...
  Int_t count_wire[kNChambers];
  
  for (Int_t iChamber = 0; iChamber < kNChambers; iChamber++) {
    count_wire[iChamber] = 0;
    for (Int_t iplane = 0; iplane < kNPlanes; iplane++) {
      Nlay_w_wires[iChamber] += Fired_layer_[iChamber][iplane];
      counter_pl[iplane] = 0;
      counter_pl[iplane]   += iw_Ch_[iChamber][iplane];
      count_wire[iChamber] += iw_Ch_[iChamber][iplane];
  ...
  if (fDebug){
    hfiredWireCh01->Fill(count_wire[0],count_wire[1]);
    hfiredWireCh23->Fill(count_wire[2],count_wire[3]);
    hfiredWireCh12->Fill(count_wire[1],count_wire[2]);
  }
...
Double_t*** Coord_xuv_, Int_t **Nclust_, Int_t *counter_pl_) {
  ...
  if (chNum > 1) Cut_time_wire = 200.;//for multitracking
  ...
  if (fRunPeriod == 8 ){
    ...
    for (Int_t ipll = 0; ipll < kNPlanes; ipll++) {
     ...
     //---look at charge----
    // if (charge34 < 6 && charge35 < 6 && charge45 < 6 && Zincharge < 6) continue;

      for (Int_t iwire = 0; iwire < kNWires; iwire++) {
        
       // if (fDebug && DigitsArray_[chNum][ipll][iwire] > 0) cout<<" DigitsArray_["<<chNum<<"]["<<ipll<<"]["<<iwire<<"] "<<DigitsArray_[chNum][ipll][iwire]<<endl;
        //if (DigitsArray_[chNum][ipll][iwire] > 220.) DigitsArray_[chNum][ipll][iwire] = 0;
        ...
        //---------
        if (DigitsArray_[chNum][ipll][iwire] > time_limit) DigitsArray_[chNum][ipll][iwire] = 0.;// from Piskunov // 08.2022
        ...
        //if( Nfirst[chNum][ipll] < 0 && DigitsArray_[chNum][ipll][iwire] > 0.&& DigitsArray_[chNum][ipll][iwire] <= (Min_time_wires + Cut_time_wire)) Nfirst[chNum][ipll] = iwire;
        ...
        if( Nfirst[chNum][ipll] >= 0 && DigitsArray_[chNum][ipll][iwire] == 0.)   Nlast[chNum][ipll] = iwire -1;
                        //|| DigitsArray_[chNum][ipll][iwire+ 1] > (Min_time_wires + Cut_time_wire) ) ) Nlast[chNum][ipll] = iwire;// > DigitsArray_[chNum][ipll][iwire+ 1] > (Min_time_wires + 2*time_windows)
        
         
        if (Nfirst[chNum][ipll] >= 0 && Nlast[chNum][ipll] >= Nfirst[chNum][ipll]) {
          Int_t min_time_in_clust = 999, num_fast_wire = -1;
          sum_time = 0.;
          //if (fDebug ) cout<<" Nfirst "<<Nfirst[chNum][ipll]<<" Nlast "<<Nlast[chNum][ipll]<<endl;
          
          ClusterSize_[chNum][ipll][Nclust_[chNum][ipll]] = Nlast[chNum][ipll] - Nfirst[chNum][ipll] + 1;
          //if (fDebug ) cout<<" ClusterSize_ "<<ClusterSize_[chNum][ipll][Nclust_[chNum][ipll]]<<endl;
          ...
          for (Int_t iww = Nfirst[chNum][ipll] ; iww < Nlast[chNum][ipll] + 1; iww++) {
            ...
            sum_time += DigitsArray_[chNum][ipll][iww];
          }
          
          if (fDebug) hsum_time_Ch.at(chNum)->Fill(sum_time);
          if (DigitsArray_[chNum][ipll][iwire] > 0. && (iwire > num_fast_wire || iwire < num_fast_wire) ){
            if (fDebug) hdw_fastest_other_Ch.at(chNum)->Fill(fabs(num_fast_wire - iwire));
            if (fDebug) hdt_fastest_other_Ch.at(chNum)->Fill(fabs(DigitsArray_[chNum][ipll][num_fast_wire] - DigitsArray_[chNum][ipll][iwire]));
            if (fDebug) hdt_dwire_Ch.at(chNum)->Fill(fabs(num_fast_wire - iwire),fabs(DigitsArray_[chNum][ipll][num_fast_wire] - DigitsArray_[chNum][ipll][iwire]));
          }
          
          if (fDebug) hClusterSize.at(chNum)->Fill(ClusterSize_[chNum][ipll][Nclust_[chNum][ipll]]);

          if ( min_time_in_clust < 900) {
            //fastest wire per cluster
            //Coord_wire_[chNum][ipll][Nclust_[chNum][ipll] ] = num_fast_wire + 0.5;
            //coordinate is the middle of the cluster
            ...
            if (fDebug) hClustSizePlCh.at(chNum)->Fill(ipll,ClusterSize_[chNum][ipll][Nclust_[chNum][ipll]]);
  ...
  if (fDebug) hNclustCh.at(chNum)->Fill( Nclust_[chNum][0]);
...
void BmnMwpcHitFinder::ProcessSegments( Int_t chNum, Int_t *Nsegm, Double_t ***XVU_coor, Double_t ***Cluster_coor, Int_t **Nhits_Ch_,
  ...
  //if (fDebug) cout<<" Nlay_w_wires_["<<chNum<<"] "<<Nlay_w_wires_[chNum]<<endl;
  ...
  //if (Nsegm[chNum] > kBig - 2) return; //criterion for noisy chamber
  ...
  */
  //if (fDebug) cout<<" first view 6 points "<<endl;
...
void BmnMwpcHitFinder::SegmentParamAlignment(Int_t chNum, Int_t *Nbest, Double_t ***par_ab, Float_t **shiftt ) {
  ...
  if (fDebug) cout<<" shiftt["<<chNum<<"][0] "<<shiftt[chNum][0] <<" shiftt["<<chNum<<"][1] "<<shiftt[chNum][1] <<" shiftt["<<chNum<<"][2] "<<shiftt[chNum][2] <<" shiftt["<<chNum<<"][3] "<<shiftt[chNum][3] <<endl;
...
void BmnMwpcHitFinder::PrepareArraysToProcessEvent() {
  ...
  charge34=fDstEventHeader->GetZoutBC34_12();
  charge35=fDstEventHeader->GetZoutBC35_12();
  charge45=fDstEventHeader->GetZoutBC45_12();
  Zincharge = fDstEventHeader->GetZinBC12();
