...
void BmnMwpcTrackFinder::Exec(Option_t* opt) {
  ...
  if (fDebug){
    ...
    for (Int_t p = 0; p < kNumPairs; p++) {
      for (Int_t se = 0; se < Nbest_pair[p]; se++) {
  ...
  for (Int_t iPair = 0; iPair < kNumPairs; iPair++) {
    if ( Nbest[iPair] > 0) {
      ...
      for (Int_t itr = 0; itr < Nbest[iPair]; itr++) {
        ...
        if (fDebug){
          cout<<" iPair "<<iPair<<" Tx "<<par_ab[iPair][0][itr]<<" x "<<par_ab[iPair][1][itr]<<" Ty "<<par_ab[iPair][2][itr]<<" y "<<par_ab[iPair][3][itr]<<endl; 
...
void BmnMwpcTrackFinder::ReadSegments(Double_t ***par_ab, Int_t **Nhits, Double_t **Chi2_ndf, Int_t *Nbest, Double_t ***XVU, Double_t ***Clust, vector<MC_points>&vec){
  ...
  if (fDebug){
    ...
    if (Nbest[1] > 0) {
                       ChambersDen->Fill(0);
    if (Nbest[0] > 0 ) ChambersNum->Fill(0);
    }
    if (Nbest[0] > 0) {
                       ChambersDen->Fill(1);
    if (Nbest[1] > 0 ) ChambersNum->Fill(1);
    }
    if (Nbest[3] > 0) {
                       ChambersDen->Fill(2);
    if (Nbest[2] > 0 ) ChambersNum->Fill(2);
    }
    if (Nbest[2] > 0) {
                       ChambersDen->Fill(3);
    if (Nbest[3] > 0 ) ChambersNum->Fill(3);
    }
  }
  

  for (Int_t iChamber = 0; iChamber < kNChambers; iChamber++) {
  if (fDebug) cout<<" Nbest["<<iChamber<<"] = "<<Nbest[iChamber]<<endl;
...
void BmnMwpcTrackFinder::SegmentParamAlignment(Int_t chNum, Int_t *Nbest, Double_t ***par_ab, Float_t **shiftt ) {
  ...
  for (Int_t iBest = 0; iBest < Nbest[chNum]; iBest++) {
    if (fDebug) cout<<" before Pair "<<chNum<<" Tx "<<par_ab[chNum][0][iBest]<<" x "<<par_ab[chNum][1][iBest]<<" Ty "<<par_ab[chNum][2][iBest]<<" y "<<par_ab[chNum][3][iBest]<<endl;
    //                   ax     alpha                   ax^2
    ...
    if (fDebug) cout<<" after  Pair "<<chNum<<" Tx "<<par_ab[chNum][0][iBest]<<" x "<<par_ab[chNum][1][iBest]<<" Ty "<<par_ab[chNum][2][iBest]<<" y "<<par_ab[chNum][3][iBest]<<endl;
  }//iBest
  ...
  if (Nbest_Ch[first_Ch] > 0 && Nbest_Ch[Secon_Ch] > 0) {
    ...
    for (Int_t bst1 = 0; bst1 < Nbest[first_Ch]; bst1++) {
      ...
      for (Int_t bst2 = 0; bst2 < Nbest[Secon_Ch]; bst2++) {
        ...
        if (fDebug)cout<<" dx_loc "<<dx_loc<<" dy_loc "<<dy_loc<<endl;
        
        if (Chi2_m < min_Chi2m && Nvariat < Nvariations && fabs(dx_loc) < 2. && fabs(dy_loc) < 2.) {//09.2019
...
void BmnMwpcTrackFinder::SegmentMatchingAfterTarget( Int_t first_Ch, Int_t *Nbest, Double_t ***par_ab, Float_t *Zmid, Int_t **best_Ch, Int_t *Nbest_pair_, Double_t **Chi2_match_, Double_t ***XVU_Ch_, Int_t **Nhits_, Int_t **Nhits_m) {
  ...
  if (Nbest_Ch[first_Ch] > 0 && Nbest_Ch[Secon_Ch] > 0) {
    ...
    if (fDebug)cout<<" first chamber "<<endl;
    for (Int_t bst1 = 0; bst1 < Nbest[first_Ch]; bst1++) {
      if (fDebug)cout<<" Ax "<< par_ab[first_Ch][0][bst1]<<" bx "<< par_ab[first_Ch][1][bst1]<<" Ay "<< par_ab[first_Ch][2][bst1]<<" by "<< par_ab[first_Ch][3][bst1]<<endl;
    }
    if (fDebug)cout<<" second chamber "<<endl;
    for (Int_t bst2 = 0; bst2 < Nbest[Secon_Ch]; bst2++){
      if (fDebug)cout<<" Ax "<< par_ab[Secon_Ch][0][bst2]<<" bx "<< par_ab[Secon_Ch][1][bst2]<<" Ay "<< par_ab[Secon_Ch][2][bst2]<<" by "<< par_ab[Secon_Ch][3][bst2]<<endl;
    }
...
void BmnMwpcTrackFinder::PairMatching( Int_t *Nbest_p, Double_t ***par_ab_p, Float_t *kZ_midle_pair_) {
  ...
  for (Int_t pair0 = 0; pair0 < Nbest_p[0]; pair0++) {
    if (Nbest_p[0] > 1 || Nbest_p[1] > 1) continue;
    ...
    for (Int_t pair1 = 0; pair1 < Nbest_p[1]; pair1++) {
      ...
      if (Chi2_m < min_Chi2m) {
        ...
        if (fDebug){
          ...
          hTxpai0_pair1->Fill(par_ab_p[1][0][pair1],par_ab_p[0][0][pair0]);
          hTypai0_pair1->Fill(par_ab_p[1][2][pair1],par_ab_p[0][2][pair0]);
          hxpai0_pair1->Fill(par_ab_p[1][1][pair1],par_ab_p[0][1][pair0]);
          hypai0_pair1->Fill(par_ab_p[1][3][pair1],par_ab_p[0][3][pair0]);
        }
...
void BmnMwpcTrackFinder::SegmentFit(Int_t First_Ch, Float_t **z_gl_, Float_t *sig, Int_t *Nbest_pair_, Int_t ** ind_best_Ch_, Double_t ***par_ab_pair_, Double_t **Chi2_ndf_pair_, Double_t ***XVU_Ch_, Double_t ***Clust_Ch_, Int_t **ind_best_pair_,Int_t **Nhits_Ch_, Int_t **Nhits_pair_, Double_t ****sigma2_p) {
  ...
  Int_t Pair1 = 0;
  if ((fRunPeriod == 7 && First_Ch == 2) || (fRunPeriod == 8 && First_Ch == 2)) Pair1 = 1;
  ...
  for (Int_t bst = 0; bst < Nbest_pair_[Pair1]; bst++) {
    ...
    if (fRunPeriod == 8 &&  Pair1 == 0){
      //par_ab_pair_[Pair1][1][bst] = -par_ab_pair_[Pair1][1][bst];
     // par_ab_pair_[Pair1][0][bst] = -par_ab_pair_[Pair1][0][bst];
    }
    ...
    if (fDebug) cout<<" Pair "<<Pair1<<" Chi2_ndf_pair["<<Pair1<<"]["<<bst<<"] "<< Chi2_ndf_pair_[Pair1][bst]<<endl;
                    //  " Tx "<<par_ab_pair_[Pair1][0][bst]<<" x "<<par_ab_pair_[Pair1][1][bst]<<" Ty "<<par_ab_pair_[Pair1][2][bst]<<" y "<<par_ab_pair_[Pair1][3][bst]<<endl;
...
InitStatus BmnMwpcTrackFinder::Init() {
  ...
  if (fDebug){
    ChambersDen= new TH1D("ChambersDen","ChambersDen", 4, 0, 4);fList.Add(ChambersDen);
    ChambersNum= new TH1D("ChambersNum","ChambersNum", 4, 0, 4);fList.Add(ChambersNum);
    ChambersEff= new TH1D("ChambersEff","ChambersEfficiency", 4, 0, 4);fList.Add(ChambersEff);
    ChambersEff->GetXaxis()->SetBinLabel(1,"NsegCh0/Ch1}");
    ChambersEff->GetXaxis()->SetBinLabel(2,"NsegCh1/Ch0}");
    ChambersEff->GetXaxis()->SetBinLabel(3,"NsegCh2/Ch3}");
    ChambersEff->GetXaxis()->SetBinLabel(4,"NsegCh3/Ch2}");
  ...
  for (int i=0; i < kNumPairs; ++i) {
    if (fDebug){
      TH1D *h;
      if (i == 0) h = new TH1D(Form("Chi2_match_pair%d",i), Form("Chi2_match_pair%d",i), 50, 0., 5.0);
      else h = new TH1D(Form("Chi2_match_pair%d",i), Form("Chi2_match_pair%d",i), 100, 0., 10.0);
      fList.Add(h); hChi2_match_pair.push_back(h);
      ...
      h = new TH1D(Form("Chi2_ndf_pair%d",i), Form("Chi2_ndf_pair%d",i), 20, 0., 20);fList.Add(h);hChi2_ndf_pair.push_back(h);
      ...
      TH2D *h2;
      h2 = new TH2D(Form("YvsX_pair%d",i), Form("YvsX_pair%d; X[cm]; Y[cm]",i),  200, -10.,10., 200, -10.,10.);fList.Add(h2);hYvsX_pair.push_back(h2);
      h2 = new TH2D(Form("Y_X_in_target_pair%d",i), Form("posY vs posX pair%d in target; X[cm]; Y[cm]",i), 200, -10.,10., 200, -10.,10.);fList.Add(h2);hY_X_in_target_pair.push_back(h2);
      h2 = new TH2D(Form("Ax_bx_in_target_pair%d",i), Form("slopeX vs posX in target pair%d; posX[cm]; slopeX",i), 100, -10.,10., 100, -2.3, 2.3);fList.Add(h2);hAx_bx_in_target_pair.push_back(h2);
      h2 = new TH2D(Form("Ay_by_in_target_pair%d",i), Form("slopeY vs posY in target pair%d; posY[cm]; slopeY",i), 100, -10.,10., 100, -2.3, 2.3);fList.Add(h2);hAy_by_in_target_pair.push_back(h2);
    }//if (fDebug)
  ...
  if (fDebug){
    ...
    hTxpai0_pair1 =   new TH2D("Txpai0_pair1", "Tx(pai0 vs pair1)[rad]; pair1; pair0", 80,-0.02,0.02, 80,-0.02,0.02);fList.Add(hTxpai0_pair1);
    hTypai0_pair1 =   new TH2D("Typai0_pair1", "Ty(pai0 vs pair1)[rad]; pair1; pair0", 80,-0.02,0.02, 80,-0.02,0.02);fList.Add(hTypai0_pair1);
    hxpai0_pair1  =   new TH2D("xpai0_pair1", "X(pai0 vs pair1)[cm]; pair1; pair0", 200,-15,15, 200, -15,15);fList.Add(hxpai0_pair1);
    hypai0_pair1  =   new TH2D("ypai0_pair1", "Y(pai0 vs pair1)[cm]; pair1; pair0", 200,-15,15, 200, -15,15);fList.Add(hypai0_pair1);
    ...
    hdAx_target =   new TH1D("dAx_target", "SlopeX(pair0-pair1); rad; Events ", 200,  -.05, .05);fList.Add( hdAx_target);
    hdAy_target =   new TH1D("dAy_target", "SlopeY(pair0-pair1); rad; Events ", 200,  -.05, .05);fList.Add( hdAy_target);
  ...
  for(Int_t i = 0; i < kNumPairs; i++) {
    ...
    */
    ...
    if (fRunPeriod == 8){
      ...
      // normal Z
      // //pair0
      // shift_pair[0][0] =  -0.003123;//-0.0023;//Ax
      // shift_pair[0][1] =  -0.8026-0.000104;//-.41 -0.3;//x
      // shift_pair[0][2] =  -0.002229;//-0.0048;//Ay
      // shift_pair[0][3] =  0.6666+0.00018;// 1.518;//y
      // //pair1
      // shift_pair[1][0] =  -0.000965-0.000032;//Ax
      // shift_pair[1][1] =  -0.5544-0.004657;//x
      // shift_pair[1][2] =  0.00295+0.00012;//Ay
      // shift_pair[1][3] =  0.4971+0.0032;//y

      //Z fixed
      //pair0
      shift_pair[0][0] =  0.00298+0;//-0.0023;//Ax
      shift_pair[0][1] =  0.7969+0.00010;//-.41 -0.3;//x
      shift_pair[0][2] =  0.00233+0;//-0.0048;//Ay
      shift_pair[0][3] =  0.6617-0.00004;// 1.518;//y
      //pair1
      shift_pair[1][0] =  -0.000965-0.000032+0;//Ax
      shift_pair[1][1] =  -0.5544-0.004657-0.00001;//x
      shift_pair[1][2] =  0.00295+0.00012+0;//Ay
      shift_pair[1][3] =  0.4971+0.0032-0.00002;//y
      
    }
    
    //if (fDebug) cout<<" i "<<i<<" kZ_midle_pair[i] "<<kZ_midle_pair[i]<<" i1 "<<i1<<" i1+1 "<<i1+1<<" -( ZCh[i1]- ZCh[i1+1] )= "<<-( ZCh[i1]- ZCh[i1+1] )<<endl;
    for(Int_t ipar = 0; ipar < 4; ipar++) {
     if (fDebug) cout<<" ipair "<<i<<" ipar "<<ipar<<" shift_pair "<<shift_pair[i][ipar]<<endl;
    }
...
void BmnMwpcTrackFinder::Finish() {
  ...
  if (fDebug) {
    ...
    ChambersEff->Sumw2();
    ChambersEff->Divide(ChambersNum,ChambersDen,1,1);
