  ...
  m_builder()
{
  ...
  if (!in.is_open()) {
	...
	continue;
      ...
      m_ped[land_plane - 1][land_bar - 1][land_t - 1].ped = ped;
    }
    in.close();
  }
  /*
  ...
  /*
   * Read vscint file.
   * This file contains the light propagation speed in the LAND scintillators.
   */
  path = std::string(getenv("VMCWORKDIR")) + "/input/" +
  ...
  if (!in.is_open()) {
	...
	continue;
      ...
      if (end == p || globbar < 1 || globbar > 120) {
	continue;
      }
      --globbar;
    ...
    TClonesArray *land_array)
{
  ...
  for (int i = 0; i < tacquila_array->GetEntriesFast(); i++) {
    ...
    if (-1 == det.plane || -1 == det.bar || -1 == det.side) {
      /* Don't emit an error message, an unused channel could fire. */
      continue;
    }
    if (m_builder[det.plane][det.bar][det.side]) {
	...
	* diff_sync.energy_diff1 * diff_sync.energy_sync;
      Float_t vscint = m_vscint[det.plane][det.bar].vscint;
      Float_t position = (time1 - time0) * vscint;
      new (ar_land[land_array->GetEntriesFast()]) BmnLANDDigit(det.plane,
