...
{
  public:
    StorableTimeslice();   // KAA - moved from private
    /// Copy constructor.
    ...
    explicit StorableTimeslice(uint32_t num_core_microslices, uint64_t index = UINT64_MAX, uint64_t ts_pos = UINT64_MAX)
    {
                               ...
                               const uint8_t* content)
    {
        ...
        if (microslice > 0) {
            uint64_t offset = this_data.size() - this_desc.num_microslices * sizeof(MicrosliceDescriptor);
            uint64_t first_offset = reinterpret_cast<MicrosliceDescriptor*>(this_data.data())->offset;
  ...
  private:
    ...
    friend class InputArchive<Timeslice, StorableTimeslice, ArchiveType::TimesliceArchive>;
    friend class InputArchiveLoop<Timeslice, StorableTimeslice, ArchiveType::TimesliceArchive>;
