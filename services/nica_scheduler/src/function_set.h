...
// convert string to vector of strings according to the given set of delimiters
...
string find_first_number(const string& str, bool isOnlyPositive = true);  string find_first_double_number(const string& str, bool isOnlyPositive = true);
string find_last_number(const string& str, bool isOnlyPositive = true);   string find_last_double_number(const string& str, bool isOnlyPositive = true);
// extract first number or last number in string, only positive number by default (result number has string type)
...
string find_first_number(const string& str, size_t &beg_pos, size_t& end_pos, bool isOnlyPositive = true); string find_first_double_number(const string& str, size_t& beg_pos, size_t &end_pos, bool isOnlyPositive = true);
string find_last_number(const string& str, size_t &beg_pos, size_t& end_pos, bool isOnlyPositive = true);  string find_last_double_number(const string& str, size_t& beg_pos, size_t &end_pos, bool isOnlyPositive = true);
...
vector<string> string_to_vector(string str, const string& delimiters)
{
    ...
    // if no delimiters are set then return original string
    if (delimiters == "")
    {
    ...
    // replace other delimiters with the first one
    for (size_t i = 1; i < delimiters.length(); i++)
        replace(str.begin(), str.end(), delimiters[i], delimiter);

    stringstream ss(str); // Create a stringstream from the input string
    string segment;

    // Use std::getline to extract segments based on the delimiter
    while (getline(ss, segment, delimiter))
        result_vector.push_back(segment);
